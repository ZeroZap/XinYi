...
int main(void){UNITY_BEGIN();RUN_TEST(test_identity_and_config);RUN_TEST(test_wrong_identity_rejected);RUN_TEST(test_xyz_reorder_and_atomic_error);RUN_TEST(test_ready);RUN_TEST(test_read_overflow_preserves_output);RUN_TEST(test_deinit_failure_preserves_lifecycle);RUN_TEST(test_init_config_failure_is_atomic);return UNITY_END();}
