...
{ memset(d,0,sizeof(*d)); d->i2c_handle=init_without_handle ? NULL : h; d->dev_addr=a; d->timeout=t; d->base.initialized=true; return init_result; }
int xy_i2c_device_read_reg(xy_i2c_device_t *d,uint8_t r,uint8_t *p,size_t n)
{ (void)d; reads++; if(read_result)return read_result; if(r==0x0A)memcpy(p,id,n);else if(r==0x09)*p=status;else if(r==0x01)*p=gain_reg;else memcpy(p,raw,n);return XY_DEVICE_OK; }
...
void setUp(void){memset(id,0,sizeof(id));memset(raw,0,sizeof(raw));status=0;gain_reg=0x20U;read_result=write_result=init_result=init_without_handle=0;reads=0;writes=0;}
void tearDown(void){}
...
static void test_internal_transport_rejects_handle_cleared_during_init(void)
{
    xy_hmc5883l_t d;
    int bus;
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_init(&d, &bus));
...
static void test_public_operations_reject_lost_transport_without_side_effects(void)
{
    xy_hmc5883l_t d;
    xy_hmc5883l_data_t data = {7, 8, 9};
    xy_hmc5883l_field_t field = {11, 22, 33};
    xy_hmc5883l_gain_t gain = XY_HMC5883L_GAIN_8_10_GA;
    uint8_t ready = 0xA5U;
    unsigned read_snapshot;
    unsigned write_snapshot;

    init_ok(&d);
    d.data = (xy_hmc5883l_data_t){4, 5, 6};
    d.i2c_dev.i2c_handle = NULL;
    read_snapshot = reads;
    write_snapshot = writes;

    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_data_ready(&d, &ready));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM,
                          xy_hmc5883l_set_gain(&d, XY_HMC5883L_GAIN_0_88_GA));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_get_gain(&d, &gain));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_read(&d, &data));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_read_field(&d, &field));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_hmc5883l_deinit(&d));

    TEST_ASSERT_EQUAL_UINT8(0xA5U, ready);
    TEST_ASSERT_EQUAL_INT(XY_HMC5883L_GAIN_8_10_GA, gain);
    TEST_ASSERT_EQUAL_MEMORY(&((xy_hmc5883l_data_t){7, 8, 9}), &data, sizeof(data));
    TEST_ASSERT_EQUAL_MEMORY(&((xy_hmc5883l_field_t){11, 22, 33}), &field, sizeof(field));
    TEST_ASSERT_EQUAL_INT16(4, d.data.x);
    TEST_ASSERT_EQUAL_INT16(5, d.data.y);
    TEST_ASSERT_EQUAL_INT16(6, d.data.z);
    TEST_ASSERT_EQUAL_INT(XY_HMC5883L_GAIN_1_30_GA, d.gain);
    TEST_ASSERT_TRUE(d.initialized);
    TEST_ASSERT_EQUAL_UINT(read_snapshot, reads);
    TEST_ASSERT_EQUAL_UINT(write_snapshot, writes);
}
int main(void){UNITY_BEGIN();RUN_TEST(test_identity_and_config);RUN_TEST(test_wrong_identity_rejected);RUN_TEST(test_xyz_reorder_and_atomic_error);RUN_TEST(test_ready);RUN_TEST(test_gain_validation_and_transport);RUN_TEST(test_field_conversion);RUN_TEST(test_default_gain_uses_distinct_xy_and_z_sensitivity);RUN_TEST(test_read_overflow_preserves_output);RUN_TEST(test_read_field_preserves_all_public_state_on_gain_failure);RUN_TEST(test_read_field_preserves_all_public_state_on_gain_transport_error);RUN_TEST(test_init_identity_failure_clears_nested_lifecycle);RUN_TEST(test_deinit_clears_transport);RUN_TEST(test_init_config_failure_is_atomic);RUN_TEST(test_init_helper_failure_clears_nested_lifecycle);RUN_TEST(test_internal_transport_rejects_handle_cleared_during_init);RUN_TEST(test_public_operations_reject_lost_transport_without_side_effects);return UNITY_END();}
