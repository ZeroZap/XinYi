...
STALE_QMA6100_LEGACY = ROOT / "components" / "sensor" / "sensors" / "sensor_qma6100.c"
STALE_BMP390_LEGACY = ROOT / "components" / "sensor" / "sensors" / "sensor_bmp390.c"
STALE_INA219_LEGACY = ROOT / "components" / "sensor" / "sensors" / "sensor_ina219.c"
STALE_GD30DF_LEGACY = ROOT / "components" / "sensor" / "sensors" / "sensor_gd30df.c"
...
STALE_BNO055_SOURCE = ROOT / "components" / "sensor" / "src" / "xy_bno055.c"
STALE_BNO055_HEADER = ROOT / "components" / "sensor" / "inc" / "xy_bno055.h"
...
def main() -> int:
    ...
    require(len(device) == 44, f"expected 44 Device-model sources, found {len(device)}", errors)
    ...
    require(canonical_names & legacy_names ==
            {"sht30", "mpu6050", "bmp280", "bh1750", "aht20", "aht10", "ap3216c",
             "icm20608", "as5600", "max44009", "vcnl4040", "qmc5883l", "ak09918", "ist8310", "bma400", "kx023", "adxl362", "lsm6dso", "lsm6dsl", "lsm6dsr", "lsm9ds1", "lis2dh12", "apds9960"},
    ...
    require(not STALE_QMA6100_LEGACY.exists(),
            "retired invalid sensor_qma6100 lifecycle must not reappear", errors)
    require(not STALE_BMP390_LEGACY.exists(),
            "retired uncompensated sensor_bmp390 lifecycle must not reappear", errors)
    require(not STALE_INA219_LEGACY.exists(),
            "retired uncalibrated sensor_ina219 lifecycle must not reappear", errors)
    require(not STALE_GD30DF_LEGACY.exists(),
            "retired ungrounded sensor_gd30df lifecycle must not reappear", errors)
    ...
    require(not STALE_BNO055_SOURCE.exists() and not STALE_BNO055_HEADER.exists(),
            "retired experimental BNO055 lifecycle must not reappear", errors)
    ...
    print("sensor_active_source_manifest_ok legacy_active=39 legacy_subdir=38 "
          "legacy_top_level=1 experimental_test_only=6 device_active=44 "
          "approved_wrappers=23 overlap_duplicates=0 false_owners=0 "
          "hardware=mixed")
    return 0
