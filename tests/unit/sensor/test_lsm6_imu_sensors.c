...
static void test_lsm6dsl_spi_accel_gyro_init_read_deinit_and_helpers(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_ACCEL_RANGE_2G,
                            ((lsm6dsl_priv_t *)accel->priv_data)->accel_range);
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_GYRO_RANGE_250DPS, ((lsm6dsl_priv_t *)accel->priv_data)->gyro_range);
    ...
    ((lsm6dsl_priv_t *)gyro->priv_data)->gyro_range = LSM6DSL_GYRO_RANGE_250DPS;
    for (uint8_t i = 0; i < sizeof(raw); ++i) {
    ...
    TEST_ASSERT_EQUAL_INT32(1998, data.value.val_3axis.z);
    queue_spi_read8(&fake_bus, cs, LSM6DSL_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_spi_send2(&fake_bus, cs, LSM6DSL_REG_CTRL2_G & 0x7FU, 0x44U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsl_set_gyro_range(accel, LSM6DSL_GYRO_RANGE_500DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_GYRO_RANGE_500DPS, ((lsm6dsl_priv_t *)accel->priv_data)->gyro_range);

    queue_spi_read8(&fake_bus, cs, LSM6DSL_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_spi_send2(&fake_bus, cs, LSM6DSL_REG_CTRL2_G & 0x7FU, 0x42U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsl_set_gyro_range(accel, LSM6DSL_GYRO_RANGE_125DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_GYRO_RANGE_125DPS,
                            ((lsm6dsl_priv_t *)accel->priv_data)->gyro_range);

    queue_spi_read8(&fake_bus, cs, LSM6DSL_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_spi_send2(&fake_bus, cs, LSM6DSL_REG_CTRL2_G & 0x7FU, 0x4CU, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsl_set_gyro_range(accel, LSM6DSL_GYRO_RANGE_2000DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_GYRO_RANGE_2000DPS,
                            ((lsm6dsl_priv_t *)accel->priv_data)->gyro_range);

    queue_spi_read8(&fake_bus, cs, LSM6DSL_REG_CTRL1_XL, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsl_set_accel_range(accel, LSM6DSL_ACCEL_RANGE_8G));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_ACCEL_RANGE_4G, ((lsm6dsl_priv_t *)accel->priv_data)->accel_range);
    queue_spi_read8(&fake_bus, cs, LSM6DSL_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_spi_send2(&fake_bus, cs, LSM6DSL_REG_CTRL2_G & 0x7FU, 0x48U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsl_set_gyro_range(accel, LSM6DSL_GYRO_RANGE_1000DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSL_GYRO_RANGE_2000DPS,
                            ((lsm6dsl_priv_t *)accel->priv_data)->gyro_range);
...
static void test_lsm6dso_init_failure_does_not_commit_partial_config_cache(void)
{
    int bus;
    sensor_device_t *sensor = lsm6dso_create_accel("dso-fail", &bus, 0U);
    lsm6dso_priv_t *priv;

    TEST_ASSERT_NOT_NULL(sensor);
    priv = (lsm6dso_priv_t *)sensor->priv_data;
    priv->accel_range = LSM6DSO_ACCEL_RANGE_8G;
    priv->accel_rate = 208U;
    priv->gyro_range = LSM6DSO_GYRO_RANGE_1000DPS;
    priv->gyro_rate = 416U;
    sensor->odr = 208U;
    queue_i2c_read8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_WHOAMI,
                    LSM6DSO_WHOAMI_VALUE, SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL3_C, 0x01U, SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL4_C, 0x00U, SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL,
                     (uint8_t)((LSM6DSO_ACCEL_RATE_104Hz << 4) |
                               LSM6DSO_ACCEL_RANGE_2G), SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G,
                     (uint8_t)((LSM6DSO_GYRO_RATE_104Hz << 4) |
                               LSM6DSO_GYRO_RANGE_250DPS), SENSOR_EIO);

    TEST_ASSERT_EQUAL_INT(SENSOR_EIO, sensor->ops->init(sensor));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSO_ACCEL_RANGE_8G, priv->accel_range);
    TEST_ASSERT_EQUAL_UINT32(208U, priv->accel_rate);
    TEST_ASSERT_EQUAL_UINT8(LSM6DSO_GYRO_RANGE_1000DPS, priv->gyro_range);
    TEST_ASSERT_EQUAL_UINT32(416U, priv->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, sensor->odr);
    destroy_sensor(sensor);
}

static void test_lsm6dso_reinit_success_resynchronizes_public_odr(void)
{
    int bus;
    sensor_device_t *sensor = lsm6dso_create_accel("dso-reinit", &bus, 0U);
    lsm6dso_priv_t *priv;

    TEST_ASSERT_NOT_NULL(sensor);
    priv = (lsm6dso_priv_t *)sensor->priv_data;
    queue_i2c_read8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x50U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK,
                          lsm6dso_set_accel_rate(sensor, LSM6DSO_ACCEL_RATE_208Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, sensor->odr);

    queue_lsm6dso_i2c_init(&bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_WHOAMI_VALUE);
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, sensor->ops->init(sensor));
...
static void test_lsm6dso_i2c_accel_gyro_init_read_helpers_and_errors(void)
{
    ...
    ((lsm6dso_priv_t *)gyro->priv_data)->gyro_range = LSM6DSO_GYRO_RANGE_250DPS;
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, gyro->ops->read(gyro, &data));
    ...
    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x44U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dso_set_gyro_range(accel, LSM6DSO_GYRO_RANGE_500DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSO_GYRO_RANGE_500DPS,
                            ((lsm6dso_priv_t *)accel->priv_data)->gyro_range);

    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x42U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dso_set_gyro_range(accel, LSM6DSO_GYRO_RANGE_125DPS));

    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x4CU, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dso_set_gyro_range(accel, LSM6DSO_GYRO_RANGE_2000DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSO_GYRO_RANGE_2000DPS,
                            ((lsm6dso_priv_t *)accel->priv_data)->gyro_range);

    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x02U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x52U, SENSOR_EOK);
    ...
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dso_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dso_set_accel_rate(accel, 0x0BU));

    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x0AU, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL2_G, 0x5AU, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dso_set_gyro_rate(accel, LSM6DSO_GYRO_RATE_208Hz));
    ...
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dso_priv_t *)accel->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dso_set_gyro_rate(accel, 0x0BU));

    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dso_set_accel_range(accel, LSM6DSO_ACCEL_RANGE_8G));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSO_ACCEL_RANGE_4G, ((lsm6dso_priv_t *)accel->priv_data)->accel_range);
    queue_i2c_read8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0xF0U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x60U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dso_set_accel_rate(accel, LSM6DSO_ACCEL_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dso_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);

    ((lsm6dso_priv_t *)accel->priv_data)->accel_rate = 208U;
    ((lsm6dso_priv_t *)accel->priv_data)->gyro_rate = 416U;
    accel->odr = 208U;
    queue_i2c_write8(&fake_bus, LSM6DSO_ADDR_DEFAULT, LSM6DSO_REG_CTRL1_XL, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO, accel->ops->deinit(accel));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dso_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(416U, ((lsm6dso_priv_t *)accel->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);
    ...
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, accel->ops->deinit(accel));
    TEST_ASSERT_EQUAL_UINT32(0U, ((lsm6dso_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(0U, ((lsm6dso_priv_t *)accel->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(0U, accel->odr);
...
static void test_lsm6dsr_init_failure_does_not_commit_partial_config_cache(void)
{
    int bus;
    sensor_device_t *sensor = lsm6dsr_create_accel("dsr-fail", &bus, 0U);
    lsm6dsr_priv_t *priv;
    ...
    queue_i2c_write8(&bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL4_C, 0x00U, SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL,
                     (uint8_t)((LSM6DSR_ACCEL_RATE_104Hz << 4) |
                               LSM6DSR_ACCEL_RANGE_2G), SENSOR_EOK);
    queue_i2c_write8(&bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G,
                     (uint8_t)((LSM6DSR_GYRO_RATE_104Hz << 4) |
...
static void test_lsm6dsr_i2c_accel_gyro_init_read_helpers_and_errors(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, accel->ops->init(accel));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_ACCEL_RANGE_2G,
                            ((lsm6dsr_priv_t *)accel->priv_data)->accel_range);
    ...
    ((lsm6dsr_priv_t *)accel->priv_data)->accel_range = LSM6DSR_ACCEL_RANGE_4G;
    for (uint8_t i = 0; i < sizeof(raw); ++i) {
        queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT,
                        (uint8_t)(LSM6DSR_REG_OUTX_L_XL + i), raw[i], SENSOR_EOK);
    }
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, accel->ops->read(accel, &data));
    TEST_ASSERT_EQUAL_INT32(249, data.value.val_3axis.x);
    TEST_ASSERT_EQUAL_INT32(-249, data.value.val_3axis.y);
    TEST_ASSERT_EQUAL_INT32(124, data.value.val_3axis.z);

    for (uint8_t i = 0; i < sizeof(raw); ++i) {
        queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, (uint8_t)(LSM6DSR_REG_OUTX_L_G + i), raw[i], SENSOR_EOK);
    }
    ((lsm6dsr_priv_t *)gyro->priv_data)->gyro_range = LSM6DSR_GYRO_RANGE_250DPS;
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, gyro->ops->read(gyro, &data));
    ...
    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x48U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_accel_range(accel, LSM6DSR_ACCEL_RANGE_4G));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_ACCEL_RANGE_4G,
                            ((lsm6dsr_priv_t *)accel->priv_data)->accel_range);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x48U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x4CU, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_accel_range(accel, LSM6DSR_ACCEL_RANGE_8G));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_ACCEL_RANGE_8G,
                            ((lsm6dsr_priv_t *)accel->priv_data)->accel_range);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x4CU, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x44U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_accel_range(accel, LSM6DSR_ACCEL_RANGE_16G));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_ACCEL_RANGE_16G,
                            ((lsm6dsr_priv_t *)accel->priv_data)->accel_range);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x02U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x52U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_accel_rate(accel, LSM6DSR_ACCEL_RATE_208Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_accel_rate(accel, LSM6DSR_ACCEL_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x52U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x62U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_accel_rate(accel, LSM6DSR_ACCEL_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x44U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_gyro_range(accel, LSM6DSR_GYRO_RANGE_500DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_GYRO_RANGE_500DPS, ((lsm6dsr_priv_t *)accel->priv_data)->gyro_range);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x42U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_gyro_range(accel, LSM6DSR_GYRO_RANGE_125DPS));

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x40U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x4CU, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_gyro_range(accel, LSM6DSR_GYRO_RANGE_2000DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_GYRO_RANGE_2000DPS,
                            ((lsm6dsr_priv_t *)accel->priv_data)->gyro_range);
    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0xA2U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x52U, SENSOR_EOK);
    TEST_ASSERT_EQUAL_INT(0, lsm6dsr_set_gyro_rate(gyro, LSM6DSR_GYRO_RATE_208Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)gyro->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, gyro->odr);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_gyro_rate(gyro, LSM6DSR_GYRO_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)gyro->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, gyro->odr);

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x52U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x62U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_gyro_rate(gyro, LSM6DSR_GYRO_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)gyro->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, gyro->odr);
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dsr_set_gyro_rate(gyro, 0x0CU));

    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_gyro_range(accel, LSM6DSR_GYRO_RANGE_1000DPS));
    TEST_ASSERT_EQUAL_UINT8(LSM6DSR_GYRO_RANGE_2000DPS,
                            ((lsm6dsr_priv_t *)accel->priv_data)->gyro_range);
    queue_i2c_read8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0xF0U, SENSOR_EOK);
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL2_G, 0x60U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO,
                          lsm6dsr_set_gyro_rate(accel, LSM6DSR_GYRO_RATE_416Hz));
    TEST_ASSERT_EQUAL_UINT32(104U, ((lsm6dsr_priv_t *)accel->priv_data)->gyro_rate);

    ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate = 208U;
    ((lsm6dsr_priv_t *)accel->priv_data)->gyro_rate = 416U;
    accel->odr = 208U;
    queue_i2c_write8(&fake_bus, LSM6DSR_ADDR_DEFAULT, LSM6DSR_REG_CTRL1_XL, 0x00U, SENSOR_EIO);
    TEST_ASSERT_EQUAL_INT(SENSOR_EIO, accel->ops->deinit(accel));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(416U, ((lsm6dsr_priv_t *)accel->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, accel->odr);
    ...
    TEST_ASSERT_EQUAL_INT(SENSOR_EOK, accel->ops->deinit(accel));
    TEST_ASSERT_EQUAL_UINT32(0U, ((lsm6dsr_priv_t *)accel->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(0U, ((lsm6dsr_priv_t *)accel->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(0U, accel->odr);
...
static void test_lsm6dso_dsr_rate_boundaries_fail_closed(void)
{
    int bus;
    sensor_device_t *dso = lsm6dso_create_accel("dso-rate", &bus, 0U);
    sensor_device_t *dsr = lsm6dsr_create_accel("dsr-rate", &bus, 0U);

    TEST_ASSERT_NOT_NULL(dso);
    TEST_ASSERT_NOT_NULL(dsr);
    ((lsm6dso_priv_t *)dso->priv_data)->accel_rate = 208U;
    ((lsm6dso_priv_t *)dso->priv_data)->gyro_rate = 416U;
    ((lsm6dsr_priv_t *)dsr->priv_data)->accel_rate = 208U;
    ((lsm6dsr_priv_t *)dsr->priv_data)->gyro_rate = 416U;
    dso->odr = 208U;
    dsr->odr = 208U;

    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dso_set_accel_rate(NULL, LSM6DSO_ACCEL_RATE_104Hz));
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dso_set_gyro_rate(dso, 0xFFU));
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dsr_set_accel_rate(NULL, LSM6DSR_ACCEL_RATE_104Hz));
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lsm6dsr_set_gyro_rate(dsr, 0xFFU));
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dso_priv_t *)dso->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(416U, ((lsm6dso_priv_t *)dso->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, ((lsm6dsr_priv_t *)dsr->priv_data)->accel_rate);
    TEST_ASSERT_EQUAL_UINT32(416U, ((lsm6dsr_priv_t *)dsr->priv_data)->gyro_rate);
    TEST_ASSERT_EQUAL_UINT32(208U, dso->odr);
    TEST_ASSERT_EQUAL_UINT32(208U, dsr->odr);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_read_index + g_spi_send_index);
...
static void test_lsm6dsr_preserves_transport_errors(void)
{
    int bus;
    const uint8_t cs = 4U;
    sensor_data_t data = {.type = SENSOR_TYPE_CUSTOM,
                          .value.val_3axis = {44, 55, 66},
                          .timestamp = 0x5678U,
                          .accuracy = 9U};
    const sensor_data_t snapshot = data;
    sensor_device_t *sensor = lsm6dsr_create_spi_accel("dsr-errors", &bus, cs);

    TEST_ASSERT_NOT_NULL(sensor);
    queue_spi_send1(&bus, cs, (uint8_t)(LSM6DSR_REG_WHOAMI | 0x80U), SENSOR_ETIMEOUT);
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->init(sensor));
    TEST_ASSERT_EQUAL_UINT(0U, g_spi_recv_index);

    queue_spi_send1(&bus, cs, (uint8_t)(LSM6DSR_REG_OUTX_L_XL | 0x80U), SENSOR_EOK);
    g_spi_recvs[g_spi_recv_count++] =
        (spi_recv_op_t){&bus, cs, 0U, 1U, SENSOR_ETIMEOUT};
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->read(sensor, &data));
    TEST_ASSERT_EQUAL_MEMORY(&snapshot, &data, sizeof(data));
    ...
    sensor->odr = 208U;
    queue_spi_send2(&bus, cs, LSM6DSR_REG_CTRL1_XL & 0x7FU, 0x00U, SENSOR_ETIMEOUT);
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->deinit(sensor));
    TEST_ASSERT_EQUAL_UINT32(208U, sensor->odr);
