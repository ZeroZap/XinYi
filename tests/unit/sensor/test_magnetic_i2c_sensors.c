...
static void test_qmc5883l_io_failures_stop_and_preserve_outputs(void)
{
    ...
    queue_write(&fake_bus, QMC5883L_ADDR_DEFAULT, QMC5883L_REG_CONTROL2, 0x80U,
                SENSOR_ETIMEOUT);
    ...
    queue_write(&fake_bus, QMC5883L_ADDR_DEFAULT, QMC5883L_REG_CONTROL1, 0x00U,
                SENSOR_ETIMEOUT);
...
static void test_ist8310_rejects_invalid_public_boundaries_without_side_effects(void)
{
    int fake_bus;
    sensor_data_t data = {
        .type = SENSOR_TYPE_GYROSCOPE,
    ...
    const sensor_data_t snapshot = data;
    sensor_device_t *sensor = ist8310_create("ist8310-guards", &fake_bus);

    TEST_ASSERT_NULL(ist8310_create(NULL, &fake_bus));
    TEST_ASSERT_NULL(ist8310_create("ist8310-null-bus", NULL));
    TEST_ASSERT_NOT_NULL(sensor);
...
static void test_ak09918_read_propagates_first_transport_error(void)
{
    int fake_bus;
    sensor_data_t data = {.type = SENSOR_TYPE_GYROSCOPE,
                          .unit = SENSOR_UNIT_DEGREE_PER_SECOND,
                          ...
                          .timestamp = 42U};
    sensor_data_t snapshot = data;
    sensor_device_t *sensor = ak09918_create("ak09918-timeout", &fake_bus);
    TEST_ASSERT_NOT_NULL(sensor);

    queue_read(&fake_bus, AK09918_ADDR_DEFAULT, AK09918_REG_DATA, NULL, 1U, SENSOR_EOK);
    queue_read(&fake_bus, AK09918_ADDR_DEFAULT, (uint8_t)(AK09918_REG_DATA + 1U), NULL, 1U,
               SENSOR_ETIMEOUT);
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->read(sensor, &data));
    TEST_ASSERT_EQUAL_MEMORY(&snapshot, &data, sizeof(data));
...
int main(void)
{
    ...
    RUN_TEST(test_cmm905_deinit_propagates_transport_error);
    return UNITY_END();
}
