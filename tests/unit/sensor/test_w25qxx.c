...
static void test_w25qxx_transfer_failures_preserve_existing_contracts(void)
{
    ...
    queue_tx1(W25Q_CMD_READ_STATUS_REG1, XY_DEVICE_ERROR);
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_read_status(&dev, &status));
    TEST_ASSERT_EQUAL_UINT8(0xA5U, status);

    queue_tx1(W25Q_CMD_READ_STATUS_REG1, XY_DEVICE_ERROR);
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_wait_idle(&dev, 10U));
...
{
    xy_w25qxx_t dev = make_ready_dev();
    const uint8_t sector_addr[4] = {W25Q_CMD_SECTOR_ERASE, 0x00U, 0x10U, 0x00U};
    size_t op_before;

    queue_tx1(W25Q_CMD_WRITE_ENABLE, XY_DEVICE_ERROR);
    op_before = g_op_index;
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_sector_erase(&dev, 0x1000U));
    TEST_ASSERT_EQUAL_UINT(op_before + 1U, g_op_index);

    queue_tx1(W25Q_CMD_WRITE_ENABLE, XY_OK);
    queue_tx(sector_addr, sizeof(sector_addr), XY_DEVICE_ERROR);
    op_before = g_op_index;
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_sector_erase(&dev, 0x1000U));
...
static void test_w25qxx_chip_erase_stops_on_transport_failures(void)
{
    xy_w25qxx_t dev = make_ready_dev();
    const uint8_t chip_cmd = W25Q_CMD_CHIP_ERASE;
    size_t op_before;
    ...
    queue_tx1(W25Q_CMD_WRITE_ENABLE, XY_DEVICE_ERROR);
    op_before = g_op_index;
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_chip_erase(&dev));
    TEST_ASSERT_EQUAL_UINT(op_before + 1U, g_op_index);

    queue_tx1(W25Q_CMD_WRITE_ENABLE, XY_OK);
    queue_tx(&chip_cmd, sizeof(chip_cmd), XY_DEVICE_ERROR);
    op_before = g_op_index;
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_w25qxx_chip_erase(&dev));
    TEST_ASSERT_EQUAL_UINT(op_before + 2U, g_op_index);
}
...
static void test_w25qxx_write_data_at_sector_boundary_erases_then_programs(void)
{
    ...
    const uint8_t sector_addr[4] = {W25Q_CMD_SECTOR_ERASE, 0x00U, 0x10U, 0x00U};
    const uint8_t page_addr[4] = {W25Q_CMD_PAGE_PROGRAM, 0x00U, 0x10U, 0x00U};
    ...
    TEST_ASSERT_EQUAL_INT(XY_W25Q_OK, xy_w25qxx_write_data(&dev, 0x1000U, payload, sizeof(payload)));
}
