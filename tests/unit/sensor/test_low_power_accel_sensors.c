...
static void test_bma400_init_rejects_invalid_context_without_io(void)
{
    int fake_bus;
    sensor_device_t *sensor = bma400_create("bma400-init-boundary", &fake_bus);
    void *priv;
...
static void test_iis2iclp_i2c_create_init_read_deinit_and_set_range(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(8U, ((iis2iclp_priv_t *)sensor->priv_data)->range);
...
static void test_lis2dw12_set_range_rejects_invalid_range_and_null_device_without_io(void)
{
    int fake_bus;
    sensor_device_t *sensor = lis2dw12_create("lis2dw12-range", &fake_bus, 0U);

    TEST_ASSERT_NOT_NULL(sensor);
    ((lis2dw12_priv_t *)sensor->priv_data)->range = 4U;
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lis2dw12_set_range(NULL, LIS2DW12_RANGE_2G));
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lis2dw12_set_range(sensor, 0xFFU));
    TEST_ASSERT_EQUAL_UINT8(4U, ((lis2dw12_priv_t *)sensor->priv_data)->range);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_read_index);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_write_index);
...
static void test_lis2dw12_set_mode_rejects_invalid_mode_without_io_or_cache_change(void)
{
    int fake_bus;
    sensor_device_t *sensor = lis2dw12_create("lis2dw12-mode", &fake_bus, 0U);

    TEST_ASSERT_NOT_NULL(sensor);
    ((lis2dw12_priv_t *)sensor->priv_data)->mode = LIS2DW12_MODE_NORMAL;
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, lis2dw12_set_mode(sensor, 0xFFU));
    TEST_ASSERT_EQUAL_UINT8(LIS2DW12_MODE_NORMAL,
                            ((lis2dw12_priv_t *)sensor->priv_data)->mode);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_read_index);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_write_index);
...
static void test_iis2iclp_propagates_i2c_and_spi_failures_without_cache_updates(void)
{
    ...
    TEST_ASSERT_NOT_NULL(sensor);
    queue_i2c_read8(&fake_bus, IIS2ICLP_ADDR_DEFAULT, IIS2ICLP_REG_WHOAMI, 0x00U,
                    SENSOR_ETIMEOUT);
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->init(sensor));
    ...
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT,
                          iis2iclp_set_range(sensor, IIS2ICLP_RANGE_16G));
    TEST_ASSERT_EQUAL_UINT8(4U, ((iis2iclp_priv_t *)sensor->priv_data)->range);

    setUp();
    ((iis2iclp_priv_t *)sensor->priv_data)->initialized = 1U;
    queue_i2c_read8(&fake_bus, IIS2ICLP_ADDR_DEFAULT, IIS2ICLP_REG_OUT_X_L, 0U,
                    SENSOR_ETIMEOUT);
    TEST_ASSERT_EQUAL_INT(SENSOR_ETIMEOUT, sensor->ops->read(sensor, &data));
    TEST_ASSERT_EQUAL_MEMORY(&snapshot, &data, sizeof(data));

    setUp();
    TEST_ASSERT_EQUAL_INT(SENSOR_EINVAL, iis2iclp_set_range(sensor, 4U));
    TEST_ASSERT_EQUAL_UINT8(4U, ((iis2iclp_priv_t *)sensor->priv_data)->range);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_read_index);
    TEST_ASSERT_EQUAL_UINT(0U, g_i2c_write_index);
