...
static void test_init_final_status_read_failure_fails_closed(void)
{
    xy_aht20_t dev;
    int fake_bus;
    ...
    queue_read(NULL, 1U, XY_DEVICE_ERROR);
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_aht20_init(&dev, &fake_bus));
    TEST_ASSERT_EQUAL_MEMORY(&(xy_aht20_t){0}, &dev, sizeof(dev));
}
...
int main(void)
{
    ...
    RUN_TEST(test_init_final_status_read_failure_fails_closed);
