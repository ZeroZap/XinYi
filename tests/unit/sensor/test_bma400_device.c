                                  ...
                                  size_t length)
{
    ...
    if (reg == XY_BMA400_REG_CHIP_ID) {
        *data = XY_BMA400_CHIP_ID;
    } else {
...
static void test_bma400_init_read_and_deinit(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_OK, xy_bma400_read(&dev, &sample));
...
static void test_bma400_init_failure_clears_partial_transport(void)
{
    ...
    init_error = XY_DEVICE_IO_ERROR;

    TEST_ASSERT_EQUAL_INT(XY_DEVICE_IO_ERROR, xy_bma400_init(&dev, &bus));
    TEST_ASSERT_FALSE(dev.initialized);
    TEST_ASSERT_FALSE(dev.i2c_dev.base.initialized);
    TEST_ASSERT_NULL(dev.i2c_dev.i2c_handle);
}
...
static void test_bma400_fail_closed_on_invalid_nested_transport(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_bma400_read(&dev, &output));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_INVALID_PARAM, xy_bma400_deinit(&dev));
...
static void test_bma400_transport_failure_preserves_state(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_TIMEOUT, xy_bma400_read(&dev, &output));
    TEST_ASSERT_EQUAL_MEMORY(&snapshot, &output, sizeof(output));
    TEST_ASSERT_EQUAL_MEMORY(&snapshot, &dev.sample, sizeof(dev.sample));
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_TIMEOUT, xy_bma400_deinit(&dev));
    TEST_ASSERT_TRUE(dev.initialized);
