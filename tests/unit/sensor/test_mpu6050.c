...
{
    queue_init_ok();
    TEST_ASSERT_EQUAL_INT(XY_MPU6050_OK, xy_mpu6050_init_addr(dev, bus, MPU6050_ADDR_AD0_LOW));
}
...
static void test_mpu6050_device_helper_init_failure_is_propagated(void)
{
    xy_mpu6050_t dev;
    int bus;
...
static void test_mpu6050_init_config_failure_does_not_mark_ready(void)
{
    xy_mpu6050_t dev;
    int bus;
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_mpu6050_init_addr(&dev, &bus, MPU6050_ADDR_AD0_LOW));
    TEST_ASSERT_FALSE_MESSAGE(dev.initialized, "Failed configuration must not mark MPU6050 ready");
    TEST_ASSERT_EQUAL_INT(MPU6050_ACCEL_2G, dev.accel_range);
    TEST_ASSERT_EQUAL_INT(MPU6050_GYRO_250DPS, dev.gyro_range);
...
static void test_mpu6050_deinit_write_failure_preserves_initialized(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XY_DEVICE_ERROR, xy_mpu6050_deinit(&dev));
