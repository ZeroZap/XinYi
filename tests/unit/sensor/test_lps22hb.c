...
static void test_measure_read_failure_preserves_output_after_ready(void)
{
    ...
    init_ok(&dev, &iface);
    dev.last_data.pressure = 900.0f;
    dev.measurement_count = 3U;
    queue_read8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_10HZ, XY_OK);
    queue_write8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_ONE_SHOT, XY_OK);
    queue_read8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_10HZ, XY_OK);
    queue_write8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_ONE_SHOT, XY_OK);
    ...
    TEST_ASSERT_EQUAL_INT(XY_ERROR, xy_lps22hb_measure(&dev, &data, 20U));
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 900.0f, dev.last_data.pressure);
    TEST_ASSERT_EQUAL_UINT32(3U, dev.measurement_count);
}

static void test_auto_zero_read_failures_and_timeout_propagate(void)
{
    xy_lps22hb_dev_t dev;
    xy_interface_dev_t iface = fake_interface();
    ...
    queue_read8(LPS22HB_CTRL_REG2, 0x00U, XY_OK);
    queue_write8(LPS22HB_CTRL_REG2, LPS22HB_AUTOZERO, XY_ERROR);
    TEST_ASSERT_EQUAL_INT(XY_ERROR, xy_lps22hb_auto_zero(&dev));

    queue_read8(LPS22HB_CTRL_REG2, 0x00U, XY_OK);
    queue_write8(LPS22HB_CTRL_REG2, LPS22HB_AUTOZERO, XY_OK);
    for (int i = 0; i < 100; i++) {
        queue_read8(LPS22HB_CTRL_REG2, LPS22HB_AUTOZERO, XY_OK);
    }
    TEST_ASSERT_EQUAL_INT(-2, xy_lps22hb_auto_zero(&dev));
}
...
static void test_deinit_stop_failure_preserves_ready_state(void)
{
    ...
    init_ok(&dev, &iface);
    queue_read8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_10HZ, XY_OK);
    queue_write8(LPS22HB_CTRL_REG1, XY_LPS22HB_ODR_ONE_SHOT, XY_ERROR);
...
static void test_configure_fifo_rejects_invalid_mode_and_watermark_without_io(void)
{
    xy_lps22hb_dev_t dev;
    xy_interface_dev_t iface = fake_interface();
    size_t reads_before;
    ...
    TEST_ASSERT_EQUAL_INT(XY_ERROR,
                          xy_lps22hb_configure_fifo(&dev, (xy_lps22hb_fifo_mode_t)0x61U, 0U));
    TEST_ASSERT_EQUAL_INT(XY_ERROR,
                          xy_lps22hb_configure_fifo(&dev, XY_LPS22HB_FIFO_STREAM, 32U));
    TEST_ASSERT_EQUAL_UINT(reads_before, g_read_index);
    TEST_ASSERT_EQUAL_UINT(writes_before, g_write_index);
    TEST_ASSERT_EQUAL_INT(XY_LPS22HB_FIFO_BYPASS, dev.config.fifo_mode);
    TEST_ASSERT_EQUAL_UINT8(0U, dev.config.fifo_wtm);
...
static void test_configure_lpf_rejects_invalid_enum_without_io_or_cache_change(void)
{
    xy_lps22hb_dev_t dev;
    xy_interface_dev_t iface = fake_interface();
    size_t reads_before;
    ...
    TEST_ASSERT_EQUAL_INT(XY_ERROR,
                          xy_lps22hb_configure_lpf(&dev, true, (xy_lps22hb_lpf_t)0x10U));
    TEST_ASSERT_EQUAL_UINT(reads_before, g_read_index);
    TEST_ASSERT_EQUAL_UINT(writes_before, g_write_index);
    TEST_ASSERT_TRUE(dev.config.enable_lpf);
    TEST_ASSERT_EQUAL_INT(XY_LPS22HB_LPF_ODR_20, dev.config.lpf);
...
static void test_clear_interrupt_rejects_uninitialized_device_without_io(void)
{
    xy_lps22hb_dev_t dev;
    xy_interface_dev_t iface = fake_interface();
...
static void test_status_and_stop_reject_uninitialized_device_without_io(void)
{
    xy_lps22hb_dev_t dev;
    xy_interface_dev_t iface = fake_interface();
    bool ready = true;
