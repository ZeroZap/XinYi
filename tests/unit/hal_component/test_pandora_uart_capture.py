...
class PandoraUartCaptureTest(unittest.TestCase):
    ...
    def test_captures_uart_bytes_and_records_source_identity(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(record["firmware_commit_marker_matched"], True)
    ...
    def test_mismatched_firmware_commit_marker_is_review_ineligible(self):
        master, slave = pty.openpty()
        device = os.ttyname(slave)
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "uart.log"
            ...
            record = json.loads(metadata.read_text(encoding="utf-8"))
            self.assertEqual(record["status"], "CAPTURE_CONTENT_MISMATCH")
            self.assertEqual(record["firmware_commit_marker_matched"], False)
            self.assertEqual(record["runtime_evidence"], "NONE")
            self.assertIn("firmware commit marker", record["error"])
            self.assertIn("CAPTURE_CONTENT_MISMATCH", stdout)
        os.close(master)
        os.close(slave)

    def test_matching_commit_marker_before_banner_is_review_ineligible(self):
        master, slave = pty.openpty()
        device = os.ttyname(slave)
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "uart.log"
            ...
            self.assertEqual(record["firmware_commit_marker_matched"], False)
            self.assertEqual(record["runtime_evidence"], "NONE")
            self.assertIn("ordered firmware commit marker", record["error"])
            self.assertIn("CAPTURE_CONTENT_MISMATCH", stdout)
        os.close(master)
        os.close(slave)
    ...
    def test_nack_before_firmware_commit_does_not_grant_b2_candidate(self):
        master, slave = pty.openpty()
        device = os.ttyname(slave)
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "uart.log"
    ...
    def test_recovery_events_from_different_cycles_do_not_grant_b2_candidate(self):
        master, slave = pty.openpty()
        device = os.ttyname(slave)
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "uart.log"
