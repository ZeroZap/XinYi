...
static void test_ascon_128_incremental_matches_split_one_shot(void)
{
    ...
    uint8_t split_tag[XY_ASCON_128_TAG_SIZE];
    xy_ascon_128_ctx_t ctx;
    size_t chunk_index;
    size_t index;
    size_t offset;
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128_encrypt(key, nonce, ad, sizeof(ad), plaintext,
                                               sizeof(plaintext), expected_ciphertext,
                                               expected_tag));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_encrypt_init(&ctx, key, nonce));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_encrypt_ad(&ctx, ad, sizeof(ad)));
    offset = 0U;
    for (chunk_index = 0U; chunk_index < sizeof(chunks) / sizeof(chunks[0]); ++chunk_index) {
        TEST_ASSERT_EQUAL_INT(
            XY_ASCON_SUCCESS,
            xy_ascon_128_encrypt_update(&ctx, plaintext + offset, chunks[chunk_index],
                                        split_ciphertext + offset));
        offset += chunks[chunk_index];
    }
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_encrypt_final(&ctx, split_tag));
    ...
    memset(decrypted, 0xA5, sizeof(decrypted));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_decrypt_init(&ctx, key, nonce));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_decrypt_ad(&ctx, ad, sizeof(ad)));
    offset = 0U;
    for (chunk_index = 0U; chunk_index < sizeof(chunks) / sizeof(chunks[0]); ++chunk_index) {
        TEST_ASSERT_EQUAL_INT(
            XY_ASCON_SUCCESS,
            xy_ascon_128_decrypt_update(&ctx, split_ciphertext + offset, chunks[chunk_index],
                                        decrypted + offset));
        offset += chunks[chunk_index];
    }
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_decrypt_final(&ctx, split_tag));
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128_decrypt_ad(&ctx, ad, sizeof(ad)));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128_decrypt_update(&ctx, split_ciphertext,
                                                     sizeof(split_ciphertext), decrypted));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_AUTH_FAILED, xy_ascon_128_decrypt_final(&ctx, split_tag));
...
static void test_ascon_128a_incremental_matches_split_one_shot(void)
{
    ...
    uint8_t split_tag[XY_ASCON_128A_TAG_SIZE];
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_encrypt(key, nonce, ad, sizeof(ad), plaintext,
                                                sizeof(plaintext), expected_ciphertext,
                                                expected_tag));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_encrypt_init(&ctx, key, nonce));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_encrypt_ad(&ctx, ad, sizeof(ad)));
    offset = 0U;
    for (chunk_index = 0U; chunk_index < sizeof(chunks) / sizeof(chunks[0]); ++chunk_index) {
        TEST_ASSERT_EQUAL_INT(
            XY_ASCON_SUCCESS,
            xy_ascon_128a_encrypt_update(&ctx, plaintext + offset, chunks[chunk_index],
                                         split_ciphertext + offset));
        offset += chunks[chunk_index];
    }
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_encrypt_final(&ctx, split_tag));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_ciphertext, split_ciphertext, sizeof(plaintext));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_tag, split_tag, sizeof(expected_tag));
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_decrypt_ad(&ctx, ad, sizeof(ad)));
    offset = 0U;
    for (chunk_index = 0U; chunk_index < sizeof(chunks) / sizeof(chunks[0]); ++chunk_index) {
        TEST_ASSERT_EQUAL_INT(
            XY_ASCON_SUCCESS,
            xy_ascon_128a_decrypt_update(&ctx, split_ciphertext + offset, chunks[chunk_index],
                                         decrypted + offset));
        offset += chunks[chunk_index];
    }
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_decrypt_final(&ctx, split_tag));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(plaintext, decrypted, sizeof(plaintext));
    TEST_ASSERT_EQUAL_HEX8(0xA5U, decrypted[sizeof(plaintext)]);
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_AUTH_FAILED,
                          xy_ascon_128a_decrypt_final(&ctx, split_tag));
    TEST_ASSERT_EACH_EQUAL_UINT8(0U, decrypted, sizeof(plaintext));
    TEST_ASSERT_EQUAL_HEX8(0xA5U, decrypted[sizeof(plaintext)]);
}

static void assert_ascon_context_scrubbed(const xy_ascon_128a_ctx_t *ctx)
{
    xy_ascon_128a_ctx_t expected;
...
static void test_ascon_128a_incremental_scrubs_context_on_final_paths(void)
{
    uint8_t key[XY_ASCON_128A_KEY_SIZE];
    uint8_t nonce[XY_ASCON_128A_NONCE_SIZE];
    uint8_t plaintext[17];
    uint8_t ciphertext[sizeof(plaintext)];
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_encrypt_update(&ctx, plaintext, sizeof(plaintext),
                                                      ciphertext));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_encrypt_final(&ctx, tag));
    assert_ascon_context_scrubbed(&ctx);

    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_decrypt_init(&ctx, key, nonce));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_decrypt_ad(&ctx, NULL, 0U));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_decrypt_update(&ctx, ciphertext, sizeof(ciphertext),
                                                      decrypted));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS, xy_ascon_128a_decrypt_final(&ctx, tag));
    assert_ascon_context_scrubbed(&ctx);
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_decrypt_update(&ctx, ciphertext, sizeof(ciphertext),
                                                      decrypted));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_AUTH_FAILED, xy_ascon_128a_decrypt_final(&ctx, tag));
    assert_ascon_context_scrubbed(&ctx);
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_decrypt_update(&ctx, ciphertext, sizeof(ciphertext),
                                                      decrypted));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_INVALID_PARAM, xy_ascon_128a_decrypt_final(&ctx, NULL));
    ...
    TEST_ASSERT_EQUAL_INT(XY_ASCON_SUCCESS,
                          xy_ascon_128a_encrypt_update(&ctx, plaintext, sizeof(plaintext),
                                                      ciphertext));
    TEST_ASSERT_EQUAL_INT(XY_ASCON_INVALID_PARAM, xy_ascon_128a_encrypt_final(&ctx, NULL));
    assert_ascon_context_scrubbed(&ctx);
}
...
static void test_tinyjambu_incremental_rejects_length_overflow(void)
{
    uint8_t key[XY_TINYJAMBU_128_KEY_SIZE] = {0};
    uint8_t nonce[XY_TINYJAMBU_128_NONCE_SIZE] = {0};
    uint8_t input = 0x42U;
    ...
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_ad(&ctx, NULL, 0U));
    ctx.plaintext_len = SIZE_MAX;
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_encrypt_update(&ctx, &input, 1U, &output));
    TEST_ASSERT_EQUAL_HEX8(0xA5U, output);
    ...
    ctx.plaintext_len = SIZE_MAX;
    ctx.decrypt_output_len = SIZE_MAX;
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_decrypt_update(&ctx, &input, 1U, &output));
    TEST_ASSERT_EQUAL_HEX8(0xA5U, output);
}
...
static void test_tinyjambu_incremental_matches_split_one_shot(void)
{
    static const size_t chunks[] = {1U, 7U, 9U, 16U};
    uint8_t key[XY_TINYJAMBU_128_KEY_SIZE] = {0};
    uint8_t nonce[XY_TINYJAMBU_128_NONCE_SIZE] = {0};
    ...
    uint8_t split_tag[XY_TINYJAMBU_128_TAG_SIZE];
    xy_tinyjambu_128_ctx_t ctx;
    size_t chunk_index;
    size_t index;
    size_t offset;
    ...
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt(key, nonce, NULL, 0U, plaintext,
                                                   sizeof(plaintext), expected_ciphertext,
                                                   expected_tag));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_init(&ctx, key, nonce));
    ...
    for (chunk_index = 0U; chunk_index < sizeof(chunks) / sizeof(chunks[0]); ++chunk_index) {
        TEST_ASSERT_EQUAL_INT(
            ...
            xy_tinyjambu_128_encrypt_update(&ctx, plaintext + offset, chunks[chunk_index],
                                            split_ciphertext + offset));
        offset += chunks[chunk_index];
    }
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_final(&ctx, split_tag));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_ciphertext, split_ciphertext, sizeof(plaintext));
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_tag, split_tag, sizeof(expected_tag));
...
static void test_tinyjambu_incremental_accepts_zero_length_null_update(void)
{
    uint8_t key[XY_TINYJAMBU_128_KEY_SIZE] = {0};
    uint8_t nonce[XY_TINYJAMBU_128_NONCE_SIZE] = {0};
    uint8_t dummy = 0U;
    uint8_t expected_tag[XY_TINYJAMBU_128_TAG_SIZE];
    uint8_t incremental_tag[XY_TINYJAMBU_128_TAG_SIZE];
    xy_tinyjambu_128_ctx_t ctx;

    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt(key, nonce, NULL, 0U, NULL, 0U, &dummy,
                                                   expected_tag));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_init(&ctx, key, nonce));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_ad(&ctx, NULL, 0U));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_encrypt_update(&ctx, NULL, 1U, &dummy));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_encrypt_update(&ctx, &dummy, 1U, NULL));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_update(&ctx, NULL, 0U, NULL));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_encrypt_final(&ctx, incremental_tag));
    ...
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_decrypt_ad(&ctx, NULL, 0U));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_decrypt_update(&ctx, NULL, 1U, &dummy));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_INVALID_PARAM,
                          xy_tinyjambu_128_decrypt_update(&ctx, &dummy, 1U, NULL));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
                          xy_tinyjambu_128_decrypt_update(&ctx, NULL, 0U, NULL));
    TEST_ASSERT_EQUAL_INT(XY_TINYJAMBU_SUCCESS,
...
int main(void)
{
    ...
    RUN_TEST(test_tinyjambu_incremental_rejects_length_overflow);
