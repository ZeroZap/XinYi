...
#define AT_RESP_DOWNLOAD_OK                 "DOWNLOAD"
...
at_response_t at_resp_set_tiemout(at_response_t resp, rt_int32_t timeout)
{
			resp->timeout = timeout;
	    return resp;
}
...
static const struct at_urc *get_urc_obj(at_client_t client)  //如果是URC数据就返回
{
    ...
    for (i = 0; i < client->urc_table_size; i++)
    {
        for (j = 0; j < client->urc_table[i].urc_size; j++)
        {
            ...
            if ((prefix_len ? !rt_strncmp(buffer, urc->cmd_prefix, prefix_len) : 1)									//校验head
                    && (suffix_len ? !rt_strncmp(buffer + bufsz - suffix_len, urc->cmd_suffix, suffix_len) : 1))   //校验end
            {
                return urc;
            }
        }
...
static int at_recv_readline(at_client_t client)
{
    ...
    while (1)
    {
        ...
        if ((client->urc = get_urc_obj(client)) != RT_NULL || (ch == '\n' && last_ch == '\r')  //URC数据 或者新行
                || (client->end_sign != 0 && ch == client->end_sign))							//结束标志    
        {
            if (is_full)																		//溢出
            {
...
static void client_parser(at_client_t client)
{
    while(1)
    {
        if (at_recv_readline(client) > 0)                //读出一行的字节数
        {
            if (client->urc != RT_NULL)                  //如果是URC的数据就调用URC处理函数
            {
            ...
            else if (client->resp != RT_NULL)				//那么就有一行的数据
            {
                ...
                client->recv_line_buf[client->recv_line_len - 1] = '\0';     //将\n变成\0
                        ...
                        && resp->line_num == 0)
                {
                    ...
                    client->resp_status = AT_RESP_OK;
								...
								else if (rt_memcmp(client->recv_line_buf, AT_RESP_DOWNLOAD_OK, rt_strlen(AT_RESP_END_OK)) == 0
                        && resp->line_num == 0)
                {
                    /* get the end data by response result, return response state END_OK. */
                    client->resp_status = AT_RESP_OK;
