 ...
 * @brief    作业读操作
 */
static unsigned int at_work_read(struct at_work_ctx *e, void *buf,
                                 unsigned int len)
{
    at_obj_t *at = e->at;
 ...
 * @brief    作业写操作
 */
static unsigned int at_work_write(struct at_work_ctx *e, const void *buf,
                                  unsigned int len)
{
...
{
    at_urc_ctx_t context = { at->adap.read, urcline, at->adap.urc_bufsize,
                             size };
    at->adap.debug("<=\r\n%s\r\n", urcline);
