 ...
 * Date           Author       Notes
 * 2020-01-02     Morro        初版
 * 2021-01-20     Morro        增加debug调试接口,
 *解决链表未初始化导至段错误的问题 调通单行命令、多行命令、自定义命令等接口
 ...
 * @brief  AT执行回调
 */
static void do_at_callbatk(at_obj_t *a, at_item_t *i, at_callbatk_t cb,
                           at_return ret)
{
    at_response_t r;
...
static void at_work_manager(at_obj_t *at)
{
    ...
    /*作业处理表
     * --------------------------------------------------------------*/
    static int (*const work_handler_table[])(
        at_obj_t *) = { do_work_handler, do_cmd_handler, send_multiline_handler,
