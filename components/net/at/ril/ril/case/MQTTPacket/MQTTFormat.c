...
int MQTTStringFormat_connect(char* strbuf, int strbuflen, MQTTPacket_connectData* data)
{
	...
	if (data->willFlag)
		strindex += snprintf(&strbuf[strindex], strbuflen - strindex,
				...
				data->will.qos, data->will.retain,
...
int MQTTStringFormat_publish(char* strbuf, int strbuflen, unsigned char dup, int qos, unsigned char retain,
		unsigned short packetid, MQTTString topicName, unsigned char* payload, int payloadlen)
{
	int strindex = snprintf(strbuf, strbuflen,
				"PUBLISH dup %d, QoS %d, retain %d, packet id %d, topic %.*s, payload length %d, payload %.*s",
				dup, qos, retain, packetid,
...
char* MQTTFormat_toClientString(char* strbuf, int strbuflen, unsigned char* buf, int buflen)
{
	...
	case PUBLISH:
	{
		...
		if (MQTTDeserialize_publish(&dup, &qos, &retain, &packetid, &topicName,
				&payload, &payloadlen, buf, buflen) == 1)
			strindex = MQTTStringFormat_publish(strbuf, strbuflen, dup, qos, retain, packetid,
