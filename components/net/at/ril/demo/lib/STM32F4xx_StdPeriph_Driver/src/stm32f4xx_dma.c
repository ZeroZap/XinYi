...
/**
  ...
  * @retval None
  */
void DMA_DeInit(DMA_file_typeDef* DMAy_Streamx)
{
...
/**
  ...
  * @retval None
  */
void DMA_Init(DMA_file_typeDef* DMAy_Streamx, DMA_InitTypeDef* DMA_InitStruct)
{
  uint32_t tmpreg = 0;
...
/**
  ...
  * @retval None
  */
void DMA_Cmd(DMA_file_typeDef* DMAy_Streamx, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  ...
  * @retval None
  */
void DMA_PeriphIncOffsetSizeConfig(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_Pincos)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  ...
  * @retval None
  */
void DMA_FlowControllerConfig(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_FlowCtrl)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
@verbatim   
 ...
 ===============================================================================  
  ...
  The following function can be used to write the Stream data counter value:
    - void DMA_SetCurrDataCounter(DMA_file_typeDef* DMAy_Streamx, uint16_t Counter);
...
@note If the Source and Destination Data Sizes are different, then the value written in
  ...
  The following function can be used to read the Stream data counter value:
     - uint16_t DMA_GetCurrDataCounter(DMA_file_typeDef* DMAy_Streamx);
...
@verbatim   
 ...
 ===============================================================================  
  ...
  Two functions must be called before calling the DMA_Init() function:
   - void DMA_DoubleBufferModeConfig(DMA_file_typeDef* DMAy_Streamx, uint32_t Memory1BaseAddr,
                                uint32_t DMA_CurrentMemory);
   - void DMA_DoubleBufferModeCmd(DMA_file_typeDef* DMAy_Streamx, FunctionalState NewState);
  ...
  stopped) to modify on of the target Memories addresses or to check wich Memory target is currently
   used:
    - void DMA_MemoryTargetConfig(DMA_file_typeDef* DMAy_Streamx, uint32_t MemoryBaseAddr,
                            uint32_t DMA_MemoryTarget);
    - uint32_t DMA_GetCurrentMemoryTarget(DMA_file_typeDef* DMAy_Streamx);
...
@verbatim   
 ...
 1. DMA Enable status:
   ...
   To monitor this state it is possible to use the following function:
     - FunctionalState DMA_GetCmdStatus(DMA_file_typeDef* DMAy_Streamx); 
 
 2. FIFO Status:
   It is possible to monitor the FIFO status when a transfer is ongoing using the following 
   function:
     - uint32_t DMA_GetFIFOStatus(DMA_file_typeDef* DMAy_Streamx); 
 ...
 3. DMA Interrupts and Flags:
  ...
  =============
   ...
   In this Mode it is advised to use the following functions:
      - FlagStatus DMA_GetFlagStatus(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_FLAG);
      - void DMA_ClearFlag(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_FLAG);
  ...
  In this Mode it is advised to use the following functions:
     - void DMA_ITConfig(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_IT, FunctionalState NewState);
     - ITStatus DMA_GetITStatus(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_IT);
     - void DMA_ClearITPendingBit(DMA_file_typeDef* DMAy_Streamx, uint32_t DMA_IT);
