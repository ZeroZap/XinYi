...
All Net product protocols are default-off. `NETWORK=ON` only enables the core library;
MQTT, AT client, AT server, CAN, and LTE each require an explicit root Kconfig opt-in.
The active AT owners are `at_client.c` and `xy_ats.c`; the vendor-style
AT-Command-V2, FreeRTOS Cellular, and RT-Thread AT trees are reference/imported
material and are not linked into `xy_net`. The active MQTT owner is
`src/xy_mqtt_client.c`; `xy_mqtt/` remains legacy reference material. CAN and LTE
remain direct opt-ins with Host contracts only, pending controller/modem board evidence.
...
#define CONFIG_NETWORK          1  // core only; default is 0
#define CONFIG_PROTO_MQTT       1  // explicit opt-in; default is 0
#define CONFIG_PROTO_AT_CLIENT  1  // required by PROTO_LTE
#define CONFIG_PROTO_AT_SERVER  0
#define CONFIG_PROTO_CAN        0
#define CONFIG_PROTO_LTE        0
...
Root Kconfig selections now drive the matching source list and public umbrella
exports. Enabling `PROTO_MQTT` selects the active MQTT owner; enabling `PROTO_LTE`
requires the active AT client. None of these selections constitutes modem, CAN
controller, long-run, performance, or hardware evidence.
