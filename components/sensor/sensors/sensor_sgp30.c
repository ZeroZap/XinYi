...
static sensor_err_t sgp30_read(sensor_device_t *s, sensor_data_t *d)
{
    ...
    d->value.val_float = 100.0f;
    d->timestamp = SENSOR_GET_TICK();
    return SENSOR_EOK;
}

static sensor_err_t sgp30_deinit(sensor_device_t *s)
{
    if (s == NULL || s->priv_data == NULL || s->bus == NULL) {
        return SENSOR_EINVAL;
    }
...
static const sensor_ops_t sgp30_ops = {.init = sgp30_init, .deinit = sgp30_deinit, .read = sgp30_read};
...
sensor_device_t *sgp30_create(const char *name, void *i2c_bus)
{
    ...
    sgp30_priv_t *p = (sgp30_priv_t *)SENSOR_MALLOC(sizeof(sgp30_priv_t));
    if (!s || !p) {
        SENSOR_FREE(s);
        SENSOR_FREE(p);
        return NULL;
