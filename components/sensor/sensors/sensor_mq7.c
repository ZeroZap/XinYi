...
static sensor_err_t mq7_init(sensor_device_t *s)
{
    ...
    SENSOR_LOG("Initializing MQ7");
    return SENSOR_EOK;
}

static sensor_err_t mq7_read(sensor_device_t *s, sensor_data_t *d)
{
    ...
    d->value.val_float = hal_adc_read(((mq7_priv_t *)s->priv_data)->adc_pin) / 10.0f;
...
sensor_device_t *mq7_create(const char *name, uint8_t adc_pin)
{
    ...
    mq7_priv_t *p = (mq7_priv_t *)SENSOR_MALLOC(sizeof(mq7_priv_t));
    if (!s || !p) {
        SENSOR_FREE(s);
        SENSOR_FREE(p);
        return NULL;
