...
static sensor_err_t pa122_init(sensor_device_t *s)
{
    if (s == NULL || s->priv_data == NULL) {
        return SENSOR_EINVAL;
    }

    SENSOR_LOG("Initializing PA122");
    return SENSOR_EOK;
}
...
static sensor_err_t pa122_read(sensor_device_t *s, sensor_data_t *d)
{
    ...
    d->value.val_float = (st & 0x80) ? 50.0f : 200.0f;
