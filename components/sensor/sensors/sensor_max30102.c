...
static sensor_err_t max30102_init(sensor_device_t *s)
{
    if (s == NULL || s->priv_data == NULL) {
        return SENSOR_EINVAL;
    }

    SENSOR_LOG("Initializing MAX30102");
    return SENSOR_EOK;
}
...
static sensor_err_t max30102_read(sensor_device_t *s, sensor_data_t *d)
{
    ...
    d->type = SENSOR_TYPE_HEART_RATE;
    d->value.val_float = 72.0f;
...
sensor_device_t *max30102_create(const char *name, void *i2c_bus)
{
    ...
    p->i2c_addr = MAX30102_ADDR;
    strncpy(s->info.name, name, SENSOR_NAME_MAX_LEN - 1);
    s->info.vendor = "Maxim";
    s->info.model = "MAX30102";
    s->info.type = SENSOR_TYPE_HEART_RATE;
    s->ops = &max30102_ops;
    s->bus = i2c_bus;
    s->priv_data = p;
