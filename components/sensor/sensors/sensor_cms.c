...
static const sensor_ops_t cms_ops = {
    .init = cms_init, .deinit = cms_deinit, .read = cms_read};
...
sensor_device_t *cms_create(const char *name, void *i2c_bus, uint8_t addr)
{
    ...
    cms_priv_t *priv = (cms_priv_t *)SENSOR_MALLOC(sizeof(cms_priv_t));
    ...
    priv->i2c_addr = (addr != 0U) ? addr : CMS_ADDR_DEFAULT;
    strncpy(sensor->info.name, name, SENSOR_NAME_MAX_LEN - 1U);
    sensor->info.vendor = "CRmicro";
    sensor->info.model = "CMS";
