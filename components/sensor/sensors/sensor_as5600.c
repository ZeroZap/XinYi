...
static sensor_err_t as5600_init(sensor_device_t *sensor)
{
    if (sensor == NULL || sensor->priv_data == NULL) {
        return SENSOR_EINVAL;
    }

    SENSOR_LOG("Initializing AS5600");
    return SENSOR_EOK;
}

static sensor_err_t as5600_read(sensor_device_t *sensor, sensor_data_t *data)
{
    ...
    uint8_t buf[2];
    as5600_priv_t *priv = (as5600_priv_t *)sensor->priv_data;
    sensor_err_t ret = (sensor_err_t)hal_i2c_mem_read(sensor->bus, priv->i2c_addr, 0x0E, buf, 2);
    if (ret != SENSOR_EOK) {
        return ret;
    }
...
sensor_device_t *as5600_create(const char *name, void *i2c_bus)
{
    ...
    as5600_priv_t *priv = (as5600_priv_t *)SENSOR_MALLOC(sizeof(as5600_priv_t));
