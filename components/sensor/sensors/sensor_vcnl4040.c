...
static sensor_err_t vcnl4040_init(sensor_device_t *s)
{
    if (s == NULL || s->priv_data == NULL) {
        return SENSOR_EINVAL;
    }

    SENSOR_LOG("Initializing VCNL4040");
    return SENSOR_EOK;
}
...
static sensor_err_t vcnl4040_read(sensor_device_t *s, sensor_data_t *d)
{
    ...
    d->value.val_float = (float)(((uint16_t)buf[1] << 8) | buf[0]);
...
sensor_device_t *vcnl4040_create(const char *name, void *i2c_bus)
{
    ...
    p->i2c_addr = VCNL4040_ADDR;
    strncpy(s->info.name, name, SENSOR_NAME_MAX_LEN - 1);
    s->info.vendor = "Vishay";
    s->info.model = "VCNL4040";
    s->info.type = SENSOR_TYPE_PROXIMITY;
    s->ops = &vcnl4040_ops;
    s->bus = i2c_bus;
    s->priv_data = p;
