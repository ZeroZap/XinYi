                                     ...
                                     fee_page_header_t *hdr)
{
    ...
    fee_status_t status =
        flash_read(addr, (uint8_t *)hdr, FEE_PAGE_HEADER_BASE_SIZE);
    if (status != FEE_OK)
        return status;
                                      ...
                                      fee_page_header_t *hdr)
{
    ...
    for (uint16_t offset = 0; offset < aligned_size; offset += gran) {
        fee_status_t status =
            flash_write(h, addr + offset, g_work_buffer + offset, gran);
        if (status != FEE_OK)
            return status;
    }
...
static uint16_t calc_write_offset(fee_handle_t *h, uint8_t page_idx)
{
    ...
    while (offset + rec_size <= max_size) {
        ...
        if (flash_read(addr + offset, (uint8_t *)&rec, FEE_RECORD_HEADER_SIZE)
            != FEE_OK) {
            break;
        }
...
static fee_status_t rebuild_cache(fee_handle_t *h, uint8_t page_idx)
{
    ...
    while (offset + gran <= max_size) {
        ...
        if (flash_read(
                addr + offset + FEE_RECORD_HEADER_SIZE, data_buf, data_size)
            == FEE_OK) {
                      ...
                      uint8_t *cache_buffer, uint8_t *work_buffer)
{
    ...
    handle->fee_page_size =
        config->flash_page_size * config->pages_per_fee_page;
