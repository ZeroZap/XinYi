...
| RGB extended effects compile seam | `effects/xy_rgb_fx_music.c`, `effects/xy_rgb_fx_matrix.c`, serial-RGB public headers, fake RGB-strip fixture | `gui_rgb_extended_effects_compile` | music extended implementation 已由 fake `xy_rgb_*` strip seam 与 test-owned `g_frame_count` 覆盖 setter/beat/frequency/autocorr/VU 基础路径；matrix extended implementation 已覆盖 2D matrix size/plasma 基础路径；该目标只证明 compile-boundary 与低风险调用契约，不代表视觉算法质量或硬件验证 |
