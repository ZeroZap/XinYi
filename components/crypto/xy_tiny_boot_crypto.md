...
| **Random Number Generator** | ChaCha20-based CSPRNG | Ephemeral key generation | ~1.5 KB |
| **Protocol Logic** | State machine + handlers | Bootloader protocol flow | ~2.0 KB |

**Total Code Size**: ~20.5 KB (4.5 KB + 9.0 KB + 3.5 KB + 1.5 KB + 2.0 KB)
...
### 5. Random Number Generator (CSPRNG)

**Purpose**: Generate cryptographically secure random numbers for ephemeral keys and nonces

#### Why RNG is Critical

⚠️ **Security Warning**: Weak or predictable random numbers completely compromise cryptographic security:
- **X25519**: Ephemeral private keys must be unpredictable
- **ChaCha20**: Nonces must never repeat with same key
- **Session keys**: Must be unique per boot session

#### Implementation Strategy: ChaCha20-based CSPRNG
...
/**
 * @brief CSPRNG state (ChaCha20-based)
 */
typedef struct {
...
static xy_csprng_ctx_t g_rng_ctx;

/**
 * @brief Initialize CSPRNG with hardware entropy
 *
 * CRITICAL: This function MUST be called once at bootloader startup
 * with a true random seed from hardware sources.
 */
void xy_csprng_init(void) {
    uint8_t seed[32];

    // ===== Entropy Sources (Platform-Specific) =====

    // Option 1: Hardware RNG (if available)
    #ifdef HAS_HARDWARE_RNG
    hw_rng_read(seed, 32);
    #endif

    // Option 2: ADC noise sampling (fallback)
    #ifdef HAS_ADC
    uint32_t entropy = 0;
    for (int i = 0; i < 32; i++) {
        // Sample ADC noise from floating pin
        uint16_t adc_value = adc_read_noise();

        // Mix with previous readings
        entropy ^= adc_value;
        entropy = (entropy << 1) | (entropy >> 31);

        // Accumulate timing jitter
        entropy ^= system_tick_count();

        seed[i] = (uint8_t)(entropy & 0xFF);
    }
    #endif

    // Option 3: Unique device ID + timestamp (weaker)
    #ifdef FALLBACK_ENTROPY
    uint8_t device_id[12];
    uint32_t timestamp;

    read_unique_device_id(device_id, 12);
    timestamp = rtc_get_timestamp();

    // Hash device ID + timestamp + boot count
    xy_blake2s_ctx_t hash_ctx;
    xy_blake2s_init(&hash_ctx, 32);
    xy_blake2s_update(&hash_ctx, device_id, 12);
    xy_blake2s_update(&hash_ctx, (uint8_t*)&timestamp, 4);
    xy_blake2s_final(&hash_ctx, seed, 32);
    #endif

    // Initialize ChaCha20 state with seed
    memcpy(g_rng_ctx.key, seed, 32);
    g_rng_ctx.counter = 0;
    g_rng_ctx.available = 0;
 ...
 */
void xy_csprng_generate(uint8_t *output, size_t len) {
    size_t offset = 0;

    while (offset < len) {
        // Refill buffer if empty
        if (g_rng_ctx.available == 0) {
            uint8_t nonce[12] = {0};
            uint8_t plaintext[64] = {0};

            // Encode counter in nonce
            memcpy(nonce + 4, &g_rng_ctx.counter, 8);

            // Generate 64 bytes of keystream
            xy_chacha20_ctx_t chacha_ctx;
            xy_chacha20_init(&chacha_ctx, g_rng_ctx.key, nonce, 0);
            xy_chacha20_update(&chacha_ctx, plaintext, 64,
                               g_rng_ctx.buffer);

            g_rng_ctx.counter++;
            g_rng_ctx.available = 64;

            // Rekey every 2^20 bytes (prevent state compromise)
            if ((g_rng_ctx.counter % 16384) == 0) {
                memcpy(g_rng_ctx.key, g_rng_ctx.buffer, 32);
            }
        }

        // Copy available bytes to output
        size_t to_copy = (len - offset < g_rng_ctx.available)
                         ? (len - offset) : g_rng_ctx.available;

        memcpy(output + offset,
               g_rng_ctx.buffer + (64 - g_rng_ctx.available),
               to_copy);

        offset += to_copy;
        g_rng_ctx.available -= to_copy;
    }
}

/**
 * @brief Generate X25519 ephemeral keypair with CSPRNG
 */
void generate_ephemeral_keypair(uint8_t private_key[32],
                                 uint8_t public_key[32]) {
    // Generate random private key
    xy_csprng_generate(private_key, 32);
    ...
    // Compute public key
    xy_x25519_public_key(public_key, private_key);
}
```

#### Entropy Source Comparison

| Source | Quality | Availability | Notes |
|--------|---------|--------------|-------|
| **Hardware RNG** | ⭐⭐⭐⭐⭐ | STM32L0/L4/H7 | Best option, FIPS 140-2 compliant |
| **ADC Noise** | ⭐⭐⭐⭐ | Most MCUs | Sample floating pin, ~8 bits/sample |
| **Clock Jitter** | ⭐⭐⭐ | All MCUs | Use PLL jitter, slower collection |
| **Device ID + Time** | ⭐⭐ | All MCUs | Predictable, only for mixing |
| **Compile-time seed** | ⭐ | All | ❌ Never use alone (predictable) |

**Recommendation**: Combine multiple sources for defense-in-depth.

#### Resource Usage

- **Stack**: ~150 bytes (CSPRNG context)
- **Flash**: ~1.5 KB (ChaCha20 keystream generation)
- **Execution**: ~50µs per 32 bytes @ 48MHz (Cortex-M0+)

#### Security Considerations

1. **Initialization**:
   - ✅ Call `xy_csprng_init()` once at boot with hardware entropy
   - ❌ Never use uninitialized CSPRNG
   - ✅ Verify entropy quality (e.g., health tests)

2. **Reseeding**:
   - Automatic rekey every 1 MB of output
   - Optional: Reseed from hardware RNG periodically

3. **State Protection**:
   - Store CSPRNG state in RAM (not persistent)
   - Zeroize state on bootloader exit

4. **Testing**:
   ```c
   // Simple statistical test (detect stuck-at faults)
   void test_rng_quality(void) {
       uint8_t sample[256];
       uint32_t ones = 0;

       xy_csprng_generate(sample, 256);

       for (int i = 0; i < 256; i++) {
           for (int bit = 0; bit < 8; bit++) {
               if (sample[i] & (1 << bit)) ones++;
           }
       }

       // Should be close to 1024 (50% ones)
       // Fail if < 900 or > 1124 (outside 12% margin)
       if (ones < 900 || ones > 1124) {
           bootloader_panic("RNG health check failed");
       }
   }
   ```
...
| **CSPRNG (ChaCha20-based)** | 1.5 KB | Random number generation |
| **Protocol logic** | 2.0 KB | State machine + handlers |
| **Total crypto** | ~20.5 KB | Complete bootloader crypto |
...
| **CSPRNG state** | 150 B | 32 B (RNG key) |
| **Packet buffers** | 0 B | 512 B (2x256B) |
| **Total RAM** | ~650 B | ~704 B |
| **Grand Total** | **~1.4 KB** | (Safe for M0 with 8KB+ RAM) |
...
| **CSPRNG generate** | ~50 µs | ~640 KB/s (32 bytes) |
...
|----------|-----------|-----|----------|-----------|--------------|-------------|------------|
| **xy_tiny_boot_crypto** | 20.5 KB | 1.4 KB | ⭐⭐⭐⭐⭐ | ~15 ms | ~35 s | ~35 ms | ~50 s |
...
#define XY_CRYPTO_ENABLE_CSPRNG             1  // ChaCha20-based RNG
```
...
CFLAGS += -DXY_CRYPTO_ENABLE_CSPRNG=1
```
...
4. **Random Number Generation**:
   - ⚠️ **CRITICAL**: Initialize CSPRNG with hardware entropy
   - Use multiple entropy sources (HW RNG + ADC noise + jitter)
   - Perform health checks on RNG output
   - Never use predictable seeds (e.g., compile-time constants)
   - Reseed periodically if long-running
...
**Optimization Strategy A: 32-bit Limb Representation**
```c
...
**Trade-off**: +500 bytes code size

**Optimization Strategy B: Cortex-M0 Assembly (Haase/Schwabe approach)**

Based on the `curve25519-cortexm0` reference implementation, use:

1. **Packed 256-bit Representation** (8 × 32-bit limbs)
   ```c
   typedef union UN_256bitValue_ {
       uint8_t  as_uint8[32];
       uint16_t as_uint16[16];
       uint32_t as_uint32[8];   // Primary representation
...
2. **Assembly-Optimized Core Operations**
   - `multiply256x256_asm()`: 256×256→512 bit multiply
   - `square256_asm()`: Optimized squaring
   - `fe25519_reduceTo256Bits_asm()`: Fast modular reduction
   - `fe25519_mpyWith121666_asm()`: Constant multiplication

3. **On-the-Fly Reduction**
   ```c
   static void fe25519_add(fe25519 *out,
                           const fe25519 *a,
                           const fe25519 *b) {
       uint64_t accu = 0;

       // Add MSW first for on-the-fly reduction
       accu = a->as_uint32[7] + b->as_uint32[7];
       out->as_uint32[7] = ((uint32_t)accu) & 0x7FFFFFFF;

       // Propagate carry with 19x multiplier
       accu = ((uint32_t)(accu >> 31)) * 19;

       for (int i = 0; i < 7; i++) {
           accu += a->as_uint32[i] + b->as_uint32[i];
           out->as_uint32[i] = (uint32_t)accu;
           accu >>= 32;
       }
       accu += out->as_uint32[7];
       out->as_uint32[7] = (uint32_t)accu;
   }
   ```

4. **Montgomery Ladder with Conditional Swap**
   ```c
   static void fe25519_cswap(fe25519 *a, fe25519 *b,
                             int condition) {
       int32_t mask = -condition;  // Constant-time mask

       for (int i = 0; i < 8; i++) {
           uint32_t val1 = a->as_uint32[i];
           uint32_t val2 = b->as_uint32[i];
           uint32_t temp = val1;
...
**Implementation Recommendation: xy_25519_m0**

Create a specialized M0 implementation:
```
components/crypto/xy_25519/
├── xy_25519.c              # Generic C implementation
├── xy_25519.h              # Common API
├── xy_25519_m0.c          # M0-optimized C code
├── cortex_m0_mpy121666.s  # Assembly: multiply by 121666
├── cortex_m0_reduce25519.s # Assembly: modular reduction
├── mul.s                   # Assembly: 256×256 multiply
└── sqr.s                   # Assembly: 256-bit square
```
...
**Key Optimizations from Reference Implementation**:
1. ✅ Use 8×32-bit packed representation (not 10×25.5-bit)
2. ✅ Assembly for 256×256 multiply (critical path)
3. ✅ On-the-fly reduction in add/sub
4. ✅ Constant-time conditional swap
5. ✅ Optional: Replace last 3 ladder steps with doubling
6. ✅ Inversion using Fermat's little theorem (optimized chain)
...
| **xy_25519_m0 (ASM)** | +2 KB | 0 | **4-5x (X25519)** | ⭐⭐⭐⭐ Very Hard |
| **M0+ ROR support** | 0 | 0 | 1.2x | ⭐ Easy |
| **HW crypto (AES)** | +2 KB | +256 B | 10x | ⭐⭐⭐⭐ Very Hard |

**Quick Wins** (Phase 1): Compiler + loop unrolling = **2x speedup**
**Advanced** (Phase 2): Add xy_25519_m0 assembly = **3.3x total speedup** (50s → 15s)

**Note**: xy_25519_m0 is the single most impactful optimization for M0 bootloaders, reducing X25519 from 15ms to 3-4ms.

### Advanced: xy_25519_m0 Dedicated Implementation

#### Rationale for Specialized M0 Version

Based on the `curve25519-cortexm0` reference implementation by Haase & Schwabe, creating a dedicated [xy_25519_m0](file://d:projXinYicomponentscryptoxy_25519) variant can achieve **3-5x performance improvement** for X25519 on Cortex-M0.

#### Implementation Architecture

```
xy_25519/
├── xy_25519.h                  # Common API (unchanged)
├── xy_25519.c                  # Generic implementation
├── xy_25519_m0.c              # M0-optimized C wrapper
├── fe25519_m0.c               # M0 field arithmetic
├── asm/
│   ├── cortex_m0_mpy121666.s  # Multiply by (A+2)/4 = 121666
│   ├── cortex_m0_reduce.s     # Fast modular reduction
│   ├── mul256x256.s           # 256×256→512 multiply
│   └── square256.s            # Optimized squaring
└── Makefile
```

#### Key Design Decisions

| Aspect | Generic xy_25519 | xy_25519_m0 | Improvement |
|--------|------------------|-------------|-------------|
| **Representation** | 10×25.5-bit limbs | 8×32-bit limbs | Simpler reduction |
| **Multiply** | C with 64-bit | Assembly 32×32 | 3-4x faster |
| **Square** | Generic multiply | Dedicated squaring | 2x faster |
| **Reduction** | Separate step | On-the-fly | Eliminates overhead |
| **Ladder** | 255 steps | 252 + 3 doubles | Slight optimization |
| **X25519 Time** | ~15 ms @ 48MHz | ~3-5 ms @ 48MHz | **3-5x faster** |

#### Assembly Functions Required

**1. 256×256-bit Multiplication** (`mul256x256.s`)
```assembly
@ Input:  r0 = result pointer (512-bit)
@         r1 = operand a pointer (256-bit)
@         r2 = operand b pointer (256-bit)
@ Output: [r0] = a * b (512-bit)
...
multiply256x256_asm:
    ...
    @ Optimized Comba multiplication
    @ Uses register scheduling for M0
    @ ~300-400 cycles on Cortex-M0
...
**2. Modular Reduction** (`cortex_m0_reduce.s`)
```assembly
@ Input:  r0 = result pointer (256-bit)
@         r1 = input pointer (512-bit)
@ Output: [r0] = [r1] mod (2^255-19)
...
fe25519_reduceTo256Bits_asm:
    ...
    @ Barrett reduction optimized for 2^255-19
    @ ~150-200 cycles on Cortex-M0
...
**3. Multiply by 121666** (`cortex_m0_mpy121666.s`)
```assembly
@ Input:  r0 = output pointer
@         r1 = input pointer
@ Output: [r0] = [r1] * 121666 mod (2^255-19)
...
fe25519_mpyWith121666_asm:
    ...
    @ Optimized multiply by constant
    @ 121666 = 0x1DB42 = (1<<17) + (1<<16) + (1<<13) + ...
    @ Use shift-and-add instead of full multiply
    @ ~80-100 cycles on Cortex-M0
...
// Packed 256-bit representation
typedef struct {
    uint32_t limbs[8];  // 8 × 32-bit limbs
} fe25519_m0;

// Field multiplication using assembly
void fe25519_mul_m0(fe25519_m0 *result,
                    const fe25519_m0 *a,
                    const fe25519_m0 *b) {
    uint32_t tmp[16];
    multiply256x256_asm(tmp, a->limbs, b->limbs);
    fe25519_reduceTo256Bits_asm(result->limbs, tmp);
}

// On-the-fly reduction for addition
void fe25519_add_m0(fe25519_m0 *out,
                    const fe25519_m0 *a,
                    const fe25519_m0 *b) {
    uint64_t accu = 0;

    // Add MSW first, reduce immediately
    accu = (uint64_t)a->limbs[7] + b->limbs[7];
    out->limbs[7] = ((uint32_t)accu) & 0x7FFFFFFF;

    // Propagate with 19x multiplier
    accu = ((uint32_t)(accu >> 31)) * 19;
...
// Montgomery ladder (optimized)
int xy_x25519_m0(uint8_t shared_secret[32],
    ...
    for (int pos = 254; pos >= 0; pos--) {
        ...
        // Ladder step using assembly-optimized ops
        ...
        fe25519_mpyWith121666_asm(z2.limbs, e.limbs);
        fe25519_add_m0(&z2, &z2, &bb);
    ...
    // Final inversion and multiplication
    fe25519_invert_m0(&z2, &z2);
    fe25519_mul_m0(&x2, &x2, &z2);
    fe25519_pack_m0(shared_secret, &x2);
...
**Makefile**:
```makefile
# Conditional compilation for M0
ifeq ($(TARGET_CPU),cortex-m0)
    SRCS += xy_25519_m0.c fe25519_m0.c
    SRCS_ASM += asm/cortex_m0_mpy121666.s \
                asm/cortex_m0_reduce.s \
                asm/mul256x256.s \
                asm/square256.s
...
**Compile flags**:
```makefile
CFLAGS_M0 = -mcpu=cortex-m0 -mthumb \
...
|-----------|---------|-------------|----------|
| **256×256 multiply** | ~2000 cycles | ~400 cycles | **5x** |
| **Modular reduction** | ~800 cycles | ~180 cycles | **4.4x** |
| **Field multiplication** | ~2800 cycles | ~580 cycles | **4.8x** |
| **Full X25519** | ~720k cycles (15ms) | ~180k cycles (3.7ms) | **4x** |

**Total Improvement**: 64KB firmware update time reduces from ~50s to **~40s** (20% faster overall).

#### Implementation Priority

| Component | Effort | Impact | Priority |
|-----------|--------|--------|----------|
| **Assembly multiply** | High | Very High | ⭐⭐⭐ Critical |
| **Assembly reduction** | High | High | ⭐⭐⭐ Critical |
| **Assembly mpyWith121666** | Medium | Medium | ⭐⭐ Important |
| **C field arithmetic** | Low | Medium | ⭐⭐ Important |
| **Ladder optimization** | Low | Low | ⭐ Optional |

#### Testing Strategy

1. **Correctness**: Use RFC 7748 test vectors
2. **Constant-time**: Valgrind/ctgrind verification
3. **Performance**: Compare against reference implementation
4. **Stack**: Ensure <300 bytes usage

#### When to Use xy_25519_m0

✅ **Use specialized M0 version when**:
- Target is Cortex-M0/M0+ exclusively
- X25519 is performance bottleneck
- Can afford ~2 KB extra code for assembly
- Need <5ms key exchange time

❌ **Stick with generic version when**:
- Multi-platform support required (M0/M3/M4/PC)
- Code size is critical constraint
- Performance is acceptable
- Prefer portability over speed
...
- **curve25519-cortexm0**: Haase & Schwabe's optimized Cortex-M0 implementation
  - Paper: "NaCl on 8-Bit AVR Microcontrollers" (AFRICACRYPT 2013)
  - Code: High-performance Curve25519 for ARM Cortex-M0 (2015)
  - Location: `components/crypto/curve25519-cortexm0-20150813/`
  - Key optimizations: 8×32-bit packed representation, assembly multiply/reduce, on-the-fly reduction
  - Performance: ~720k cycles for X25519 @ Cortex-M0 (vs ~2.8M cycles generic)

---

**Status**: Design Complete - Ready for Implementation
**Target**: Cortex-M0+ MCU Bootloader
**Crypto Suite**: X25519 + Ed25519 + ChaCha20-Poly1305 + BLAKE2s + CSPRNG
**Total Footprint**: 20.5 KB Flash + 1.4 KB RAM (with RNG)
**With xy_25519_m0**: 22.5 KB Flash (Assembly optimizations included)
