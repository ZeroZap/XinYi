...
static uint64_t ascon_load64_le(const uint8_t data[8])
{
...
static uint8_t ascon_get_rate_byte(const uint64_t S[5], size_t index)
{
    return (uint8_t)(S[index / 8U] >> ((index % 8U) * 8U));
}
                         ...
                         uint8_t tag[XY_ASCON_128_TAG_SIZE])
{
    ...
    while (i < plaintext_len) {
        if (i + ASCON_RATE <= plaintext_len) {
            ...
            for (j = 0; j < ASCON_RATE; ++j) {
                ciphertext[i + j] = buffer[j] ^ plaintext[i + j];
            }
            S[0] = ascon_load64_le(ciphertext + i);
            ascon_p(S, ASCON_ROUNDS_A);
            i += ASCON_RATE;
        } else {
            size_t j;
            ...
            S[0] ^= (uint64_t)0x80U << (remaining * 8U);
            break;
        }
    }
                                  ...
                                  uint8_t *ciphertext)
{
    ...
    for (i = 0U; i < plaintext_len; ++i) {
        ...
        ctx->data_pos++;
        if (ctx->data_pos == ASCON_RATE_128A) {
            ascon_p(ctx->S, ASCON_ROUNDS_A);
            ctx->data_pos = 0U;
        }
    }
