...
| `crypto_lwc` | Focused test-only LWC contracts; includes Photon one-shot/incremental equivalence across split updates, empty payloads with NULL input/output, contiguous-output enforcement, auth-failure and missing-tag plaintext wipe, final context scrubbing, state-order guards, rate-boundary roundtrips, tampered tags, pointer/length guards, canaries, and cumulative-length overflow rejection before state/output mutation; TinyJambu one-shot 128/192/256 and incremental paths accept empty payloads with NULL input/output, while incremental processing also supports empty AD, zero-length NULL updates, split updates matching one-shot, contiguous decrypt output, auth-failure plaintext wipe, final context scrubbing, and cumulative-length overflow rejection before state/output mutation; Ascon-128 and Ascon-80pq incremental support split updates matching one-shot, contiguous decrypt output, auth-failure plaintext wipe, and final context scrubbing; all Ascon incremental updates reject cumulative-length overflow before mutating output/state; Ascon-128a incremental also rejects repeated AD submission and wipes auth-failed/missing-tag plaintext; no authoritative KAT/provenance/security claim |
...
The active Photon-Beetle source now passes a dedicated warning-as-error compile gate after removing eight excess S-box initializers and an unused padding helper; `crypto_lwc` still guards its existing focused-test-only roundtrip/tag/hash contracts. The root runtime deliberately excludes Ascon/TinyJAMBU/Photon-Beetle. The focused module-source `crypto_cipher_hmac` CTest guards RFC 8439 Poly1305/AEAD vectors, `crypto_blake2` guards focused BLAKE2s vector/guard contracts, `crypto_root_target_smoke` guards the compact reviewed-candidate/compatibility path, and `crypto_review_manifest` keeps source ownership in sync. These remain host contract evidence only and must not be promoted to security/provenance approval, authoritative LWC KAT conformance, constant-time evidence, hardware validation, or compliance claims.
