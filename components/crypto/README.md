...
| `crypto_lwc` | Focused test-only LWC contracts; includes Photon one-shot/incremental equivalence across split updates, contiguous-output enforcement, auth-failure plaintext wipe, final context scrubbing, state-order guards, rate-boundary roundtrips, tampered tags, pointer/length guards, and canaries |
