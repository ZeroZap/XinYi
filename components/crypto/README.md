...
| `crypto_review_manifest` | Policy smoke for security/provenance review status, source ownership map links, mapped root aggregate sources, root `src/*.c` collection shape, and currently byte-identical CRC/Base64/Hex root/module duplicate pairs; not cryptographic validation |
...
1. **Security/provenance review**: initial review policy now lives in `docs/design/xinyi-crypto-security-provenance-review-plan-2026-08-12.md`, with record template `docs/validation/xinyi-crypto-security-provenance-review-record-template-2026-08-12.md`; `crypto_review_manifest` guards `components/crypto/crypto_review_manifest.json` so all algorithms stay review-pending unless a real review record is linked. It also records root aggregate BLAKE2/ECDSA copies as `root-source-unreviewed` so mapped-but-unreviewed code is not mistaken for reviewed algorithm coverage, checks that `components/crypto/CMakeLists.txt` still uses the mapped `src/*.c` aggregate-source collection shape, and keeps currently byte-identical CRC/Base64/Hex/Random/CSPRNG/MD5/HMAC/AES/BLAKE2 root/module duplicate source pairs from silently diverging before an explicit ownership slice. The root smoke exercises ECDSA only as the documented format-only guard contract; root ECDSA no longer pulls the duplicate SHA-256 implementation into `xy_tiny_crypto`, and it is still not production signature validation.
