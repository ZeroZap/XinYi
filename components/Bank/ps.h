...
/** power supply charge type which algorithm is the charger using. */
typedef enum {
    ps_chrg_type_unkown = 0,
    ps_chrg_type_none,
    ps_chrg_type_trickle,    /* slow speed */
    ps_chrg_type_fast,       /* fast speed */
    ps_chrg_type_std,        /* normal speed */
    ps_chrg_type_adaptive,   /* dynamically adjusted speed*/
    ps_chrg_type_custom,     /* use CHARGE_CONTROL_* props */
    ps_chrg_type_longlife    /* slow speed, longer life */
        ps_chrg_type_bypass, /* bypassing the charger */
} ps_chrg_type_t;
...
extern void
power_supply_put_battery_info(struct power_supply *psy,
                              struct power_supply_battery_info *info);
extern int
power_supply_ocv2cap_simple(struct power_supply_battery_ocv_table *table,
...
extern bool
power_supply_battery_bti_in_range(struct power_supply_battery_info *info,
                                  int resistance);
...
{
    return ((info->resist_table != NULL) && info->resist_table_size > 0);
}
...
#else
static inline int
power_supply_set_property(struct power_supply *psy,
                          enum power_supply_property psp,
                          const union power_supply_propval *val)
{
    return 0;
...
#ifdef CONFIG_SYSFS
ssize_t power_supply_charge_behaviour_show(
    struct device *dev, unsigned int available_behaviours,
    enum power_supply_charge_behaviour behaviour, char *buf);

int power_supply_charge_behaviour_parse(unsigned int available_behaviours,
                                        const char *buf);
#else
static inline ssize_t power_supply_charge_behaviour_show(
    struct device *dev, unsigned int available_behaviours,
    enum power_supply_charge_behaviour behaviour, char *buf)
{
    return -EOPNOTSUPP;
...
static inline int
power_supply_charge_behaviour_parse(unsigned int available_behaviours,
                                    const char *buf)
{
    return -EOPNOTSUPP;
}
