 ...
 * @brief UART 读取回调
 */
static int32_t xy_mux_uart_read_cb(uint8_t channel, void *data, size_t len)
{
    if (!data || len == 0) {
 ...
 * @brief UART 写入回调
 */
static int32_t xy_mux_uart_write_cb(uint8_t channel, const void *data, size_t len)
{
    if (!data || len == 0) {
        return XY_MUX_ERROR_INVALID_PARAM;
    }
...
static const xy_mux_ops_t g_uart_ops = {
    ...
    .read = xy_mux_uart_read_cb,
    .write = xy_mux_uart_write_cb,
