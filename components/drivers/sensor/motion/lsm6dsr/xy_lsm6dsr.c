...
static int transport_ready(const xy_lsm6dsr_t *dev)
{
    return dev != NULL && dev->i2c_dev.base.initialized && dev->i2c_dev.i2c_handle != NULL;
}

static int ready(const xy_lsm6dsr_t *dev)
{
    return transport_ready(dev) && dev->initialized;
}

static xy_error_t read_reg(xy_lsm6dsr_t *dev, uint8_t reg, uint8_t *data, size_t length)
{
...
static xy_error_t write_reg(xy_lsm6dsr_t *dev, uint8_t reg, uint8_t value)
{
    if (!transport_ready(dev)) {
        return XY_DEVICE_INVALID_PARAM;
    }
...
xy_error_t xy_lsm6dsr_deinit(xy_lsm6dsr_t *dev)
{
    xy_error_t result;
    ...
    result = write_reg(dev, XY_LSM6DSR_REG_CTRL1_XL, 0U);
    if (result == XY_DEVICE_OK) {
        result = write_reg(dev, XY_LSM6DSR_REG_CTRL2_G, 0U);
    }
    if (result != XY_DEVICE_OK) {
        return result;
    }
...
xy_error_t xy_lsm6dsr_read(xy_lsm6dsr_t *dev, xy_lsm6dsr_sample_t *sample)
{
    uint8_t accel[6];
    uint8_t gyro[6];
    xy_lsm6dsr_sample_t next;
    xy_error_t result;
    ...
    result = read_reg(dev, XY_LSM6DSR_REG_OUTX_L_XL, accel, sizeof(accel));
    if (result != XY_DEVICE_OK) {
        return result;
    }
    result = read_reg(dev, XY_LSM6DSR_REG_OUTX_L_G, gyro, sizeof(gyro));
    if (result != XY_DEVICE_OK) {
        return result;
    }
