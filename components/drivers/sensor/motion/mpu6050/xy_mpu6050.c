...
int xy_mpu6050_init(xy_mpu6050_t *dev, void *i2c_handle)
{
    return xy_mpu6050_init_addr(dev, i2c_handle, MPU6050_ADDR_AD0_LOW);
}
...
int xy_mpu6050_deinit(xy_mpu6050_t *dev)
{
    ...
    /* 进入睡眠模式 */
    int ret = xy_mpu6050_write_reg(dev, MPU6050_REG_PWR_MGMT_1, 0x40);
    if (ret != XY_DEVICE_OK) {
        return ret;
    }
...
int xy_mpu6050_set_accel_range(xy_mpu6050_t *dev, xy_mpu6050_accel_range_t range)
{
    ...
    int ret = xy_mpu6050_write_reg(dev, MPU6050_REG_ACCEL_CONFIG, range << 3);
    if (ret == XY_DEVICE_OK) {
...
int xy_mpu6050_set_gyro_range(xy_mpu6050_t *dev, xy_mpu6050_gyro_range_t range)
{
    ...
    int ret = xy_mpu6050_write_reg(dev, MPU6050_REG_GYRO_CONFIG, range << 3);
    if (ret == XY_DEVICE_OK) {
