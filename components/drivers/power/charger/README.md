...
- 当前证据仅为 `Host-guarded` 的 fake-I2C transaction/status contract 与 PC build；
  ...
  measurements, rejects reserved setpoint encodings instead of publishing out-of-contract values, status
  ...
  and successful teardown clears the handle/callback context. The
