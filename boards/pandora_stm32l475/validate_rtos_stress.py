...
W25Q128_FOTA_STORAGE = "PANDORA_W25Q128_FOTA_DOWNLOAD_CRC_OK"
W25Q128_FOTA_FULL_IMAGE = "PANDORA_W25Q128_FOTA_FULL_IMAGE_OK"
FOTA_BOOTABLE_CANDIDATE = "PANDORA_FOTA_BOOTABLE_CANDIDATE_VALIDATED"
W25Q128_MCU_RESET = (
...
def analyze_capture(
    ...
    if lines.count(SPI_DMA_ABORT_RECOVERY) != 1:
        failures.append(
    ...
    if lines.count(W25Q128_FOTA_STORAGE) not in (1, 2):
        failures.append(
            f"{W25Q128_FOTA_STORAGE} count {lines.count(W25Q128_FOTA_STORAGE)} not in 1..2"
        )
    if lines.count(W25Q128_FOTA_FULL_IMAGE) not in (1, 2):
        failures.append(
            f"{W25Q128_FOTA_FULL_IMAGE} count "
            f"{lines.count(W25Q128_FOTA_FULL_IMAGE)} not in 1..2"
        )
    if lines.count(FOTA_BOOTABLE_CANDIDATE) not in (1, 2):
        failures.append(
            f"{FOTA_BOOTABLE_CANDIDATE} count "
            f"{lines.count(FOTA_BOOTABLE_CANDIDATE)} not in 1..2"
        )
    reset_positions = []
