...
static xy_error_t sc7a22h_sample(xy_sc7a22h_data_t *d, uint8_t *status)
{
    ...
    return xy_sc7a22h_read(&sc7a22h, d);
}
void _init(void) {}
void _fini(void) {}
...
int main(void){
 ...
 if(xy_sc7a22h_init(&sc7a22h,&i2c2)!=XY_DEVICE_OK)fail("SC7A22H_INIT_ERROR");
 text("SC7A22H_ADDR=0x18 WHO_AM_I=0x18\r\n");
 text("SC7A22H_CONFIG com=0x");hex(sc7a22h.com_cfg);text(" acc=0x");hex(sc7a22h.acc_conf);text(" range=0x");hex(sc7a22h.acc_range);text("\r\n");
 ...
 int32_t mag_max[3]={(-2147483647-1),(-2147483647-1),(-2147483647-1)};
 ...
 int32_t l3g_min[3]={2147483647,2147483647,2147483647};
 int32_t l3g_max[3]={(-2147483647-1),(-2147483647-1),(-2147483647-1)};
 text("I2C2_SENSOR_IDENTITIES_OK\r\n");
 for(uint32_t n=0;n<20U;n++){xy_aht30_data_t a;xy_l3g4200d_data_t g;xy_bme680_data_t b;xy_mpu6050_raw_data_t m;xy_hmc5883l_data_t h={0};xy_hmc5883l_field_t f={0};xy_sc7a22h_data_t sc={0};uint8_t sc_status=0U;xy_sc7a22h_accel_t sc_acc={0};float lux;uint8_t hready=0U;xy_error_t hr=XY_DEVICE_OK;xy_error_t fr=XY_DEVICE_OK;xy_error_t br=XY_DEVICE_BUSY;if(xy_aht30_read(&aht30,&a)!=XY_DEVICE_OK)fail("AHT30_READ_ERROR");if(xy_l3g4200d_read(&l3g,&g)!=XY_DEVICE_OK)fail("L3G4200D_READ_ERROR");if(g.x_mdps<l3g_min[0])l3g_min[0]=g.x_mdps;if(g.y_mdps<l3g_min[1])l3g_min[1]=g.y_mdps;if(g.z_mdps<l3g_min[2])l3g_min[2]=g.z_mdps;if(g.x_mdps>l3g_max[0])l3g_max[0]=g.x_mdps;if(g.y_mdps>l3g_max[1])l3g_max[1]=g.y_mdps;if(g.z_mdps>l3g_max[2])l3g_max[2]=g.z_mdps;for(uint32_t attempt=0U;attempt<5U&&br!=XY_DEVICE_OK;++attempt){br=xy_bme680_read(&bme680,&b);if(br!=XY_DEVICE_OK)xy_hal_delay_ms(20U);}if(br!=XY_DEVICE_OK){text("BME680_READ_ERROR result=");num(br);fail("");}if(xy_bh1750_read(&bh1750)!=XY_BH1750_OK||xy_bh1750_get_illuminance(&bh1750,&lux)!=XY_BH1750_OK)fail("BH1750_READ_ERROR");if(xy_mpu6050_read_raw(&mpu6050)!=XY_MPU6050_OK)fail("MPU6050_READ_ERROR");m=mpu6050.raw;if(sc7a22h_sample(&sc,&sc_status)!=XY_DEVICE_OK)fail("SC7A22H_SAMPLE_ERROR");if(xy_sc7a22h_read_accel(&sc7a22h,&sc_acc)!=XY_DEVICE_OK)fail("SC7A22H_ACCEL_ERROR");if(xy_hmc5883l_data_ready(&hmc5883l,&hready)!=XY_DEVICE_OK)fail("HMC5883L_STATUS_ERROR");if(hready){hr=xy_hmc5883l_read(&hmc5883l,&h);if(hr==XY_DEVICE_OK)fr=xy_hmc5883l_read_field(&hmc5883l,&f);if(fr==XY_DEVICE_OK){if(f.x_mgauss<mag_min[0])mag_min[0]=f.x_mgauss;if(f.y_mgauss<mag_min[1])mag_min[1]=f.y_mgauss;if(f.z_mgauss<mag_min[2])mag_min[2]=f.z_mgauss;if(f.x_mgauss>mag_max[0])mag_max[0]=f.x_mgauss;if(f.y_mgauss>mag_max[1])mag_max[1]=f.y_mgauss;if(f.z_mgauss>mag_max[2])mag_max[2]=f.z_mgauss;}}text("SAMPLE aht_t_centi=");num(a.temperature_centi_c);text(" aht_rh_centi=");num((int32_t)a.humidity_centi_pct);text(" lux_x10=");num((int32_t)(lux*10.0F));text(" mpu_ax=");num(m.accel_x);text(" mpu_ay=");num(m.accel_y);text(" mpu_az=");num(m.accel_z);text(" mag_ready=");num(hready);text(" mag_result=");num(hr);text(" mag_field_result=");num(fr);text(" mag_x_mgauss=");num(f.x_mgauss);text(" mag_y_mgauss=");num(f.y_mgauss);text(" mag_z_mgauss=");num(f.z_mgauss);text(" gx_mdps=");num(g.x_mdps);text(" gy_mdps=");num(g.y_mdps);text(" gz_mdps=");num(g.z_mdps);text(" sc_status=0x");hex(sc_status);text(" sc_x=");num(sc.x);text(" sc_y=");num(sc.y);text(" sc_z=");num(sc.z);text(" sc_mg_x=");num(sc_acc.x_mg);text(" sc_mg_y=");num(sc_acc.y_mg);text(" sc_mg_z=");num(sc_acc.z_mg);text(" bme_pa=");num((int32_t)b.pressure_pa);text(" bme_t_centi=");num(b.temperature_centi_c);text(" bme_rh_milli=");num((int32_t)b.humidity_milli_pct);text(" bme_gas_ohm=");num((int32_t)b.gas_ohms);text(" bme_status=0x");hex(b.status);text(" irq_pa4=");num(xy_hal_gpio_read(GPIOA,4U));text(" irq_pb8=");num(xy_hal_gpio_read(GPIOB,8U));text(" sc_drdy_edges=");num((int32_t)sc7a22h_drdy_edges);text(" irq_pb9=");num(xy_hal_gpio_read(GPIOB,9U));text(" irq_pc4=");num(xy_hal_gpio_read(GPIOC,4U));text("\r\n");xy_hal_delay_ms(100U);}text("L3G_RANGE_X=");num(l3g_min[0]);text("..");num(l3g_max[0]);text(" Y=");num(l3g_min[1]);text("..");num(l3g_max[1]);text(" Z=");num(l3g_min[2]);text("..");num(l3g_max[2]);text("\r\n");text("MAG_RANGE_X=");num(mag_min[0]);text("..");num(mag_max[0]);text(" Y=");num(mag_min[1]);text("..");num(mag_max[1]);text(" Z=");num(mag_min[2]);text("..");num(mag_max[2]);text("\r\n");text("PANDORA_I2C2_SENSOR_PROBE_DONE\r\n");
}
