...
- Root `BUILD_TESTING=ON` builds `tests/test_at_client` and `tests/test_at_server` via `tests/CMakeLists.txt`; the independent `tests/unit` suite is what `make test-unit` runs.
