...
| S2-204 | P0 | BMI270 gyro-range validation | DONE | Zero | S2-203（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `xy_bmi270_set_range()` 只校验加速度量程，非法 `gyr_range` 仍进入寄存器写流程；现同时限制 `gyr_range <= BMI270_GYR_RANGE_125`，非法输入返回 `XY_DEVICE_EINVAL`，保持 range/scale cache 且零总线 I/O。focused `sensor_bmi270`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BMI270 实板、姿态/陀螺精度或耐久证据 | 本记录提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
