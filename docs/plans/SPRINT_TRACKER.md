...
| S2-209 | P0 | BMI270 ODR field validation | DONE | Zero | S2-208（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `xy_bmi270_set_range()` 未校验 `acc_odr/gyr_odr` 是否落在 4-bit 寄存器字段，非法值会被左移/截断并进入总线；现拒绝任一 ODR > `0x0F`，返回 `XY_DEVICE_EINVAL`，保持 range/scale cache 且零总线 I/O。focused `sensor_bmi270`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BMI270 实板、ODR 精度或耐久证据 | 本记录提交 | 2026-09-15 |
