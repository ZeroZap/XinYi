...
| S2-120 | P0 | AK09918 data register window 修正 | DONE | Zero | S2-119（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 使用独立 literal `0x11+i` 期望 HXL..HZH，证明驱动实际请求 `0x31`（CNTL2）：`Expected 17 Was 49`。现 `AK09918_REG_DATA` 修正为 HXL `0x11`，连续读取 `0x11..0x16` 后按 little-endian 发布三轴样本；既有中段失败保持 caller output 契约继续通过。focused `sensor_magnetic_i2c_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 AK09918 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
