...
| S2-195 | P0 | BNO055 bus/enum public boundaries | DONE | Zero | S2-194（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 覆盖缺失 bus 进入 reset 及非法 mode/power enum；现内部 bus helper 与 public setters 在 I²C 前 fail-closed，保持零总线副作用。focused `sensor_bno055`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BNO055 实板、动态姿态、校准或耐久证据 | 本记录提交 | 2026-09-14 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
