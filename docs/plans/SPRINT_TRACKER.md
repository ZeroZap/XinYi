...
| S2-02 | P0 | Pandora GPIO/UART/I2C/SPI/IRQ/DMA 实板基础外设 | IN_PROGRESS | Zero | Pandora STM32L475VE；自动化链路 | Pandora 已有 board-local GPIO/UART/software-I2C/KEY0 B1、FreeRTOS SysTick/TIM6 ISR→task、framework DMA mem2mem/IRQ/recovery，以及 SPI1 TX request→DMA1 Channel3→framework callback→OSAL task B1。SPI 未接外设/引脚，仍缺电气/外设响应、RX/full-duplex DMA 与对应 recovery B2。STM32U5 仅保留 compile compatibility | `683ab5ea`～`d904cbf2` + 本记录提交 | 2026-09-05 |
| S2-03 | P0 | Pandora I2C→Device helper→现有非 SSD1306 设备纵切 | IN_PROGRESS | Zero | S2-02；Host 前置已完成 | Pandora board-owned software-I2C 已置于 `xy_hal_i2c_master_*` 后并通过 canonical `xy_i2c_device_*` 驱动板载 AHT10；8172-byte image write/verify/read-back，8 秒 UART 13 个完整 ACK/测量周期，取得该纵切 B1。24xx Host timeout/NACK/re-init 前置已覆盖；仍缺真实 NACK→recovery B2 与 hardware-I2C peripheral 纵切，SSD1306 排除。U5 compile 仅补充 | `72391b51`、`6ec6081c`、`b94fc3c2` + 本记录提交 | 2026-09-04 |
...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora board runtime 已用 internal-Flash journal 完成 stage handoff→boot attempt→confirm、anti-rollback floor 与 bounded automatic rollback。clean `d37ecf1c` image 11992 bytes write/verify/read-back byte-identical；5 次匹配固件身份的 boot chain 有序完成 v2 confirm、拒绝 floor 以下 v1、stage v3、一次 pending attempt，并在下一 boot 自动清除 pending、保留 active/min version=2。4096-byte metadata read-back SHA-256=`08dd61a6...`，latest generation=7、pending clear、CRC valid。该结果只证明 board runtime metadata callback/reset recovery，不是独立 bootloader、候选 image 执行或真实掉电。STM32U5 仅保留增强兼容。仍缺候选 image 双槽布局、真实 bootloader vector handoff 与掉电 B2 | `54d8b735`～`d37ecf1c` + 本记录提交 | 2026-09-04 |
...
| S5-99 | P0 | Pandora FOTA core download→W25Q128→全镜像 CRC | VERIFYING | Zero | S5-98（DONE）；canonical FOTA core | FOTA core 新增显式 expected CRC setter；`finish_download()` 对完整 candidate 执行跨 chunk 连续 CRC32，失败进入 ERROR。Pandora runtime 已改为通过 core 分两块下载 300-byte candidate 到 canonical W25Q128 adapter 并执行 read-back/CRC；Host focused 与 PC/L4/U5 compile 已通过，待 clean committed image 实板烧录、回读与 UART marker 闭环 | 本记录提交 | 2026-09-05 |
