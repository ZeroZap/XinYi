...
| S2-118 | P0 | IST8310 public boundary fail-closed | DONE | Zero | S2-117（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 对 `ops->init(NULL)` 得到 exit 139 / Segmentation fault。现 factory 在分配前拒绝 NULL name/bus，init/deinit/read 在解引用前验证 sensor/private/bus/output；非法输入返回 `SENSOR_EINVAL`、零 I2C/零 tick，caller output 保持。focused `sensor_magnetic_i2c_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 IST8310 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
