...
| S2-39 | P0 | HDC1080 配置写失败原子清理 | DONE | Zero | S2-38（DONE）；硬件测试按用户要求暂停 | TDD RED 将 custom-address 配置失败后保留 addr/helper 的旧契约改为 fail-closed，并覆盖 reset/config 两次关键写入失败；现任一写失败均清空完整对象。focused `sensor_hdc1080`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 初始化边界，不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-11 |
