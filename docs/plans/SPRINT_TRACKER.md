...
| S2-40 | P0 | Coulomb 初始电荷复位失败收口 | DONE | Zero | S2-39（DONE）；硬件测试按用户要求暂停 | TDD RED 将 current-register reset 写失败仍返回成功并置 initialized 的旧契约改为 fail-closed；现初始电荷复位失败返回错误并清空完整对象。focused `sensor_coulomb`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile contract，不升级 INA226 实板、累计电量精度、校准或告警证据 | 本记录提交 | 2026-09-11 |
