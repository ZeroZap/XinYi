...
| S2-167 | P0 | ADS1115 post-deinit setter state guards | DONE | Zero | S2-166（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 在 deinit 后调用 PGA/DR setter 得到 `Expected -2 Was 0`，证明未初始化对象仍发布配置变更。现两 setter 对 NULL、未初始化及越界枚举返回 `XY_ADS1115_INVALID_PARAM`，保持原 PGA/DR cache 且无 I²C。focused `sensor_adc_power_monitors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 ADS1115 实板、精度或耐久证据 | 本记录提交 | 2026-09-14 |
