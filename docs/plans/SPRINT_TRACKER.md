...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora 驻留 bootloader 已从 W25Q128 candidate 安装并跳转 `0x08008000` application。双记录 generation/CRC/末写 commit-marker 安装 journal 已实板证明首次安装与软件复位幂等；Host 另覆盖执行槽损坏重装、同 generation 分裂状态及相差 `2^31` 的 generation 歧义 fail-closed；install-once 在 journal 写入前复用执行布局校验，拒绝未对齐布局且不产生 journal/执行槽副作用。仍未取得真实掉电/半写实板注入、确认/回滚与 approved signature provider | `54d8b735`～`4da87513` + 本记录提交 | 2026-09-06 |
