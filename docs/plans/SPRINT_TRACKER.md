...
| S2-03 | P0 | Pandora I2C→Device helper→现有非 SSD1306 设备纵切 | IN_PROGRESS | Zero | S2-02；Host 前置已完成 | Pandora board-owned software-I2C 已置于 `xy_hal_i2c_master_*` 后并通过 canonical `xy_i2c_device_*` 驱动板载 AHT10 与 AP3216C。人工 capture 的 207 组 `03001D000500` 全同，确认旧 `0x07` 被误命名/误用：AP3216C 定义中它是一次性 ALS+PS，不是连续 ALS+PS+IR。修为连续模式 `0x03` 后，clean `f320087c` 10512-byte image write/verify/read-back byte-identical；12 秒 97 组独立寄存器样本有 60 组 raw、47 组 decoded，PS 0–14、IR 0–514，证明 freshness/config/register-map 闭环。Pandora V2.4 原理图明确 U8=AP3216C，地址/模式/寄存器与 RT-Thread package 一致。旧 capture 仅诊断，不升级 dynamic B1；仍需修复后受控 uncovered/far→covered/near delta、真实 NACK→recovery B2 与 hardware-I2C peripheral。SSD1306 排除，U5 仅 compile 补充 | `72391b51`～`f320087c` + 本记录提交 | 2026-09-06 |
