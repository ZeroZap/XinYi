...
| S5-36 | P0 | Release workflow runner 与 canonical CI 对齐 | DONE | Zero | S5-35（DONE） | RED：release readiness probe 发现 tag workflow 仍使用漂移的 `ubuntu-latest`；现固定 `ubuntu-24.04` 并机器守护 runner 与 authorization wiring；focused、Host、PC root、workflow YAML 与 `git diff --check` 通过。不构成 immutable environment、发布授权或 R1 | 本轮提交 | 2026-09-01 |
