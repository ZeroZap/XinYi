...
| S2-192 | P0 | SHT40 post-deinit precision setter guard | DONE | Zero | S2-191（DONE）；硬件测试按用户要求暂停 | TDD RED 覆盖未初始化/已 deinit 的 `set_precision()`：旧实现可修改 cache；现要求 initialized，失败前返回 `XY_SHT40_INVALID_PARAM` 并保持 precision、零总线副作用。focused `sensor_sht40`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 SHT40 实板、精度、CRC 或耐久证据 | 本记录提交 | 2026-09-14 |
