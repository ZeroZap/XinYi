...
| S1-05 | P1 | 恢复 SDL2 backend source 与 Host contract | DONE | Zero | S1-02（DONE） | 新增 explicit-context SDL2 backend；`gui_sdl_backend` 以 fake SDL seam 覆盖 configure/init、RGB565 framebuffer、flush 错误、cleanup 与 quit event；修复真实 SDL2 include contract，并在 canonical CI 增加 `libsdl2-dev` source/link gate；Host 180/180、默认/SDL PC root build、`git diff --check` 通过。仍未宣称真实 SDL window/runtime | `2fd4e668`、本轮提交 | 2026-08-25 |
...
- 下一步：S1-03 已将 OFL-1.1 Noto Sans CJK SC 的 15 个必需 UI glyph 接入 active 16x16 table，并保留其 Host-only、未视觉/实板批准边界；其余 legacy 字体与 provenance 仍 pending。S1-04 继续等待实板；S1-05 的真实 SDL2 source/link 已纳入 canonical CI，下一 slice 是补 headless window/event-loop runtime，不能以 compile/link 或 fake seam 冒充真实 SDL runtime。
