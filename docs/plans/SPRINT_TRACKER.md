...
| S2-74 | P0 | BNO055 deinit 睡眠错误传播与 ready 保留 | DONE | Zero | S2-73（DONE）；硬件测试按用户要求暂停 | 确认 focused target 链接 `components/sensor/src/xy_bno055.c` 后做 TDD RED：进入 suspend 的 power-mode 写失败时 deinit 仍返回成功并清除 initialized；现传播原始 transport 错误并保留 ready，仅 sleep 成功后清状态。focused `sensor_bno055`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 生命周期边界，不升级 BNO055 实板、融合精度、校准或功耗证据 | 本记录提交 | 2026-09-12 |
