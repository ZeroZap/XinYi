...
| S2-52 | P0 | legacy BMP280 deinit 写失败传播 | DONE | Zero | S2-51（DONE）；硬件测试按用户要求暂停 | TDD RED 注入 CTRL_MEAS sleep 写失败，证明 deinit 丢弃 transport 错误并返回成功；现写失败返回 `SENSOR_EIO`，仅成功时返回 `SENSOR_EOK`。focused `sensor_bmp280`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 反初始化边界，不升级 BMP280 实板或功耗证据 | 本记录提交 | 2026-09-11 |
