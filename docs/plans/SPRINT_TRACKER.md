...
| S2-127 | P0 | KX023 deinit mode cache 原子提交 | DONE | Zero | S2-126（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 standby write 成功后 `mode` 仍为 LOW_POWER：`Expected 0 Was 1`。现 deinit 仅在 transport 成功后提交 STANDBY cache；write 失败传播 `SENSOR_EIO` 并保留 LOW_POWER。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 KX023 实板、功耗或耐久证据 | 本记录提交 | 2026-09-13 |
