...
| S2-140 | P0 | legacy BMP280 public boundary fail-closed | DONE | Zero | S2-139（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 对 `ops->init(NULL)` 稳定复现 CTest SIGSEGV。现 pressure/temperature factories 在分配前拒绝空 name/bus，init/deinit/read 在解引用前校验 sensor/private/bus/output；非法输入返回 `SENSOR_EINVAL`、零 I2C/零 delay，caller output 保持。focused `sensor_bmp280`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BMP280 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
