...
| S2-99 | P0 | LSM6DSO accel range cache/scale domain 对齐 | DONE | Zero | S2-98（DONE）；硬件测试按用户要求暂停 | TDD RED 将初始化 cache 断言切换为公开 range enum 后稳定复现 2G 初始化仍缓存物理值 `2`；该值与 4G register enum 冲突，导致 4G read 错用 2G scale。现 cache 始终保存公开 register enum，转换按 2G/4G/8G/16G enum 选择 61/122/244/488 ug/LSB，并锁定 4G raw 转换。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板、量程精度或动态响应证据 | 本记录提交 | 2026-09-13 |
