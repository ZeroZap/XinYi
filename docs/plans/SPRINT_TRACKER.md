...
| S5-111 | P0 | Pandora onboard ST7789 SPI3 bounded visual smoke | VERIFYING | Zero | V2.4 schematic；RT-Thread Pandora BSP；更正后用户视觉复核待办 | 旧 `e125038e` 固件逻辑发送 red→green→blue，但 Eugene 更正实际观察为 blue→green→red→white→black，证明 MADCTL BGR 配置导致红蓝互换；`653f1905` 的 visual B1 声明已撤回。修复后的 clean committed `c2e34a4a` 12,820-byte image 已完成 ST-Link write/verify、byte-identical read-back 与 independent UART exact-identity/9-marker/error-0 machine gate；Host 221/221、PC/L4/U5 compile 通过。仅恢复修复镜像 control-path B1，仍须用户确认 red→green→blue→white→black 与象限颜色后才能重新授予 visual B1 | `c2e34a4a` + 本记录提交 | 2026-09-07 |
