...
| S2-104 | P0 | LSM9DS1 factory 空名称/总线 fail-closed | DONE | Zero | S2-103（DONE）；硬件测试按用户要求暂停 | TDD RED 证明 accel/gyro/mag factory 对 NULL name 会在 `strncpy` 崩溃，且接受 NULL bus 生成不可用对象。现三入口均在分配前拒绝空 name/bus，不产生对象且无 I2C 副作用。focused `sensor_lsm9ds1`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM9DS1 实板证据 | 本记录提交 | 2026-09-13 |
