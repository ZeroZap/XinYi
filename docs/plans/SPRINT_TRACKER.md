...
| S5-122 | P0 | Pandora PM sleep/wakeup 安全纵切 | BLOCKED | Zero | S5-76（DONE）；Pandora FreeRTOS tick IRQ | PM sleep API 不再空返回成功：新增 lifecycle/state guard，经 `xy_hal_sys_enter/exit_pwr_mode` 进入 STM32L4 shallow SLEEP/WFI，并由 FreeRTOS tick IRQ 唤醒；PC/L4/U5 backend 与 Host contract 已接入。2026-09-08 对当前 clean `29f9ae68` 再次重建 38576-byte image，确认 exact ELF identity 与 SVC/PendSV/SysTick/PM symbols，ST-Link write/verify 且同长度 read-back byte-identical（SHA-256 `ad7c5aa1...85d7`）；WCH-Link 在烧录前打开并 flush 后捕获 15 秒仍为 0 bytes。新增 dedicated PM UART validator，强制 exact identity、`SLEEP_ENTER→WAKE_IRQ→SLEEP_WAKE_OK` 顺序及零 error marker；当前实板记录保持 `BLOCKED_UART_NO_DATA`，不升级 B1 | `docs/validation/evidence/pandora-stm32l475/2026-09-08/pm-sleep-wakeup-29f9ae68.json` + 本记录提交 | 2026-09-08 |
