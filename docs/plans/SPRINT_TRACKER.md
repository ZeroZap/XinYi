...
| S2-54 | P0 | LSM9DS1 init reset 错误传播 | DONE | Zero | S2-53（DONE）；硬件测试按用户要求暂停 | TDD RED 注入 CTRL3_C 软复位写失败，证明旧实现仍延时、继续配置、覆盖缓存并返回成功；现 reset 写失败立即返回 `SENSOR_EIO`，不延时、不继续配置且保留调用前缓存。focused `sensor_lsm9ds1`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 初始化边界，不升级 LSM9DS1 实板、姿态/磁场精度或耐久证据 | 本记录提交 | 2026-09-11 |
