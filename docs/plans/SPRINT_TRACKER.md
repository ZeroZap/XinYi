...
| S5-50 | P0 | Pandora AHT10 恢复事件顺序门 | DONE | Zero | S5-49（DONE） | RED：ACK/measurement 后出现 trailing NACK 仍被误标 B2 candidate；现要求 NACK→后续 ACK→后续 plausible measurement 的严格顺序；focused 2/2、Host 203/203、PC root 与 `git diff --check` 通过；PTY 不升级 B2 | 待提交 | 2026-09-02 |
