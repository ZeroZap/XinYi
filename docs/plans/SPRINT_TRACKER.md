...
| S2-142 | P0 | legacy SHT30 missing-bus read fail-closed | DONE | Zero | S2-141（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 将 wrapper bus 清空后调用 read，证明其仍进入 canonical Device fixture 并越过已消费队列：`Expected 3 to be less than 3`。现 read 与 init 一致在 transport 前拒绝 missing bus，返回 `SENSOR_EINVAL`、零新增 I2C 且保持 caller output；factory 空 bus 也受检拒绝。focused `sensor_sht30_legacy`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 SHT30 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
