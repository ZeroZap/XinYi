...
| S5-110 | P0 | Pandora Actuator motor/vibration PA1/PA0 bounded forward pattern | VERIFYING | Zero | V2.4 schematic；用户提供 driver truth table | 原理图网络确认 PA1=`MOTOR_A` 经 R67 到 U12 IA、PA0=`MOTOR_B` 经 R66 到 U12 IB；新增 reusable H-bridge Actuator，L/L fail-safe standby、非 standby 模式切换强制 break-before-make、GPIO failure 回落 standby，单 step 限 5 秒。Dedicated board candidate 仅执行 forward 120 ms / standby 120 ms / forward 120 ms / standby 120 ms / forward 300 ms，最终 L/L；Host/focused/L4 link 已通过，待 clean committed image 烧录/回读/UART 与现场振动/停止确认。原理图标注 `TC214B`，exact chip identity 仍未独立确认 | 本 slice 提交 | 2026-09-06 |
