...
| S2-196 | P0 | APDS9960 factory NULL-bus boundary | DONE | Zero | S2-195（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `create_rgb()` 在有效 name、NULL bus 时仍分配并返回对象；`create_proximity()` 与 `create_gesture()` 继承同一缺陷。现 factory 在分配前拒绝 NULL bus，返回 NULL，无分配或 I²C 副作用。focused `sensor_apds9960_sensor`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 APDS9960 实板、RGB/接近/手势精度或耐久证据 | 本记录提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
