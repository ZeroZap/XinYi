...
| S2-34 | P0 | BMI088 初始化失败 false-ready 清理 | DONE | Zero | S2-33（DONE）；硬件测试按用户要求暂停 | TDD RED 证明 soft reset、chip-ID 或七项默认配置任一步失败后仍残留 SPI handle/config；现所有初始化失败统一清空完整对象，禁止失败后继续公共寄存器访问。focused `sensor_bmi088`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 初始化边界，不升级 BMI088 实板、动态响应、精度或校准证据 | 本记录提交 | 2026-09-10 |
