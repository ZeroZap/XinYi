...
| S2-107 | P0 | BMP390 factory/public ops 参数 fail-closed | DONE | Zero | S2-106（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `bmp390_create(NULL, bus)` 在 `strncpy` 触发 SIGSEGV。现 factory 分配前拒绝空 name/bus；init/read 校验 sensor/private/bus/output，非法边界返回 `SENSOR_EINVAL`，零 I2C 且保持 caller output/timestamp。focused `sensor_env_i2c_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BMP390 实板或压力精度证据 | 本记录提交 | 2026-09-13 |
