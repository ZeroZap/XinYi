...
| S2-124 | P0 | LSM6DSL reinit public ODR cache 重同步 | DONE | Zero | S2-123（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 以 public `odr=208` 进入完整成功 SPI reinit，证明 private range 已回到默认、public ODR 仍为 208。现 init 仅在所有 transport 成功后与 private range cache 一起提交 `odr=104`；最终 write 失败时保留 nondefault private/public cache。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSL 实板或动态姿态证据 | 本记录提交 | 2026-09-13 |
