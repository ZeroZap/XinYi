...
| S2-180 | P0 | LPS22HB invalid ODR enum boundary | DONE | Zero | S2-179（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 传入 `0x80`，非法 ODR 仍进入 `CTRL_REG1` 读取（`Expected 0 Was 16`）。现 set_odr 仅接受公开 `0x00..0x70`、低四位为零的编码；非法值返回 `XY_ERROR`，不读写寄存器且保持 ODR cache。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LPS22HB 实板、精度或耐久证据 | 本记录提交 | 2026-09-14 |
