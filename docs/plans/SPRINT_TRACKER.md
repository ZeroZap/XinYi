...
| S2-183 | P0 | LPS22HB inverted threshold boundary | DONE | Zero | S2-182（DONE）；硬件测试按用户要求暂停 | TDD RED 传入 `low=0x2000`、`high=0x1000`；现配置在任何 I/O 前拒绝反转阈值范围并保持 threshold cache。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LPS22HB 实板、阈值精度或耐久证据 | `18ad075a` | 2026-09-14 |
