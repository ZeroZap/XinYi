...
| S2-133 | P0 | LSM6DSO deinit ODR cache 原子提交 | DONE | Zero | S2-132（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 accel/gyro power-down 两次写成功后 `accel_rate` 仍为 208：`Expected 0 Was 208`。现仅在两次关闭写均成功后一次性清零 private accel/gyro rate 与 public ODR；首次 write 失败传播 `SENSOR_EIO` 并完整保留 cache。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板、功耗或动态姿态证据 | 本记录提交 | 2026-09-13 |
