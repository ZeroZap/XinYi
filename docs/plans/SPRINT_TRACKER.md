...
| S2-95 | P0 | LSM6DSO gyro range 编码与位域修正 | DONE | Zero | S2-94（DONE）；硬件测试按用户要求暂停 | TDD RED 证明设置 500dps 时错误覆盖 CTRL2_G ODR（寄存器 `0x40` 期望 `0x44`，实际 `0x20`）；修正 125/250/500/1000/2000dps 唯一逻辑编码及 `[3:1]` full-scale 编码，锁定 125/2000 写值并补齐 2000dps 70mdps/LSB scale。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板、量程精度或动态响应证据 | 本记录提交 | 2026-09-12 |
