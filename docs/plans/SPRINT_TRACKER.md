...
| S2-18 | P0 | LSM9DS1 初始化缓存原子提交 | DONE | Zero | S2-17（DONE）；硬件测试按用户要求暂停 | TDD RED 在最后磁力计配置写失败时证明 accel/gyro range cache 已提前提交。现仅在 IMU reset、accel、gyro 与 magnetic 配置全部成功后一次性提交 range cache；focused `sensor_lsm9ds1` 与 `git diff --check` 通过。仅为 Host contract，不升级 LSM9DS1 实板、精度或校准证据 | 本记录提交 | 2026-09-12 |
