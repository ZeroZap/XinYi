...
| S2-202 | P0 | BMA400 power-mode NULL-context boundary | DONE | Zero | S2-201（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `bma400_set_power_mode(NULL, ...)` 在解引用 `sensor->priv_data` 前崩溃；现先校验 sensor/private/bus，返回 `SENSOR_EINVAL`，无 I²C 副作用。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BMA400 实板、动态姿态、量程/ODR 精度或耐久证据 | 本记录提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
