...
| S2-115 | P0 | LSM6DSO gyro ODR cache 物理域同步 | DONE | Zero | S2-114（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 gyro 设置 208Hz 后 private cache 为 register encoding `5`、public `odr` 未同步。现 setter 显式映射 encoding 到物理 Hz，write 成功后原子同步 private/public cache；read/write 失败保持旧值，非法 encoding 返回 `SENSOR_EINVAL` 且零 I2C。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板或动态姿态证据 | 本记录提交 | 2026-09-13 |
