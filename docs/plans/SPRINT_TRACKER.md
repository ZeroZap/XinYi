...
| S5-44 | P0 | Pandora UART 掉线 fail-closed 分类 | DONE | Zero | S5-43（DONE） | RED：PTY peer 断开时 `read()` 返回 EOF，helper 曾等待到期并误记 `NO_DATA_TIMEOUT`；现立即记录 `CAPTURE_IO_FAILED`，保留已捕获字节并返回失败。focused 2/2、Host 203/203、PC root 与 `git diff --check` 通过，不升级 runtime/B1/B2 | 本轮提交 | 2026-09-02 |
