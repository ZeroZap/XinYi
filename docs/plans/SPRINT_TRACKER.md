...
| S2-148 | P0 | legacy SHT30 callable deinit lifecycle | DONE | Zero | S2-147（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 wrapper `.deinit = NULL`，调用路径稳定复现 CTest SIGSEGV。现 canonical Device 提供 `xy_sht30_deinit()` 清除完整 handle，legacy wrapper 校验 sensor/private/bus 后委托并映射错误；成功后 initialized=false 且 I2C handle 清空。focused `sensor_sht30_legacy`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 SHT30 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
