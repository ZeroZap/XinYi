...
| S2-53 | P0 | TSL2561 read enable 错误传播 | DONE | Zero | S2-52（DONE）；硬件测试按用户要求暂停 | TDD RED 注入 read 前 enable 写失败，证明旧实现忽略错误并继续延时/读取/更新数据；现立即传播 transport 错误并保持数据不变。focused `sensor_tsl2561`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 读取边界，不升级 TSL2561 实板、照度精度或耐久证据 | 本记录提交 | 2026-09-11 |
