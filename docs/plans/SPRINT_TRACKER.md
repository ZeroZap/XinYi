...
| S2-102 | P0 | LSM6DSL accel range cache/scale domain 对齐 | DONE | Zero | S2-101（DONE）；硬件测试按用户要求暂停 | TDD RED 将 init cache 契约切换为公开 range enum 后复现 2G 初始化仍缓存物理值 `2`（期望 enum `0`），且 4G enum `2` 被 read 误判为 2G scale。现 cache 始终保存公开 enum，读取按 2/4/8/16G enum 映射 61/122/244/488ug/LSB，并锁定 4G raw 转换。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSL 实板或精度证据 | 本记录提交 | 2026-09-13 |
