...
| S5-122 | P0 | Pandora PM sleep/wakeup 安全纵切 | BLOCKED | Zero | S5-76（DONE）；Pandora FreeRTOS tick IRQ | PM sleep API 不再空返回成功：新增 lifecycle/state guard，经 `xy_hal_sys_enter/exit_pwr_mode` 进入 STM32L4 shallow SLEEP/WFI，并由 FreeRTOS tick IRQ 唤醒；PC/L4/U5 backend 与 Host contract 已接入。clean `b260f28c` 38576-byte image 重建、handler/identity 检查、ST-Link write/verify 与同长度 read-back byte-identical（SHA-256 `27cd3ce…5a3`）均通过；但 WCH-Link UART 再次捕获 0 bytes，且回刷同 commit 的普通 Pandora smoke 亦为 0 bytes，确认阻塞在独立 UART 链而非 PM marker。保持不升级 B1；待修复/升级 ST-Link VCP 或接入稳定 USB-UART 后重跑 | `docs/validation/evidence/pandora-stm32l475/2026-09-08/pm-sleep-wakeup-f7c62c2f.json` + 本记录提交 | 2026-09-08 |
