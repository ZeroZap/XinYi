...
| S2-89 | P0 | LSM6DSO gyro ODR 位域修正 | DONE | Zero | S2-88（DONE）；硬件测试按用户要求暂停 | 在同一 active-linked `sensor_lsm6dso.c` 上做 TDD RED：设置 208Hz 时将 ODR 写入 CTRL2_G 低位并破坏 gyro range（寄存器 `0x0A` 期望 `0x5A`，实际 `0x05`）；现保留低四位并将 ODR 写入 `[7:4]`，成功后才提交 cache。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板、ODR 精度或动态响应证据 | 本记录提交 | 2026-09-12 |
