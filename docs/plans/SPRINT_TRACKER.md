...
| S5-131 | P0 | Crypto Photon-Beetle warning-clean gate | DONE | Zero | S5-130（DONE）；Photon provenance/KAT 仍 pending | TDD RED 用 C99 `-Wall -Wextra -Wpedantic -Werror` 编译 active Photon source，稳定复现 8 个 S-box excess initializer 与 unused padding helper。现 S-box initializer 精确为 256 项并删除未调用 helper；新增 `crypto_photon_warning_clean` CTest。既有 `crypto_lwc` roundtrip/tag/hash contract、Crypto 31/31、Host 225/225、PC/L4/U5 Crypto-only build 与 `git diff --check` 通过。仅为编译洁净和既有 Host contract，不升级 authoritative KAT、provenance、安全、constant-time、硬件或合规证据 | 本记录提交 | 2026-09-08 |
