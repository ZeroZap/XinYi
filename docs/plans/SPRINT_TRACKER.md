...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora 驻留 bootloader 已从 W25Q128 candidate 安装并跳转 `0x08008000` application。新增双记录 generation/CRC/末写 commit-marker 安装 journal；clean `27446043` 首次 boot 输出 installed，软件复位后输出 already-installed 且未重复擦写 execution slot，随后再次 vector handoff 并运行 application marker。Host 覆盖首次安装、重启幂等、写失败及损坏记录重装；仍未取得真实掉电/半写实板注入、确认/回滚与 approved signature provider | `54d8b735`～`27446043` + 本记录提交 | 2026-09-06 |
