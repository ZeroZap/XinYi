...
| S2-121 | P0 | CMM905 空总线 public boundary fail-closed | DONE | Zero | S2-120（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 factory 接受 NULL bus 并生成不可用对象。现 factory 在分配前拒绝空 name/bus，init/read 在 transport 前校验 sensor/private/bus/output；非法输入返回 `SENSOR_EINVAL`、零 I2C，caller output 保持。focused `sensor_magnetic_i2c_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 CMM905 实板、精度或耐久证据 | 本记录提交 | 2026-09-13 |
