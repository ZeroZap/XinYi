...
| S2-119 | P0 | BNO055 unit cache 原子提交 | DONE | Zero | S2-118（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 UNIT_SEL write 成功后 `unit_flags` 仍为旧值 `0xA5`（期望 `0x19`）。现仅在 transport 成功后提交新 unit cache；write 失败传播原错误并保持旧 cache。focused `sensor_bno055`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BNO055 实板、校准或动态姿态证据 | 本记录提交 | 2026-09-13 |
