...
| S2-147 | P0 | SGP40 serial output 原子提交 | DONE | Zero | S2-146（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 注入第二个 serial word CRC 错误，证明第一项 caller output 已被提前覆盖：`Expected 2863272209 Was 4660`。现三个 word 全部先在局部 staged buffer 完成 CRC 校验和解析，仅全部成功后一次性发布；transport/CRC 失败完整保留 caller array。focused `sensor_sgp40`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 SGP40 实板、VOC 精度或耐久证据 | 本记录提交 | 2026-09-13 |
