...
| S2-184 | P0 | LPS22HB init config validation/interrupt programming | DONE | Zero | S2-183（DONE）；硬件测试按用户要求暂停 | TDD RED 证明非法 custom ODR 会进入 `WHO_AM_I` I/O，且 valid interrupt config 因 public threshold setter 的 initialized guard 在 ready commit 前失败。现 init 在 delay/I/O 前验证 ODR/LPF/FIFO/boolean/threshold，并以 private helper 在 ready commit 前写入 threshold；FIFO mode 不再错误混入 CTRL_REG2。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LPS22HB 实板、中断电气或耐久证据 | 本记录提交 | 2026-09-14 |
