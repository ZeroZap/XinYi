...
| S5-122 | P0 | Pandora PM sleep/wakeup 安全纵切 | VERIFYING | Zero | S5-76（DONE）；Pandora FreeRTOS tick IRQ | PM sleep API 不再空返回成功：新增 lifecycle/state guard，经 `xy_hal_sys_enter/exit_pwr_mode` 进入 STM32L4 shallow SLEEP/WFI，并由 FreeRTOS tick IRQ 唤醒；PC/L4/U5 backend 与 Host contract 已接入。clean committed `f7c62c2f` image 38576 bytes 已由 ST-Link write/verify，且同长度 Flash read-back 与 BIN 的 SHA-256 均为 `7c883f48…df21c`。独立 WCH-Link UART 在 reset/write-reset 前 open/flush 后进行 3 次各 12 秒有限捕获，均为 0 bytes，未取得 firmware identity 或 PM marker；因此保持 VERIFYING、不升级 B1。focused 2/2、Host 223/223、PC/L4/U5 compile、Pandora RTOS link/handler symbol 与 `git diff --check` 通过 | `docs/validation/evidence/pandora-stm32l475/2026-09-08/pm-sleep-wakeup-f7c62c2f.json` | 2026-09-08 |
