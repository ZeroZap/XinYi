...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE） | boot handoff/delta/mark-valid 均 fail-closed；双副本 metadata journal 持久化 generation+CRC+commit marker、pending version/slot 与 boot-attempt count；handoff/confirm/boot-attempt callback 均执行 load→状态转换→commit，boot-attempt 只有持久化成功才向 bootloader 返回 rollback 决策，提交失败不推进 durable count；饱和/损坏的 `uint8_t` attempt count 不再 wrap 后错误继续启动；journal 扫描遇到任一 Flash read 错误即 fail-closed，不把 unreadable copy 当作 empty/corrupt 后继续覆盖；提交后的 read-back 失败会擦除目标副本，避免重启后提升未经验证的新记录；commit 前拒绝非法 active slot、未知 flags、pending flag/slot/version/count 不一致、pending 与 active 同槽及低于 rollback floor 的候选，不对 Flash 产生副作用。FOTA focused 4/4、Host 185/185、PC root、FOTA-enabled `xy_fota` target、Arm M33 syntax probe、`git diff --check` 通过。尚缺 board-owned Flash ops/保留区、可链接 STM32U5 image、bootloader 实际调用与实板 | `54d8b735`～`PENDING` | 2026-08-27 |
