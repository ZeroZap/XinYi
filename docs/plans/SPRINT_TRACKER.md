...
| S2-203 | P0 | KX023 post-deinit read lifecycle guard | DONE | Zero | S2-202（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明成功 standby 后 `kx023_read()` 仍执行 I²C 并返回成功；新增 `initialized` lifecycle cache，init 成功后置位、deinit 成功后清零，read 无效状态返回 `SENSOR_EINVAL`，不访问总线。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 KX023 实板、动态姿态、量程/ODR 精度或耐久证据 | 本记录提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
