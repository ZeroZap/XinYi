...
| S2-03 | P0 | Pandora I2C→Device helper→现有非 SSD1306 设备纵切 | IN_PROGRESS | Zero | S2-02；Host 前置已完成 | software-I2C 已通过 HAL API→canonical Device helper 驱动 AHT10/AP3216C；AP3216C continuous `0x03` 的 bounded IR response B1 已保留。2026-09-07 又将 AP3216C 切换至 STM32L4 hardware I2C3（PC0/PC1）：clean `4873bd91` image write/verify/read-back byte-identical，独立 UART exact identity/config/read-back 后有 29 个 in-range samples/17 unique tuples/error 0，关闭 hardware-I2C peripheral 正常路径 B1。仍需真实 NACK→recovery B2；ALS/PS 不升级定量响应，SSD1306 排除，U5 仅 compile 补充 | `72391b51`～`f320087c`、`4873bd91` + 本记录提交 | 2026-09-07 |
