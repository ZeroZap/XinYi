...
| S4-01 | P0 | Sensor active-source manifest 与三轨 ownership 冻结 | DONE | Zero | S3-04 的 board-owned 集成阻塞不影响治理子项 | manifest 明确 55 个 `legacy-active-root`、23 个 `experimental-test-only` 与 4 个 `device-active-root`；policy CTest 防止 test-local source 被误报为 root product source，并冻结第四套生命周期；focused policy + SHT30/multi 3/3、Host 186/186、PC root build、`git diff --check` 通过；不升级硬件声明 | `f691bd23` | 2026-08-28 |
| S4-02 | P1 | SHT30 canonical Device owner 迁移 | IN_PROGRESS | Zero | S4-01（DONE） | 首个子项已用 RED 证明 Device driver 忽略 helper init、reset 失败且不校验 CRC；现已补 focused transaction/error/output-preservation contract；focused 4/4、Host 187/187、PC root 与 `DRIVER_SENSOR=ON` 的 `xy_drivers` clean build/archive、`git diff --check` 通过。legacy/test-local compatibility implementation 尚未收口，故不得标 DONE | `10543486` | 2026-08-28 |
