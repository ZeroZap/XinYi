...
| S2-03 | P0 | Pandora I2C→Device helper→现有非 SSD1306 设备纵切 | IN_PROGRESS | Zero | S2-02；Host 前置已完成 | Pandora board-owned software-I2C 已置于 `xy_hal_i2c_master_*` 后并通过 canonical `xy_i2c_device_*` 驱动板载 AHT10 与 AP3216C。AP3216C clean `85942a67` image 10160 bytes write/verify/read-back byte-identical；10 秒 UART 83 组样本匹配 exact identity、0x1E ACK 与 mode 0x07，错误 marker 为 0，取得静态正常路径 B1。最后 50 组不变，动态 ALS/PS/IR 仍需独立遮挡/靠近 capture；真实 NACK→recovery B2 与 hardware-I2C peripheral 仍缺，SSD1306 排除。U5 compile 仅补充 | `72391b51`、`6ec6081c`、`b94fc3c2`、`85942a67` + 本记录提交 | 2026-09-06 |
