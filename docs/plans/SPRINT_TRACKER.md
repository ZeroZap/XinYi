...
| S2-122 | P0 | LSM6DSO reinit public ODR cache 重同步 | DONE | Zero | S2-121（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 先设置 208Hz，再完整成功 reinit 到默认 104Hz，证明 private rates 已回到 104、public `odr` 仍为 208。现 init 仅在全部配置写成功后与四项 private cache 一起提交 `odr=104`；最终 write 失败时完整保留 nondefault private/public cache。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSO 实板或动态姿态证据 | 本记录提交 | 2026-09-13 |
