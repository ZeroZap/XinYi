...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora board runtime 已用 internal-Flash journal 完成 stage handoff→boot attempt→confirm、anti-rollback floor 与 bounded automatic rollback；canonical W25Q128 路径现将 clean committed Pandora image 的实际 Flash load span（33108 bytes）完整 erase/download/read-back CRC 到 512 KiB candidate region，ST-Link write/verify/read-back byte-identical，UART 匹配 exact SHA 且 full-image marker 1/error 0。该结果证明真实完整镜像 candidate storage B1，但仍不是独立 bootloader、候选执行、vector handoff、签名或真实掉电。STM32U5 仅保留增强兼容 | `54d8b735`～`fb12dab9` + 本记录提交 | 2026-09-05 |
