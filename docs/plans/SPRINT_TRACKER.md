...
| S2-110 | P0 | BNO055 聚合 fused sample 原子读取 | DONE | Zero | S2-109（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 在 Euler/Quaternion 成功、accel 失败时证明函数继续 I2C 并最终返回成功（`Expected -1 Was 0`）。现全部 fused/vector/temperature/calibration 字段写入 staged sample，任一 transport 失败立即传播原错误并停止后续 I/O，caller data 逐字节不变；全部成功后一次提交。focused `sensor_bno055`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BNO055 实板、融合精度或动态姿态证据 | 本记录提交 | 2026-09-13 |
