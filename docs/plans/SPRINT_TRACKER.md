...
| S2-75 | P0 | LPS22HB deinit/启动停止错误传播与 ready 保留 | DONE | Zero | S2-74（DONE）；硬件测试按用户要求暂停 | TDD 覆盖 deinit 与 start-single 的 stop 失败；两条路径均传播 transport 错误并保留 initialized；focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LPS22HB 实板、压力/温度精度、FIFO/中断或耐久证据 | 本记录提交 | 2026-09-12 |
