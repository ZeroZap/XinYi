...
| S2-44 | P0 | LSM6DSL 初始化配置缓存原子提交 | DONE | Zero | S2-43（DONE）；硬件测试按用户要求暂停 | TDD RED 注入 CTRL2_G 写失败，证明 CTRL1_XL 成功后仍残留 `accel_range`；现 accel/gyro 配置缓存仅在两项最终配置写入均成功后提交。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 状态契约，不升级 LSM6DSL 实板、姿态动态响应、精度、FIFO/中断或耐久证据 | 本记录提交 | 2026-09-11 |
