...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora 驻留 bootloader 已从 W25Q128 candidate 安装并跳转 `0x08008000` application。双记录 generation/CRC/末写 commit-marker journal 已实板证明首次安装与软件复位幂等；安装后 boot-attempt/confirmed/rollback-required 状态均持久化。新增独立 reviewed-restage API：仅接受与已验证 candidate exact version/size/CRC 绑定的显式 authorization，且只允许 `ROLLED_BACK→RESTAGE_AUTHORIZED` journal 转换；无效授权和 Flash 写失败均不修改 execution image，失败后旧 rollback record 继续阻止重装。focused 2/2、Host 212/212、PC/L4/U5 compile、Pandora opt-in bootloader link/符号/身份、14204-byte write/verify/read-back 均通过。实板已取得 `RESTAGE_AUTHORIZED→INSTALLED→ATTEMPT_COMMITTED→application` 正常链 B1；当前外部 W25Q128 candidate 仍是旧 application `89fc5a57`，不含 gated confirmation-request 修复，因此未观察 request/confirm/ack，未虚报完整确认链 B1/B2。真实掉电/半写与 approved signature provider 仍缺 | `54d8b735`～`f3b42a7c` + 本记录提交 | 2026-09-06 |
