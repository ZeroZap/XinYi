...
| S2-181 | P0 | LPS22HB FIFO mode/watermark boundaries | IN_PROGRESS | Zero | S2-180（DONE）；硬件测试按用户要求暂停 | TDD RED 覆盖非公开 FIFO mode `0x61` 与超出 5-bit watermark `32`；目标是在任何 I/O 前拒绝非法配置并保持 FIFO cache。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 待执行；不升级 LPS22HB 实板、FIFO 时序或耐久证据 | 本记录提交 | 2026-09-14 |
...
| S2-182 | P0 | LPS22HB invalid LPF enum boundary | IN_PROGRESS | Zero | S2-181（DONE）；硬件测试按用户要求暂停 | TDD RED 传入非公开 LPF `0x10`；目标是在任何 I/O 前拒绝非法滤波编码并保持 enable/cache。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 待执行；不升级 LPS22HB 实板、滤波特性或耐久证据 | 本记录提交 | 2026-09-14 |
...
| S2-183 | P0 | LPS22HB inverted threshold boundary | IN_PROGRESS | Zero | S2-182（DONE）；硬件测试按用户要求暂停 | TDD RED 传入 `low=0x2000`、`high=0x1000`；目标是在任何 I/O 前拒绝反转阈值范围并保持 threshold cache。focused `sensor_lps22hb`、Host/PC/L4/U5 compile 与 `git diff --check` 待执行；不升级 LPS22HB 实板、阈值精度或耐久证据 | 本记录提交 | 2026-09-14 |
