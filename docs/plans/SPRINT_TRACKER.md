...
| S2-47 | P0 | KX023 初始化配置缓存原子提交 | DONE | Zero | S2-46（DONE）；硬件测试按用户要求暂停 | TDD RED 注入最终 CNTL1 低功耗启动写失败，证明 ODCNTL 成功后残留 `odr` 缓存；现 ODCNTL 与 CNTL1 均成功后才提交 odr/mode。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过。仅为 Host/compile 状态契约，不升级 KX023 实板、低功耗电流、动态响应、精度或耐久证据 | 本记录提交 | 2026-09-11 |
