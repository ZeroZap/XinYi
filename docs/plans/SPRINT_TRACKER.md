...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora 已提供 board-owned internal-Flash metadata backend：保留 `0x0807E000..0x0807EFFF` 两页并避开既有 board-reserved 尾页；L4 double-word 写入、bank-2 page erase、journal commit/load/read-back 已实板闭环。clean `de7620cd` image write/verify/read-back byte-identical；UART 2441 bytes 含匹配固件身份与 `FOTA_METADATA_FLASH_OK`；4096-byte metadata read-back 可解析 generation=1/active version=1/slot=0。STM32U5 仅保留增强兼容。仍缺真实 bootloader 对 boot-attempt/handoff/confirm 的启动链、候选 image 双槽布局、自动 rollback/reset recovery 与掉电 B2 | `54d8b735`～`de7620cd` + 本记录提交 | 2026-09-04 |
