...
| S2-207 | P0 | legacy LSM6DSL range setter 参数边界 | DONE | Zero | S2-206（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 补充证明 `lsm6dsl_set_accel_range()`/`set_gyro_range()` 对 NULL device 与 `0xFF` 非法枚举会解引用或进入 SPI；现先校验 device/private/bus 与公开 range 枚举，返回 `SENSOR_EINVAL`，保持旧 cache 且零总线操作。focused `sensor_lsm6_imu_sensors`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSL 实板、量程精度或耐久证据 | 工作区提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
