...
| S2-200 | P0 | MLX90614 emissivity setter lifecycle guard | DONE | Zero | S2-199（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 `xy_mlx90614_set_emissivity()` 对未初始化设备返回 `XY_HAL_ERROR_NOT_SUPPORTED`，而非生命周期错误；现先要求 `dev->initialized`，返回 `XY_MLX90614_INVALID_PARAM`，避免无效设备进入 setter。focused `sensor_mlx90614`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 MLX90614 实板、红外测温精度、EEPROM 写入或耐久证据 | 本记录提交 | 2026-09-15 |
| S2-193 | P0 | HDC1080 Device lifecycle deinit | DONE | Zero | S2-192（DONE）；硬件测试按用户要求暂停 | TDD RED 补充确认 deinit 同步清除 embedded I²C Device lifecycle；现清除 `base.initialized`，保持无总线 I/O。focused `sensor_hdc1080`、Host 228/228、PC/L4/U5 compile 与 `git diff --check` 通过；不升级 HDC1080 实板、精度、heater 或耐久证据 | 本记录提交 | 2026-09-14 |
