...
| S2-242 | P0 | GD30DF deinit missing-bus 边界 | DONE | Zero | S2-241（DONE）；硬件测试按用户要求暂停 | active-linked compiled RED 证明 GD30DF legacy `deinit` 在 bus 缺失时错误返回成功；现对 NULL/private/bus 在 I/O 前 fail-closed。focused `sensor_stub_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 GD30DF 实板、加速度精度或耐久证据 | 本记录提交 | 2026-09-15 |
