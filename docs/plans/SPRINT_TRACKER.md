...
| S2-105 | P0 | APDS9960 手势 FIFO 深度寄存器契约 | DONE | Zero | S2-104（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 GVALID 置位后错误从 GSTATUS 位推导 FIFO 深度，跳过 GFLVL 读取并在其应失败时返回成功。现严格按 `GSTATUS → GFLVL → GFIFO_U` 顺序读取；GFLVL/GFIFO transport 失败返回 `SENSOR_EIO` 且保持 caller output，非零深度成功路径锁定顺序。focused `sensor_apds9960_sensor`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 APDS9960 实板或手势识别准确率证据 | 本记录提交 | 2026-09-13 |
