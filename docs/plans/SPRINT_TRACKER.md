...
| S5-109 | P0 | Pandora Actuator buzzer PB2 short-short-long | VERIFYING | Zero | Pandora PB2 active buzzer；ST-Link/WCH-Link 自动链路 | 新增 reusable Actuator buzzer API 与 GPIO-backed HAL adapter，Host 覆盖 init 双重 off-safe、active-high/low、on/off、pulse/pattern、参数拒绝、错误传播与 failure 后 off attempt；dedicated firmware 只通过 `xy_actuator_buzzer_*` 驱动 PB2，UART marker 与 final-low 由 fail-closed validator 守护。待 clean committed image 烧录/回读/UART；听感 B1 必须由人工确认，不从 GPIO/UART 推导 | pending | 2026-09-06 |
