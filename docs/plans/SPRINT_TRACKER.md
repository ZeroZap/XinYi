...
| S2-134 | P0 | IIS2ICLP deinit public ODR cache 原子提交 | DONE | Zero | S2-133（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 power-down write 成功后 public `odr` 仍为 100：`Expected 0 Was 100`。现 deinit 仅在 transport 成功后清零 public ODR；write 失败传播 `SENSOR_EIO` 并保留调用前 100Hz cache。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 IIS2ICLP 实板、功耗或耐久证据 | 本记录提交 | 2026-09-13 |
