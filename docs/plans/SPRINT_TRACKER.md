...
| S2-190 | P0 | BH1750 post-deinit power-control guards | DONE | Zero | S2-189（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 证明 deinit 后 `power_down()`、`power_on()`、`reset()` 仍可进入 I²C helper；现要求 initialized，失败前无总线副作用并返回 `XY_BH1750_INVALID_PARAM`。focused `sensor_bh1750`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 BH1750 实板、光照精度或耐久证据 | 本记录提交 | 2026-09-14 |
