...
| S5-110 | P0 | Pandora Actuator motor/vibration PA1/PA0 bounded forward pattern | IN_PROGRESS | Zero | V2.4 schematic；用户提供 driver truth table | 原理图网络确认 PA1=`MOTOR_A` 经 R67 到 U12 IA、PA0=`MOTOR_B` 经 R66 到 U12 IB；reusable H-bridge Actuator 固定 L/L fail-safe standby、非 standby 模式切换强制 break-before-make、GPIO failure 回落 standby，单 step 限 5 秒。clean committed `7c779ce4` 7420-byte image write/verify/read-back byte-identical，BIN SHA-256=`c53d349c...`；8 秒 reset-synchronized UART capture 201 bytes，dedicated validator 确认 exact identity、forward 120/120/300 ms bounded pattern 与最终 L/L，error marker 0。focused 2/2、Host 220/220、PC/L4/U5 compile 与 `git diff --check` 通过。control path B1 candidate 已取得；仍待现场确认实际振动模式与最终停止。原理图标注 `TC214B`，exact chip identity 未独立确认 | `7c779ce4` + 本记录提交 | 2026-09-06 |
