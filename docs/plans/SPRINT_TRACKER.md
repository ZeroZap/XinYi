...
| S5-41 | P0 | Pandora STM32L475 board runtime smoke | BLOCKED | Zero | S5-40（DONE）；稳定 UART capture 路径 | ST-Link 已识别目标并完成 7248-byte image write/verify；打开 `/dev/ttyACM0` 后 capture 超时且 ST-Link/VCP 从 USB 掉线，未取得 banner/LED/KEY0/AHT10 runtime 证据；记录为 `BLOCKED_VCP_UNSTABLE`，不升级 B1/B2 | [记录](../validation/xinyi-pandora-stm32l475-board-smoke-record.md) | 2026-09-02 |
...
| S5-43 | P0 | Pandora UART capture 固件身份绑定 | VERIFYING | Zero | S5-42（DONE） | RED：capture metadata 自动记录当前 checkout `HEAD`，无法绑定被烧录固件；现要求显式 exact 40-char `--firmware-commit`，PTY contract 覆盖缺失/非 exact identity。focused 2/2 与 `git diff --check` 已通过；committed clean-export/full Host/PC gate 待实现提交后验证，不升级 runtime/B1/B2 | 本轮提交 | 2026-09-02 |
