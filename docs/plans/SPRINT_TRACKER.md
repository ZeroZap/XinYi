...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora 驻留 bootloader 已从 W25Q128 candidate 安装并跳转 `0x08008000` application。双记录 generation/CRC/末写 commit-marker journal 已实板证明首次安装与软件复位幂等；本轮新增安装后 boot-attempt/confirmed/rollback-required durable 状态，application 以 RTC backup handshake 请求确认，bootloader 仅在 journal commit 成功后确认或推进 attempt，3 次失败后 fail-closed 停止同 candidate 重装。Host 覆盖确认幂等、旧 reserved 字段兼容、bounded failure 与 commit/read failure 输出保持；L4 application/bootloader link 与 U5 compile 通过。实板复验发现此前已持久化的 failed-attempt journal 会在当前 candidate 进入 application 前触发 `PANDORA_BOOT_ROLLBACK_REQUIRED`；已修复 application 避免普通 boot 自发请求确认，但完整 `attempt→request→confirm→ack` 仍需受检 restage/旧 journal 重置流程，故不升级本链路 B1/B2。真实掉电/半写与 approved signature provider 仍缺 | `54d8b735`～`c8e04600` + 本记录提交 | 2026-09-06 |
