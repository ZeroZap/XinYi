...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE）；Pandora board owner | Pandora board runtime 已用 internal-Flash journal 完成 stage handoff→boot attempt→confirm、anti-rollback floor 与 bounded automatic rollback；canonical W25Q128 路径现将 clean committed Pandora image 的实际 Flash load span完整写入带版本化 header 的 512 KiB candidate region，并 fail-closed 校验 header/layout、全镜像 CRC32 与 Cortex-M vectors。新增从已验证 candidate 到 internal execution slot 的 erase→分块 program→逐块 read-back 安装 contract，并独立链接 `0x08008000` application image；尚无驻留 `0x08000000` bootloader、安全的在板 copy/jump 或 candidate execution marker，故未烧录该 application。签名与掉电仍 pending；U5 仅保留增强兼容 | `54d8b735`～`fb12dab9` + 本记录提交 | 2026-09-05 |
