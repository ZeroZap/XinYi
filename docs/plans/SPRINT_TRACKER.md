...
| S2-161 | P0 | LIS2DW12 invalid rate boundary | DONE | Zero | S2-160（DONE）；硬件测试按用户要求暂停 | active-linked TDD RED 传入 `rate=0xFF`，fake transport 捕获非预期 I²C read，证明非法值抵达硬件边界并可能被归一化为虚假 100Hz cache。现 setter 在首个 I/O 前拒绝 NULL/missing private/高于 800Hz encoding 的 rate，返回 `SENSOR_EINVAL`、零 bus 操作并保持 private/public ODR cache。focused `sensor_low_power_accel_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LIS2DW12 实板、精度或耐久证据 | 本记录提交 | 2026-09-14 |
