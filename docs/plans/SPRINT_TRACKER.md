...
| S2-93 | P0 | LSM6DSL gyro range 编码与位域修正 | DONE | Zero | S2-92（DONE）；硬件测试按用户要求暂停 | TDD RED 证明设置 500dps 时错误覆盖 CTRL2_G ODR（寄存器 `0x40` 期望 `0x44`，实际 `0x20`）；同时修正 125/250/500/1000/2000dps 的无歧义逻辑编码，分别编码 FS_125 bit1 或 FS_G `[3:2]`，补齐 2000dps 70mdps/LSB scale，并锁定 125/2000 寄存器写值。focused `sensor_lsm6_imu_sensors`、Host/PC/L4/U5 compile 与 `git diff --check` 通过；不升级 LSM6DSL 实板、量程精度或动态响应证据 | 本记录提交 | 2026-09-12 |
