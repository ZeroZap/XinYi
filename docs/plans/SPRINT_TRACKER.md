...
| S3-04 | P0 | FOTA 状态机去模拟化与 bootloader contract | IN_PROGRESS | Zero | S3-02（DONE） | boot handoff/delta/mark-valid 均 fail-closed；双副本 metadata journal 持久化 generation+CRC+commit marker、pending version/slot 与 boot-attempt count；handoff/confirm/boot-attempt callback 均执行 load→状态转换→commit，boot-attempt 只有持久化成功才向 bootloader 返回 rollback 决策，提交失败不推进 durable count；饱和/损坏的 `uint8_t` attempt count 不再 wrap 后错误继续启动；journal 扫描遇到任一 Flash read 错误即 fail-closed，不把 unreadable copy 当作 empty/corrupt 后继续覆盖；提交后的 read-back 失败会擦除目标副本；commit/load 及公开 stage/attempt/confirm 状态转换均校验 slot/flags/pending、`active_version >= min_version`，非法内存状态不被推进；backend 地址范围也在任何 Flash I/O 前校验，避免双槽地址计算发生 `uint32_t` wrap。FOTA focused 4/4、Host 185/185、PC root、FOTA-enabled `xy_fota` target、Arm M33 syntax probe、`git diff --check` 通过。尚缺 board-owned Flash ops/保留区、可链接 STM32U5 image、bootloader 实际调用与实板 | `54d8b735`～`1aa89970` + 本轮提交 | 2026-08-27 |
