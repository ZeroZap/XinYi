...
| `sensor` 新 `xy_*` | B-/C | 7 个 `src/xy_*.c`，多个环境/光学/存储 Host 测试 | 当前根 `components/sensor/CMakeLists.txt` 未纳入这些实验实现，即“测试过但未进入根产品库” | 保持 `experimental-test-only`，按 manifest 迁移而不新增平行 owner |
| `drivers/sensor` Device 路径 | B | 27 个 Device-model canonical owner，均进入 `xy_drivers` 并有 focused Host 契约；其中迁移 wrapper 所需 owner 也显式进入 `sensor_component`；MLX90614 EEPROM 写仍明确 unsupported | legacy compatibility wrapper 与其余 experimental source 尚待逐步收敛；全部仍缺完整 B1/B2 | 维持 Device-model canonical owner 决策，禁止第四套生命周期；按芯片迁移兼容边界 |
