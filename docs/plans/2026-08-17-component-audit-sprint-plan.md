...
| `drivers/sensor` Device 路径 | B | 13 个 Device-model canonical owner，均进入 `xy_drivers` 并有 focused Host 契约；其中迁移 wrapper 所需 owner 也显式进入 `sensor_component` | legacy compatibility wrapper 与其余 experimental source 尚待逐步收敛；全部仍缺完整 B1/B2 | 维持 Device-model canonical owner 决策，禁止第四套生命周期；按芯片迁移兼容边界 |
