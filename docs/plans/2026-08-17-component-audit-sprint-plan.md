...
| `sensor` legacy `sensor_*` | B | 根 `sensor_component` 实际编译 framework 与约 50 个 `sensor_*.c`；大量 focused CTest 覆盖解析、边界和 I/O 失败 | 当前产品主路径，但部分 stub/简单驱动的测试不等于完整数据手册实现；fusion 部分路径仍返回 `SENSOR_ENOSYS`；多数无实板证据 | 冻结新增 legacy；逐批迁移到 Device 模型，保留兼容层 |
