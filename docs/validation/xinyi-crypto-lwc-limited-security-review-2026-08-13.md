...
- Test evidence:
  - `crypto_lwc` covers Ascon/TinyJambu variant roundtrip/input guards, Ascon-128a incremental split-update equivalence and contiguous auth-failure plaintext wipe, TinyJambu-128 incremental empty-AD and split-update equivalence plus contiguous decrypt output and auth-failure/missing-tag plaintext wipe, and Photon-Beetle one-shot/incremental equivalence across split updates, contiguous-output enforcement, auth-failure and missing-tag plaintext wipe, final context scrubbing on success/failure, incremental state ordering, empty-AD parity, rate-boundary roundtrip, 128/64-bit tag tamper, pointer/length guards, canary, and hash behavior. These remain local self-consistency/API contracts rather than authoritative KAT evidence.
