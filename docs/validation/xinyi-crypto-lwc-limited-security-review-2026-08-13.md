...
- Test evidence:
  - `crypto_lwc` covers Ascon/TinyJambu variant roundtrip/input guards plus Photon-Beetle one-shot/incremental equivalence across split updates, incremental state ordering, empty-AD parity, rate-boundary roundtrip, 128/64-bit tag tamper, pointer/length guards, bounded wipe, canary, and hash behavior.
