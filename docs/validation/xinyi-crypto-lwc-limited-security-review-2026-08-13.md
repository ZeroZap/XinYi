...
- Test evidence:
  - `crypto_lwc` covers Ascon encrypt variants/hash, Ascon-128/128a/80pq and TinyJambu variant multi-length roundtrip/bad-tag/bounded-wipe behavior, plus Photon-Beetle rate-boundary roundtrip, 128/64-bit tag tamper, pointer/length guards, bounded wipe, canary, and hash behavior.
