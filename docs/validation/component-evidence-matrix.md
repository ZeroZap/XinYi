...
| FOTA | H1（状态机 + signature-provider/boot-handoff fail-closed） | PC/FOTA target build | pending | Secure FOTA blocked；无 approved provider | Host 原型；缺 signature provider 或 boot-handoff callback 均 fail-closed；provider 拒绝、错误 key ID、回滚版本和截断包失败；不调用 security-rejected ECDSA placeholder；不模拟成功切槽 | reviewed provider + key provisioning + durable boot metadata/mark-valid/rollback/掉电 B2 |
