...
| Actuator | H1（framework + GPIO-backed buzzer/RGB/H-bridge motor fail-safe contracts） | PC/L4/U5 build | Pandora PB2 buzzer 与 PE7/PE8/PE9 RGB 已有 control-path evidence；V2.4 原理图确认 PA1=`MOTOR_A`→IA、PA0=`MOTOR_B`→IB。motor `7c779ce4` image 已 write/verify/read-back byte-identical，UART exact identity、bounded forward pattern 与最终 L/L marker 通过 dedicated validator；物理振动与最终停止确认 pending | safety pending | motor API 固定 L/L standby、方向切换先回 L/L、单步最长 5 秒；board image 只使用 bounded forward pulse，不使用 reverse/H/H brake；exact U12 model 仍未从原理图之外确认 | 由现场确认实际振动模式与最终停止；PWM/功耗/重复启停耐久仍 pending |
