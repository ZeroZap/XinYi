...
| Sensor new `xy_*` | H1（独立测试） | [manifest](sensor-active-source-manifest.md) 记录 17 个 `experimental-test-only` sources，未进入根 Sensor target；SHT30/ADS1115/MPU6050/BH1750/AHT20/SHT40 duplicate test-local implementations 已移除或迁出 | pending | pending | 仅 test-local Host 实验实现；不得因 focused test 宣称 product-linked | 继续冻结新增并迁移高价值 owner |
