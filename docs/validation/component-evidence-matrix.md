...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；完整镜像跨 chunk CRC32；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage→attempt→confirm 与 anti-rollback B1，并取得 bounded automatic rollback/reset recovery B2；canonical W25Q128 candidate adapter 已完成 bounded storage B1，core download→全镜像 CRC 的新 runtime slice 待 committed image 实板闭环 | Secure FOTA blocked；无 approved provider | Pandora board runtime 的 metadata callback/persistence、anti-rollback、automatic rollback/reset recovery与 bounded candidate storage；不等于独立 bootloader、候选 image 执行、真实掉电或安全批准 | core download/CRC 实板闭环；双槽候选 image、真实 bootloader vector handoff、真实掉电 B2；reviewed provider/key provisioning |
