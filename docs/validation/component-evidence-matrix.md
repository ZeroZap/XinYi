...
| Charger | H1（standalone BQ25620 fake-I2C transaction/status contract；公开 full config 与 direct setters 在首个 I2C 写前拒绝越界或无法按寄存器步长精确表示的 input/charge/precharge/termination current、charge voltage 与 recharge threshold；两类配置路径使用 read-modify-write 保留非目标寄存器位并传播 read failure；未知 charge/fault 编码 fail-closed 为 FAULT/UNKNOWN；status 拒绝超出公开范围的保留 setpoint 编码并保持 caller output；public/direct callback register access 在 I2C 前拒绝 documented `0x00..0x15` 之外地址并保持 caller output） | PC/L4/U5 build | pending | safety pending | `components/drivers/power/charger/xy_bq25620.c` 为 canonical owner；公开 API 与 focused contract 迁移后旧 `components/charger/` compatibility component 及 ownerless Device dispatch 已移除；固定 7-bit 地址 `0x6A`，live-transport fail-closed、完整 status staged commit、首次失败 probe 清零、live owner re-init 失败保持原 transport/callback/lifecycle；teardown 的 stop-charge read/write 失败传播且保留完整 live owner 供重试，成功后才清理 context | 充电/热故障 B1/B2；Pandora 无 charger IC |
