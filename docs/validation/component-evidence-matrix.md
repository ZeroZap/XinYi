...
| CI/Release | canonical Host gate + PC root build 可用（2026-08-24：178/178） | PC build；[Kconfig/CMake 配置矩阵](kconfig-cmake-configuration-matrix.md)已建立，非法 Display 子功能组合 fail-closed，OLED/SSD1306、LCD SPI/I8080/ST7789 与 LED/serial RGB 合法组合已验证 source selection；无实现源的 standalone RGB 配置已移除 | n/a | supply-chain pending | development CI only；`unit-tests.yml` 为单一可信 Host/PC gate | 完成全关/逐组件/Sensor/STM32U5 矩阵、SBOM、可复现制品 R1 |
