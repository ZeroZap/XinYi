...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；版本化 candidate header/layout、完整镜像 CRC/vector 校验；validate-before-erase、分块 program/read-back 的 execution-slot install contract；双副本 metadata journal guards） | PC/FOTA target；L4/U5 compile compatibility；Pandora 独立 application 从 `0x08008000` 链接 | Pandora board-owned internal-Flash journal 已取得跨复位 metadata B2；canonical W25Q128 已完成完整镜像 candidate storage 与板上 header/CRC/vector validation；execution-slot install 尚仅 Host contract/layout compile | Secure FOTA blocked；无 approved provider | Pandora metadata/reset recovery、完整镜像 candidate storage 与 bootable-layout/vector validation；不等于独立 bootloader、internal-Flash 在板安装、候选执行或真实 vector jump、真实掉电、安全批准 | 驻留 bootloader + board internal-Flash copy ops + 安全 VTOR/MSP/reset handoff，取得 candidate execution marker；真实掉电 B2；reviewed provider/key provisioning |
