...
| Crypto | H1（契约） | C1 部分 | pending | product-classification/owner/origin/license/side-channel/allowed-use 清单已机器守护；provenance 均 review-pending；SM2/ECDSA rejected | contract-guarded；production-candidate 仅表示后续重建/审查候选，禁止泛化安全/性能 | signature provider fail-closed；从 SHA-256/HMAC 候选开始单算法 S1/P1 |
...
| FOTA | H1（状态机 + signature-provider fail-closed） | PC/FOTA target build | pending | Secure FOTA blocked；无 approved provider | Host 原型；缺 provider、provider 拒绝、错误 key ID、回滚版本和截断包均 fail-closed；不调用 security-rejected ECDSA placeholder | reviewed provider + key provisioning + bootloader/掉电 B2 |
