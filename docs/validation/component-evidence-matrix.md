...
| GUI | H1（backend 错误传播；字体 source-table review） | PC build；SDL2 opt-in 缺依赖/缺实现均 fail-closed | pending | 字体视觉 `rejected-needs-regeneration`；license/provenance/performance pending | Host GUI/font/backend contract；init/clear/draw/fill/flush backend 错误不再归一化为成功；legacy 16x24/中文资产不得作为最终产品字体；SDL2 backend 当前默认关闭且不可用 | 选择有许可证证据的字体源并建立 asset snapshot；恢复 SDL source/link；屏幕 B1/P1 |
