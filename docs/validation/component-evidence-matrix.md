...
| Kernel/OSAL | H1（bare-metal） | Q1 部分；[FreeRTOS reference](reference-rtos-decision.md) 已有 project-owned config、pinned V10.4.6 Cortex-M33 non-secure port，并由 Arm GNU 15.2 `-Werror` 编译 adapter/kernel/heap/port 共 9 objects；未链接/运行 | pending | pending | bare-metal Host 契约；FreeRTOS 仅具 source-level compile 前置，不是 runtime/ISR/并发证据 | root Kconfig/CMake fail-closed selection + link；thread/sync/queue/ISR-to-task stress 与 B1/B2 |
