...
| Drivers Sensor | H1（10 个 canonical Device owners；当前 Pandora I2C2 7 个 owners 均有 focused normal/error/output-cache/lifecycle contracts；BME680 使用 pinned Bosch BME68x API） | PC/L4/U5 build；[manifest](sensor-active-source-manifest.md) 记录 10 个 `device-active-root` sources，均进入 `xy_drivers`；Host 233/233 | Pandora I2C2 PB10/PB11 已确认 `0x18/0x1E/0x23/0x38/0x68/0x69/0x77`；SC7A22H identity/vendor profile/~1g/旋转/PB8 EXTI B1；HMC5883L identity/gain/XYZ/mgauss/磁场刺激 B1；AHT30/BH1750/MPU6050/L3G4200D/BME680 连续采样 B1 | pending | 当前设备优先稳定门已关闭，可转入 legacy `sensor_device_t` 分批迁移；不声明校准精度、FIFO/self-test、长期可靠性或 I2C fault recovery/B2 | 按单 owner + compatibility wrapper + manifest guard 迁移 legacy；当前 I2C2 owners 的 NACK/re-init B2 独立排期 |
