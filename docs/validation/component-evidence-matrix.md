...
| HAL / Storage | H1（PC；canonical W25Q128 command/error/page-boundary contract） | Q1 部分；新增通用 QSPI HAL API、STM32L4 backend 与 canonical W25Q128 Device driver；PC/L4/U5 compile | Pandora U9 W25Q128 的 JEDEC、erase、single/quad program/read、controller re-init 与 MCU software-reset recovery 已全部迁移为 `xy_w25q128 → xy_hal_qspi → STM32L4 HAL`；设备注册为 `XY_DEV_TYPE_FLASH` | pending | 允许声明 Pandora canonical storage path B1/B2；不升级掉电/NRST、耐久、性能、FOTA/DM consumer 或 STM32U5 runtime | FOTA candidate storage 接入；掉电/NRST恢复；移除/冻结 legacy Sensor W25Q 与 DM NOR placeholder |
