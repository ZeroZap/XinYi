...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0；framework DMA/SPI recovery 已有 B1/B2；板载 U9 W25Q128 已取得 QSPI JEDEC ID 及独立 4 KiB 测试扇区 erase→256-byte page-program→read-back B1 | pending | Pandora W25Q128 仅证明单线 QSPI 识别与受控测试区擦写；不升级四线模式、掉电恢复、耐久、FOTA storage、性能或 STM32U5 runtime | W25Q128 掉电恢复；四线模式；FOTA candidate storage |
