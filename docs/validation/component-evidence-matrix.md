...
| PM | H1（sleep lifecycle/state/error contract；shutdown 无 backend 时 fail-closed） | PC/L4/U5 build；STM32L4 shallow SLEEP/WFI 与 PC/U5 compatibility backend 已接入 | Pandora runtime 已证明 PM platform tick adapter 经 `XY_OSAL_AVAILABLE` 使用 canonical OSAL tick，20 个 bounded 周期无 PM error；sleep/wakeup clean committed image 已 write/verify/read-back byte-identical，但 WCH-Link 多次 reset 同步捕获均为 0 bytes；回刷普通 Pandora smoke 亦无 UART，确认当前 runtime 阻塞为独立 UART 链 | power pending | Host framework/fallback；Pandora 当前仅证明 tick adapter B1，sleep/wakeup 只有 image/link/Flash evidence；shutdown 未接 board backend 时返回 unsupported，不等于 runtime、低功耗、关机、功耗或 charger/ADC 实板能力 | 修复/升级 ST-Link VCP 或接入稳定 USB-UART 后重做 sleep→tick IRQ wake→state recovery marker；后续补电流记录 |
