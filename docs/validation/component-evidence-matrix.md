...
| Drivers Sensor | H1（SHT30、ADS1115、MPU6050 与 BMP280 Device owners 已覆盖 focused transaction/error/output-preservation contracts） | PC build；[manifest](sensor-active-source-manifest.md) 记录四个 `device-active-root` sources，且四个 canonical owners 均进入 `xy_drivers` 与 root `sensor_component`；BMP280 legacy lifecycle 仍冻结兼容，experimental source 不进入产品 root | pending | pending | Device-model canonical migration destination；四个 owner 的 root/source 边界已明确 | 四个 owner 的 B1/B2；BMP280 experimental API 的 compatibility/deprecation 决策 |
