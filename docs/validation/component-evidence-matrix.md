...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0；framework DMA/SPI recovery 已有 B1/B2；板载 U9 W25Q128 已取得 QSPI JEDEC ID、独立测试扇区 erase/program/read、quad-output fast read B1，以及 QSPI controller re-init 与 MCU software-reset 后持久化读回 B2 | pending | Pandora W25Q128 仅证明单线命令/写入、四线数据读取、受控测试区擦写、控制器重初始化及一次 software-reset recovery；不升级 quad page-program、掉电/NRST、耐久、FOTA storage、性能或 STM32U5 runtime | W25Q128 掉电/NRST 恢复；quad page-program；FOTA candidate storage |
