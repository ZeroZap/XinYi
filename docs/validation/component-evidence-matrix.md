...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；版本化 candidate header/layout、完整镜像 CRC/vector 校验；validate-before-erase、分块 program/read-back；执行基址/program-granule 对齐门；install-once journal-before-write 布局门；双记录 generation/CRC/末写 commit-marker 安装 journal；同 generation split-brain 与半范围 generation 歧义拒绝；安装后 boot-attempt/confirmed/rollback-required durable 状态；reviewed-restage exact candidate authorization） | PC/FOTA target；L4/U5 compile compatibility；Pandora bootloader `0x08000000` + application `0x08008000` 独立链接；confirmation handshake 已链接 | Pandora 驻留 bootloader 已完成 W25Q128 candidate→internal execution slot 安装、VTOR/MSP/reset handoff与 application marker；software reset 后 journal 命中同 candidate并跳过重复擦写。显式 opt-in restage image 已在旧 rollback journal 上持久化 exact candidate 授权，随后重装、提交 attempt 并执行 application，取得该 restage 正常路径 B1；当前 W25Q128 candidate 为旧 application `89fc5a57`，未发出 confirmation request，因此未取得 confirm/ack chain | Secure FOTA blocked；无 approved provider | Pandora candidate 安装/执行正常路径 B1 与软件复位幂等 B2；Host 证明无效授权或 Flash failure 不损伤 execution image，失败 journal 仍阻止重装。完整 `attempt→request→confirm→ack` 尚未升级 B1/B2 | 用当前 confirmation-capable app 重新生成外部 candidate 后重跑确认链；真实掉电/半写；reviewed provider/key provisioning |
