...
| Drivers Sensor | H1（SHT30、ADS1115、MPU6050、BMP280、AHT30、L3G4200D、BH1750、HMC5883L、SC7A22H focused contracts；BME680 使用 pinned Bosch BME68x API） | PC/L4/U5 build；[manifest](sensor-active-source-manifest.md) 记录 10 个 `device-active-root` sources，均进入 `xy_drivers`；BMP280/BH1750/HMC5883L lifecycle 仍冻结兼容 | Pandora I2C2 PB10/PB11 识别 `0x38`/`0x69`/`0x77`，完成 20 轮三设备 sample；BME680 identity `0x61`；HMC5883L 已接入 compile-only probe；SC7A22H 已接入 compile-linked I2C2 entry，但无实板 identity/register/data evidence | pending | Device-model canonical migration destination；AHT30/L3G4200D/BME680 已取得 basic-chain B1；SC7A22H 仅有 Host contract/compile evidence，不等于 identity、寄存器行为、加速度数据或 I2C fault recovery | 为 BME680/BH1750/HMC5883L/SC7A22H 增加 focused fake-I2C compensation/error contract；补 AHT30/L3G4200D/BME680 NACK/re-init B2 |
