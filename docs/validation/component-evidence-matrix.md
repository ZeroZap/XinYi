...
| GUI | H1（backend 错误传播；字体 source-table review；licensed required-UI subset active） | PC build；SDL2 opt-in 缺依赖/缺实现均 fail-closed | pending | legacy 字体视觉 `rejected-needs-regeneration` 且 provenance pending；OFL-1.1 Noto Sans CJK SC 的 15 个 required UI glyph 已按 pinned Host snapshot 接入 active 16x16 table，但尚未视觉/实板批准；performance pending | Host GUI/font/backend contract；init/clear/draw/fill/flush backend 错误不再归一化为成功；required UI subset 为 distinct/nonblank active table；其余 legacy 16x24/中文 placeholder 不得作为最终产品字体；SDL2 backend 当前默认关闭且不可用 | 人工视觉审查；替换其余 legacy table；恢复 SDL source/link；屏幕 B1/P1 |
