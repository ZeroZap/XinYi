...
| Kernel/OSAL | H1（bare-metal） | Q1 部分；[FreeRTOS reference](reference-rtos-decision.md) 已有 project-owned config、pinned V10.4.6 Cortex-M33/CM4F ports；root U5/L4 opt-in 可构建 `freertos_kernel` + `xy_osal` | Pandora STM32L475VE 已取得 scheduler/delay、task-context binary semaphore/message queue/event flags/mutex、SysTick ISR 与 TIM6 peripheral IRQ→OSAL semaphore→task、no-wait memory-pool/queue exhaustion→recovery→delete/recreate、depth-one queue 100-tick blocking timeout、120 秒 bounded stress，以及 OSAL→IPC Broker→Device registry→CLIB Trace sink 跨组件 B1 | pending | bare-metal Host 契约；STM32U5 仍为 compile-only；Pandora 仅证明声明区间内的 task primitives、两个明确 ISR-to-task 路径、资源恢复、blocking-timeout、bounded stress 与 IPC/Device/Trace 纵切，不是性能、多小时耐久或完整产品 RTOS 资格 | 补外设 IRQ 负向/恢复、PM sleep/wakeup、长时间耐久与 B2 |
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得匹配 `9b50ec38` 固件身份的 UART/AHT10、LED 与 KEY0 正常路径 [B1 记录](xinyi-pandora-stm32l475-board-smoke-record.md)；STM32U5 [HAL/HIL 记录](xinyi-stm32u5-hal-hardware-validation-record.md)仍为 `BLOCKED_NO_HARDWARE` | pending | PC simulation、部分 QEMU；Pandora B1 只覆盖 board-local LED/USART1/software-I2C AHT10/KEY0 正常路径，不升级 XinYi STM32L4 framework wrapper、STM32U5、B2 或性能声明 | Pandora AHT10 NACK→recovery B2；STM32U5 clean compile 及 GPIO/UART/I2C/SPI/IRQ/DMA B1/B2 原始日志与 capture |
...
| IPC | H1 | Q1 间接/部分 | Pandora Broker producer→consumer dispatch、单调 payload、Device lookup 与 Trace sink 已完成 17 个有序周期 B1 | concurrency pending | Host 契约 + Pandora bounded task-context 跨组件正常路径 | queue saturation、ISR ingress、多 producer/consumer stress |
...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage→attempt→confirm 与 anti-rollback B1，并取得 bounded automatic rollback/reset recovery B2：clean `d37ecf1c` image 11992 bytes write/verify/read-back byte-identical；5-boot UART chain 绑定 exact firmware SHA，v3 pending attempt 后自动清除 pending 并保留 active/min version=2；4096-byte metadata latest generation=7、CRC valid | Secure FOTA blocked；无 approved provider | Pandora board runtime 的 metadata callback/persistence、anti-rollback 与 automatic rollback/reset recovery；不等于独立 bootloader、候选 image 执行、真实掉电或安全批准 | 双槽候选 image、真实 bootloader vector handoff、真实掉电 B2；reviewed provider/key provisioning |
