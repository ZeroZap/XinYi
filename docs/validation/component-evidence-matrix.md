...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0 正常路径；dedicated STM32L4 framework DMA 已取得 DMA1 Channel1、8-word SRAM→SRAM polling 与 completion IRQ→framework callback→OSAL semaphore→task B1（clean `308faceb` image write/verify/read-back，UART fail-closed marker） | pending | PC simulation、部分 QEMU；Pandora B1 覆盖 board-local smoke 与明确的 framework DMA mem2mem polling/completion callback，不升级 SPI/DMA peripheral request、error IRQ、B2、性能或 STM32U5 runtime | Pandora framework SPI/peripheral-DMA；DMA error IRQ 与 stop/recovery；AHT10 NACK→recovery B2 |
