...
| GUI | H1（backend 错误传播；字体 source-table review；licensed replacement candidate snapshot） | PC build；SDL2 opt-in 缺依赖/缺实现均 fail-closed | pending | legacy 字体视觉 `rejected-needs-regeneration` 且 provenance pending；replacement source 为 OFL-1.1 Noto Sans CJK SC，Host candidate 尚未视觉/实板批准；performance pending | Host GUI/font/backend contract；init/clear/draw/fill/flush backend 错误不再归一化为成功；legacy 16x24/中文资产不得作为最终产品字体；replacement 仅覆盖 15 个必需 UI glyph 且未进入 firmware；SDL2 backend 当前默认关闭且不可用 | 审查并接入 replacement table；恢复 SDL source/link；屏幕 B1/P1 |
