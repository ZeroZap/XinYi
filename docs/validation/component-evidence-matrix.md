...
| GUI | H1（2026-08-24：backend 错误传播） | PC build；SDL2 opt-in 缺依赖/缺实现均 fail-closed | pending | visual/performance pending | Host GUI/font/backend contract；init/clear/draw/fill/flush backend 错误不再归一化为成功；SDL2 backend 当前默认关闭且不可用 | 恢复 SDL source/link gate + 屏幕 B1/P1 |
