...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；完整镜像跨 chunk CRC32；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage→attempt→confirm 与 bounded automatic rollback B2；canonical W25Q128 adapter 已将 clean committed Pandora image 的完整 33108-byte Flash load span 写入独立 512 KiB candidate region 并全量重读 CRC B1 | Secure FOTA blocked；无 approved provider | Pandora metadata/reset recovery与真实完整镜像 candidate storage；不等于独立 bootloader、候选执行/vector handoff、真实掉电或安全批准 | bootable candidate layout、真实 bootloader vector handoff、真实掉电 B2；reviewed provider/key provisioning |
