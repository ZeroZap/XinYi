...
| Actuator | H1（framework + GPIO-backed buzzer fail-safe/pattern contract） | PC/L4/U5 build | Pandora PB2 short-short-long committed firmware 正在验证；UART/control-path 与 final-low 不等于可听 B1 | safety pending | Host framework 与 buzzer adapter 可用；听感必须由人工确认 | PB2 write/verify/read-back/UART 后请求人工确认 short-short-long 与停止；再决定 audible B1 |
