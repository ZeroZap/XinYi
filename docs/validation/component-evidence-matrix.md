...
| Sensor new `xy_*` | H1（独立测试） | [manifest](sensor-active-source-manifest.md) 记录 21 个 `experimental-test-only` sources，未进入根 Sensor target；SHT30/ADS1115 duplicate test-local implementations 已移除 | pending | pending | 仅 test-local Host 实验实现；不得因 focused test 宣称 product-linked | MPU6050 ownership 迁移试点 |
| Drivers Sensor | H1（少量；SHT30 与 ADS1115 Device owners 已覆盖 focused transaction/error/output-preservation contracts） | PC build；[manifest](sensor-active-source-manifest.md) 记录 SHT30/MPU6050/BMP280/ADS1115 四个 `device-active-root` sources；SHT30 与 ADS1115 canonical owners 同时进入 `xy_drivers` 与 root `sensor_component` | pending | pending | Device-model canonical migration destination；SHT30/ADS1115 各只有一个实现 owner | MPU6050 ownership 迁移 + SHT30/ADS1115 B1/B2 |
