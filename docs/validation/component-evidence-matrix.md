...
| Actuator | H1（framework + GPIO-backed buzzer fail-safe/pattern contract） | PC/L4/U5 build | Pandora PB2 clean committed image 已 write/verify/read-back；UART exact identity、pattern start/done 与 final-off 有序且 error 0；现场人工确认短—短—长可听且最终停止，固定模式 audible B1 | safety pending | Host framework 与 buzzer adapter可用；仅允许声明该 PB2 固定模式的可听正常路径，不构成声压、频率、长期可靠性或其他执行器安全资格 | PWM/频率控制、声压/功耗与重复启停耐久；其他 Actuator Device adapter B1/B2 |
