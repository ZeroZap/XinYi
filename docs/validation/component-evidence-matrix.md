...
| HAL / Storage | H1（PC；canonical W25Q128 command/error/page-boundary contract；FOTA adapter range/page-split/error contract） | Q1 部分；通用 QSPI HAL API、STM32L4 QSPI/I2C/UART backends、canonical W25Q128 Device driver 与 FOTA flash-ops adapter；PC/L4/U5 compile；Pandora buzzer/motor/RGB/ST7789/AP3216C/ICM20608 test entries 的 USART1 init/send 与 GPIO AF init 已通过 canonical HAL policy/clean-build gate，hardware-I2C3 PC0/PC1 AF GPIO 也通过 `xy_hal_gpio` | Pandora U9 W25Q128 的 JEDEC、erase、single/quad program/read、recovery 已迁移为 canonical path；hardware I2C3 transaction owner 已从 board-local HAL 直调迁移到 root `xy_hal` 使用的 canonical STM32L4 backend，clean `058bcb95` 镜像取得 117 samples/42 unique 与 bounded address-NACK recovery；SPI1 TX DMA 已取得 completion、re-init 与 active-abort recovery B1/B2。2026-09-06 无 RX source 实板运行按预期停在 `PANDORA_SPI_DMA_RX_ERROR`，未取得 RX/full-duplex 证据 | pending | 允许声明 Pandora canonical storage/I2C3 与 SPI TX DMA B1/B2；UART/GPIO 分层收口仅为 source/compile evidence，不升级既有 board/visual 结果；不得将未接 RX source 的 timeout 升级为 RX/full-duplex、电气或外设响应证据 | 继续迁移 smoke/RTOS/FOTA board entries 的直接 UART/GPIO 调用；使用已确认的物理 loopback/外设补 SPI RX/full-duplex DMA；掉电/NRST恢复 |
