...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0；framework DMA 已取得 DMA1 Channel1 mem2mem polling/IRQ/callback B1 与 software abort→re-init recovery B2；framework SPI1 TX request→DMA1 Channel3→IRQ/callback→OSAL task B1，并取得 deinit→re-init 与 active-transfer software abort→re-init→retransmit B2 | pending | PC simulation、部分 QEMU；Pandora SPI DMA 仅证明未配置 GPIO/外设时的 MCU 内部 TX request/DMA/IRQ/callback 与受控 software recovery 链，不升级电气信号、外设响应、RX/full-duplex、物理故障、性能或 STM32U5 runtime | SPI 引脚/外设响应及 RX/full-duplex DMA B1/B2；DMA error IRQ/timeout/physical fault；AHT10 NACK→recovery B2 |
