...
| PM | H1（public mode dispatch、charger state/status alias、sleep lifecycle/state/error contract；deep-sleep/shutdown 无 backend 时 fail-closed；dedicated UART evidence validator 要求 exact firmware identity、sleep→IRQ wake→state recovery 顺序及零 error marker） | PC/L4/U5 build；STM32L4 shallow SLEEP/WFI 与 PC/U5 compatibility backend 已接入 | Pandora runtime 已证明 PM platform tick adapter 经 `XY_OSAL_AVAILABLE` 使用 canonical OSAL tick，20 个 bounded 周期无 PM error；2026-09-08 当前 clean `29f9ae68` 的 38576-byte sleep/wakeup image 已确认 handler/PM symbols，ST-Link write/verify/read-back byte-identical（SHA-256 `ad7c5aa1...85d7`），但 WCH-Link 在烧录前打开并 flush 后 15 秒仍捕获 0 bytes，记录为 `BLOCKED_UART_NO_DATA` | power pending | Host framework/fallback；public ACTIVE/SLEEP dispatch 委托受检状态转换，charger status compatibility API 返回 canonical complete-state snapshot；DEEP_SLEEP/SHUTDOWN 无 backend 时明确 unsupported；Pandora 当前仅证明 tick adapter B1，sleep/wakeup 只有 image/link/Flash evidence，不等于 runtime、低功耗、关机、功耗或 charger/ADC 实板能力 | 修复/升级 ST-Link VCP 或接入稳定 USB-UART 后，用 dedicated validator 重做 sleep→tick IRQ wake→state recovery marker；后续补电流记录 |
