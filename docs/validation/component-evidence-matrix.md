...
| SYS | H1（timer/SM；默认系统 API fail-closed） | PC build | pending | n/a | timer/state-machine Host 契约；无 board backend 时 reset/identity/version 查询明确返回 unsupported 且保留输出 | Pandora board-owned reset/bootreason/chip-ID strong backend；自动化软件复位 B1/B2，需人工/物理注入的原因保持 pending；U5 仅补充 compile |
...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；双副本 metadata journal partial/corrupt/read-error recovery、generation 半范围歧义与 equal-generation split-brain 拒绝、公开 backend validation、有限 boot-attempt policy、commit/load 与公开状态转换的一致性校验、active-version rollback-floor 与 backend 地址范围校验、callback adapter） | PC/FOTA target build；`projects/stm32u5_fota/main.c` 以 Arm GNU 15.2 Cortex-M33 `-fsyntax-only -Werror` 通过，仅为 enhancement C1 前置 | pending | Secure FOTA blocked；无 approved provider | Host 原型与 U5 source compatibility；缺 signature provider、handoff、delta patch、confirm callback 或有效 metadata backend 均 fail-closed。Pandora 是当前 board integration owner，U5 不再阻塞基础验收 | Pandora board-owned Flash ops/保留区、可链接 image 与 bootloader boot-attempt 调用；自动化 reset/recovery B1/B2；真实掉电、reviewed provider/key provisioning/production patch 仍独立 pending |
