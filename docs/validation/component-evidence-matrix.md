...
| Sensor legacy | H1 | PC build；[active-source manifest](sensor-active-source-manifest.md) 记录 55 个 root sources | Pandora ICM20608 已取得 `WHO_AM_I=0xAE`、单次 init 后配置寄存器回读、静态约 1g 重力与 58 组逐次变化的原始 14-byte burst B1 诊断证据；两次人工移动/约 90° 姿态 capture 未观察到应有动态/重力轴变化，不能作为动态 B1 | pending | `legacy-active-root` 当前根库 Host 契约；Pandora ICM20608 仅确认在线、配置和静态采样，未确认物理运动响应 | 先定位 PCB 上传感器与用户移动对象是否一致；原始 burst 已证明寄存器事务和采样更新，动态/姿态 B1 仍 pending |
