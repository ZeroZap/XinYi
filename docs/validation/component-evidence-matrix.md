...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；版本化 candidate header/layout、完整镜像跨 chunk CRC32、Cortex-M initial SP/Thumb reset vector 校验；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage→attempt→confirm 与 bounded automatic rollback B2；canonical W25Q128 adapter 已将 clean committed Pandora image 的完整 Flash load span 写入带 header 的独立 512 KiB candidate region并在板上执行 header/CRC/vector validation | Secure FOTA blocked；无 approved provider | Pandora metadata/reset recovery、完整镜像 candidate storage 与 bootable-layout/vector validation；不等于独立 bootloader、候选执行或真实 vector jump、真实掉电、安全批准 | 独立 bootloader 与真实 vector handoff/candidate execution；真实掉电 B2；reviewed provider/key provisioning |
