...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta-callback fail-closed） | PC/FOTA target build | pending | Secure FOTA blocked；无 approved provider | Host 原型；缺 signature provider、boot-handoff 或 delta patch callback 均 fail-closed；delta staged-read/callback failure 不假成功；不调用 security-rejected ECDSA placeholder；不模拟成功切槽 | reviewed provider + key provisioning + production patch algorithm + durable boot metadata/mark-valid/rollback/掉电 B2 |
