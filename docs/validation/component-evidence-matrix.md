...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0 正常路径；dedicated STM32L4 framework DMA 已取得 DMA1 Channel1、8-word SRAM→SRAM polling、completion IRQ→framework callback→OSAL semaphore→task B1，以及 active transfer software abort→deinit→re-init→copy recovery B2 | pending | PC simulation、部分 QEMU；Pandora 仅覆盖明确的 framework DMA mem2mem normal/recovery；不升级 SPI/peripheral request、物理故障、超时、性能或 STM32U5 runtime | Pandora framework SPI/peripheral-DMA；DMA error IRQ/timeout/physical fault；AHT10 NACK→recovery B2 |
