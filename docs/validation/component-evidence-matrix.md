...
| Device | H1 | PC/L4/U5 build | Pandora board-owned software-I2C → HAL API → canonical I2C Device helper → AHT10 已取得 B1；AP3216C continuous `0x03` 固件 write/verify/read-back 后，244-sample capture 由 fail-closed validator 确认 first-window IR median 2.5→later rolling median 257.0、error 0，取得 bounded IR stimulus-response B1 | n/a | registry/lifecycle 与 I2C helper 软件契约；Pandora 已证明两个 board software-I2C device/driver 静态正常路径及 AP3216C bounded qualitative IR response；ALS/PS 未取得定量响应，不等于 hardware-I2C peripheral 或 B2 | AHT10/AP3216C NACK→recovery B2；hardware-I2C peripheral；如需产品阈值则补受控距离/照度夹具 |
| Display drivers | H1（SSD1306 init/refresh error propagation、GUI adapter；LCD/LED transaction contracts；ST7789 checked fill 使用 256-byte bounded buffer 并传播 SPI error） | PC/L4/U5 build；Pandora onboard ST7789 SPI3 board target 已链接 | Pandora ST7789 PB3/PB5 SPI3、PD7 CS、PB4 D/C、PB6 reset、PB7 active-high backlight 已按 V2.4 schematic 与 RT-Thread upstream Pandora BSP 绑定；clean `e125038e` image write/verify/read-back byte-identical，UART exact identity + ordered RGB/white/black/pattern markers 且 error 0；Eugene 现场确认 red→green→blue→white→black 完整序列及最终四彩象限/中央黑方块可见，取得 fixed-pattern visual B1；SSD1306 deferred | pending | ST7789 canonical control path及该固定 pattern visual B1；不包含色度、逐像素完整性、撕裂、刷新吞吐、DMA、触摸或耐久资格；SSD1306 deferred 不阻塞 | 如产品需要，另建有仪器的色彩/像素/撕裂/刷新性能与耐久记录 |
