...
| Charger | H1（standalone BQ25620 fake-I2C transaction/status contract；full config 在首个 I2C 写前拒绝越界 input/charge/precharge/termination current、charge voltage 与 recharge threshold） | PC/L4/U5 build | pending | safety pending | `components/drivers/power/charger/xy_bq25620.c` 为 canonical owner；公开 API 与 focused contract 迁移后旧 `components/charger/` compatibility component 已移除；固定 7-bit 地址 `0x6A`，live-transport fail-closed、完整 status staged commit、失败 probe 原子清理和 teardown context 清理已有 Host 证据 | 充电/热故障 B1/B2；Pandora 无 charger IC |
