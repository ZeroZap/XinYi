...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；双副本 metadata journal partial/corrupt recovery） | PC/FOTA target build；`projects/stm32u5_fota/main.c` 以 Arm GNU 15.2 Cortex-M33 `-fsyntax-only -Werror` 通过，仅为源级 C1 前置，不是可链接/可烧录镜像 | pending | Secure FOTA blocked；无 approved provider | Host 原型；flash metadata journal 以 generation、CRC 与 commit marker 选择 newest valid copy，partial/corrupt newest copy 不覆盖上一有效状态；STM32U5 项目 skeleton 的 board handoff/confirm 默认 unsupported；缺 signature provider、boot-handoff、delta patch 或 boot-confirm callback 均 fail-closed；不调用 security-rejected ECDSA placeholder | 将 metadata journal 接入 board-owned Flash 与 handoff/confirm；rollback attempt policy/掉电矩阵；reviewed provider + key provisioning + production patch algorithm + bootloader link gate 与 B1/B2 |
