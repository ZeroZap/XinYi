...
| IPC | H1 | Q1 间接/部分 | Pandora Broker producer→consumer dispatch、单调 payload、Device lookup 与 Trace sink 已完成 bounded B1；depth-2 server queue 已取得 fill→`QUEUE_FULL`→clear→后续 pipeline 恢复 B2；handler rejection 现会向调用者传播并计入 dropped，随后消息可恢复处理，已取得 bounded B2 | concurrency pending | Host 契约 + Pandora bounded task-context 正常、queue saturation/recovery 与 handler rejection/recovery；不等于 ISR ingress、吞吐或多小时资格 | ISR ingress 与长时间 stress |
