...
| GUI | H1（backend 错误传播；SDL fake-library contract；字体 source-table review；licensed required-UI subset active） | PC build；SDL2 opt-in 缺依赖 fail-closed，backend source 已恢复但本机无 SDL2 real link/run 证据 | pending | legacy 字体视觉 `rejected-needs-regeneration` 且 provenance pending；OFL-1.1 Noto Sans CJK SC 的 15 个 required UI glyph 已按 pinned Host snapshot 接入 active 16x16 table，但尚未视觉/实板批准；performance pending | Host GUI/font/backend contract；init/clear/draw/fill/flush backend 错误不再归一化为成功；SDL configure/init/RGB565 framebuffer/flush/quit event 受 fake-library CTest 守护；required UI subset 为 distinct/nonblank active table；其余 legacy 16x24/中文 placeholder 不得作为最终产品字体；SDL2 real runtime 未验证 | SDL2 real source/link/run；人工视觉审查；替换其余 legacy table；屏幕 B1/P1 |
