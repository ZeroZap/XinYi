...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash metadata backend 已取得 commit→load→raw read-back B1：两页保留区 `0x0807E000..0x0807EFFF`、bank-2 erase、double-word program；clean `de7620cd` image write/verify/read-back，UART `FOTA_METADATA_FLASH_OK` 与 raw metadata generation/version/slot 一致 | Secure FOTA blocked；无 approved provider | Pandora metadata persistence 正常路径 B1；不等于 bootloader、候选 image、rollback、掉电或安全批准 | 真实 bootloader boot-attempt/handoff/confirm；双槽候选 image 与自动 reset/rollback recovery；真实掉电 B2；reviewed provider/key provisioning |
