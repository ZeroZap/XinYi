...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；完整镜像跨 chunk CRC32；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage→attempt→confirm 与 anti-rollback B1，并取得 bounded automatic rollback/reset recovery B2；canonical W25Q128 adapter 已完成 bounded storage 及 FOTA core 300-byte 分块 download→read-back→全镜像 CRC B1 | Secure FOTA blocked；无 approved provider | Pandora board runtime 的 metadata callback/persistence、anti-rollback、automatic rollback/reset recovery与 bounded candidate download/CRC；不等于独立 bootloader、候选 image 执行、真实掉电或安全批准 | 双槽候选 image、真实 bootloader vector handoff、真实掉电 B2；reviewed provider/key provisioning |
