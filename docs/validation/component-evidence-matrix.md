...
| Kernel/OSAL | H1（bare-metal） | Q1 部分；[FreeRTOS reference selected](reference-rtos-decision.md)，但 config/STM32U5 Cortex-M33 port/C1 尚缺 | pending | pending | bare-metal Host 契约；FreeRTOS 仅为 Sprint 5 选择，不是 runtime/ISR/并发证据 | FreeRTOS config + Cortex-M33 clean compile；thread/sync/queue/ISR-to-task stress 与 B1/B2 |
