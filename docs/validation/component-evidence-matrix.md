...
| Device | H1 | PC/L4/U5 build | Pandora board-owned software-I2C → HAL API → canonical I2C Device helper → AHT10/AP3216C 已取得 B1；AP3216C continuous `0x03` 另有 244-sample bounded IR response B1。2026-09-07 clean `4873bd91` 将 AP3216C 切至 STM32L4 hardware I2C3（PC0/PC1），write/verify/read-back byte-identical，独立 UART exact identity/config 后 29 samples/17 unique tuples/error 0 | n/a | registry/lifecycle、I2C helper 与 Pandora hardware-I2C3→Device→AP3216C 正常路径 B1；ALS/PS 未取得定量响应，且不等于 NACK recovery 或完整 I2C HAL 资格 | AHT10/AP3216C NACK→recovery B2；如需产品阈值则补受控距离/照度夹具 |
