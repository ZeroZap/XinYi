...
| PM | H1（umbrella 公开并由 Host 编译消费完整 lifecycle/update、mode dispatch、charger state/control、Fuel Gauge state/SOH、ADC sample/helper API；charger 无 board backend 及 hardware init/enable/disable 失败时不提交软件成功状态；sleep lifecycle/state/error contract；deep-sleep/shutdown 无 backend 时 fail-closed；dedicated UART evidence validator 要求 exact firmware identity、sleep→IRQ wake→state recovery 顺序及零 error marker） | PC/L4/U5 build；STM32L4 shallow SLEEP/WFI 与 PC/U5 compatibility backend 已接入 | Pandora runtime 已证明 PM platform tick adapter 经 `XY_OSAL_AVAILABLE` 使用 canonical OSAL tick，20 个 bounded 周期无 PM error；2026-09-08 clean `0cfd921e` 修复 RTOS UART instance 后，38592-byte image 已确认 handler/PM symbols，ST-Link write/verify/read-back byte-identical（SHA-256 `9a57a31b...dad1`）；reset-synchronized WCH-Link capture 取得严格有序 `SLEEP_ENTER→WAKE_IRQ→SLEEP_WAKE_OK`、零 PM error，validator 返回 `B1_PM_SLEEP_WAKE_PASS` | power pending | Host framework/fallback；public ACTIVE/SLEEP dispatch 委托受检状态转换；charger API 在无 backend 或硬件动作失败时 fail-closed，state/status alias 仍返回 canonical snapshot；public ADC sample API 提供 NULL guard 与毫伏输出；DEEP_SLEEP/SHUTDOWN 无 backend 时明确 unsupported；Pandora 已证明 shallow SLEEP/WFI + tick IRQ wake B1，但不等于 STOP/STANDBY/SHUTDOWN、功耗、wake latency、耐久、charger/ADC 实板或 U5 runtime | 补电流、低功耗深度、wake latency 与 endurance 记录；charger/ADC 按产品硬件接入后验证 |
