...
| Sensor new `xy_*` | H1（独立测试） | [manifest](sensor-active-source-manifest.md) 记录 22 个 `experimental-test-only` sources，未进入根 Sensor target；SHT30 duplicate test-local implementation 已移除 | pending | pending | 仅 test-local Host 实验实现；不得因 focused test 宣称 product-linked | MPU6050/ADS1115 ownership 迁移试点 |
| Drivers Sensor | H1（少量；SHT30 Device driver 已覆盖 init/reset I/O、CRC、输出保持、双地址与未初始化 guard；legacy 生命周期委托同一实现） | PC build；[manifest](sensor-active-source-manifest.md) 记录 SHT30/MPU6050/BMP280/ADS1115 四个 `device-active-root` sources；SHT30 canonical owner 同时进入 `xy_drivers` 与 root `sensor_component` | pending | pending | Device-model canonical migration destination；SHT30 只有一个实现 owner，root legacy API 为 compatibility-only wrapper | MPU6050/ADS1115 ownership 迁移 + SHT30 B1/B2 |
