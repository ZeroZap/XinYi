...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed） | PC/FOTA target build | pending | Secure FOTA blocked；无 approved provider | Host 原型；缺 signature provider、boot-handoff、delta patch 或 boot-confirm callback 均 fail-closed；只有 durable confirmation callback 接受后才提交 active slot/anti-rollback floor；不调用 security-rejected ECDSA placeholder | reviewed provider + key provisioning + production patch algorithm + durable metadata backend/rollback/掉电 B2 |
