...
| SYS | H1（timer/SM；默认系统 API fail-closed） | PC/L4/U5 build | Pandora strong backend 已取得稳定 96-bit UID B1、自动 `NVIC_SystemReset`→`SFTRSTF` 与 IWDG timeout→`IWDGRSTF` reason/recovery B2；两条路径均有 Flash write/verify/read-back 与 UART 证据 | n/a | timer/state-machine Host 契约；无 board backend 时 fail-closed；Pandora chip identity、软件复位与独立看门狗复位原因已实证 | 外部 NRST/上电原因区分与 watchdog refresh policy 仍 pending；U5 仅补充 compile |
