...
| Device | H1 | PC/L4/U5 build | Pandora board-owned software-I2C → HAL API → canonical I2C Device helper → AHT10 已取得 B1；AP3216C 已纠正 `0x07` one-shot 误用为连续 ALS+PS `0x03`，`f320087c` image write/verify/read-back byte-identical，12 秒 97 组样本含 60 组不同 raw，证明 fresh register path | n/a | registry/lifecycle 与 I2C helper 软件契约；Pandora 已证明两个 board software-I2C device/driver 静态正常路径及 AP3216C 连续更新，不等于 hardware-I2C peripheral、受控动态传感响应或 B2 | AP3216C 修复后遮挡/靠近动态 delta；AHT10 NACK→recovery B2；hardware-I2C peripheral |
...
| Sensor legacy | H1 | PC/L4/U5 build；[active-source manifest](sensor-active-source-manifest.md) 记录 55 个 root sources | Pandora ICM20608 已取得在线/配置/静态采样诊断 B1；AP3216C 旧 `0x07` one-shot capture 全静态，修为连续 `0x03` 后 97 组中 raw 60 组唯一、PS/IR 持续更新；V2.4 原理图明确 U8=AP3216C | pending | AP3216C freshness 与芯片/寄存器身份已建立，但没有修复后的受控 uncovered/far→covered/near 刺激，旧 207 组全同 capture 仅为诊断；ICM20608 两次动态 capture 未观察到预期响应，均不得升级动态 B1 | AP3216C 修复后独立 uncovered/far 与 covered/near capture 并机器验证 meaningful delta；ICM20608 动态诊断保持隔离 |
