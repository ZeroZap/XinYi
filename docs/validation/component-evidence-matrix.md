...
| Kernel/OSAL | H1（bare-metal） | Q1 部分；[FreeRTOS reference](reference-rtos-decision.md) 已有 project-owned config、pinned V10.4.6 Cortex-M33/CM4F ports；root U5/L4 opt-in 可构建 `freertos_kernel` + `xy_osal` | Pandora STM32L475VE 已取得 scheduler/delay、task-context binary semaphore/message queue/event flags/mutex、SysTick ISR→OSAL semaphore→task、no-wait memory-pool/queue exhaustion→recovery→delete/recreate、depth-one queue 100-tick blocking timeout、120 秒 bounded stress，以及 OSAL→IPC Broker→Device registry→CLIB Trace sink 跨组件 B1 | pending | bare-metal Host 契约；STM32U5 仍为 compile-only；Pandora 仅证明声明区间内的 task primitives、单一 SysTick ISR-to-task、资源恢复、blocking-timeout、bounded stress 与 IPC/Device/Trace 纵切，不是性能、多小时耐久或完整产品 RTOS 资格 | 补外设 IRQ-to-task、PM 跨组件并发、长时间耐久与 B2 |
...
| FOTA | H1（状态机 + signature-provider/boot-handoff/delta/boot-confirm fail-closed；双副本 metadata journal recovery/ordering/state guards） | PC/FOTA target；L4/U5 compile compatibility | Pandora board-owned internal-Flash journal 已取得跨复位 stage handoff→boot attempt→confirm B1：clean `e4828a27` image 11488 bytes write/verify/read-back byte-identical；3 个固件身份与 transition 顺序由 validator 接受，raw metadata 最终为 generation=4/active version=2/min=2/slot=1。clean `33011a76` image 又实证 version 1 在 floor=2 时被拒绝，UART 身份/marker 机器解析通过且 metadata read-back hash 保持 `56c51ef...` | Secure FOTA blocked；无 approved provider | Pandora board runtime 的 metadata callback/persistence 与 anti-rollback floor 正常路径 B1；不等于独立 bootloader、候选 image 执行、自动 rollback、掉电或安全批准 | 双槽候选 image、真实 bootloader vector handoff、自动 reset/rollback recovery、真实掉电 B2；reviewed provider/key provisioning |
