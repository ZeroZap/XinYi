...
| PM | H1（sleep lifecycle/state/error contract） | PC/L4/U5 build；STM32L4 shallow SLEEP/WFI 与 PC/U5 compatibility backend 已接入 | Pandora runtime 已证明 PM platform tick adapter 经 `XY_OSAL_AVAILABLE` 使用 canonical OSAL tick，20 个 bounded 周期无 PM error；sleep/wakeup clean committed image 已 write/verify/read-back byte-identical，但 WCH-Link 3 次 reset 同步捕获均为 0 bytes，runtime 仍未取得 | power pending | Host framework/fallback；Pandora 当前仅证明 tick adapter B1，sleep/wakeup 只有 image/link/Flash evidence，不等于 runtime、低功耗、功耗或 charger/ADC 实板能力 | 恢复独立 UART 后重做 sleep→tick IRQ wake→state recovery marker 闭环；后续补电流记录 |
