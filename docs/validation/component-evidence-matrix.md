...
| Drivers Sensor | H1（SHT30、ADS1115、MPU6050、BMP280、AHT30、L3G4200D、BH1750 focused contracts；BME680 使用 pinned Bosch BME68x API） | PC/L4/U5 build；[manifest](sensor-active-source-manifest.md) 记录 8 个 `device-active-root` sources，均进入 `xy_drivers`；BMP280/BH1750 legacy lifecycle 仍冻结兼容 | Pandora I2C2 PB10/PB11 识别 `0x38`/`0x69`/`0x77`，完成 20 轮三设备 sample；BME680 identity `0x61` | pending | Device-model canonical migration destination；新增三 owner 已取得 basic-chain B1，不等于校准精度、环境舱、动态陀螺性能、长期可靠性或 I2C fault recovery | 为 BME680/BH1750 增加 focused fake-I2C compensation/error contract；补 AHT30/L3G4200D/BME680 NACK/re-init B2 |
