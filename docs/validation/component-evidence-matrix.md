...
| HAL | H1（PC） | Q1 部分；[HAL 平台实现与证据矩阵](hal-platform-evidence-matrix.md)逐平台列明 source/unsupported/compile 状态 | STM32L475 Pandora board-local smoke 已取得 UART/AHT10、LED 与 KEY0；framework DMA/SPI recovery 已有 B1/B2；板载 U9 W25Q128 已经 QSPI `0x9F` 读取 `EF 40 18`，取得 16 MiB Flash 识别 B1 | pending | Pandora W25Q128 仅证明 QSPI 引脚与单线 JEDEC ID 响应；不升级擦写、四线模式、掉电恢复、耐久、FOTA storage、性能或 STM32U5 runtime | W25Q128 专用测试区擦除→写入→读回；四线模式；FOTA candidate storage |
