...
| Drivers Sensor | H1（少量；SHT30 Device driver 已覆盖 init/reset I/O、CRC、输出保持与未初始化 guard） | PC build；[manifest](sensor-active-source-manifest.md) 记录 SHT30/MPU6050/BMP280/ADS1115 四个 `device-active-root` sources | pending | pending | Device-model canonical migration destination；SHT30 test-local duplicate 已移除，但 root legacy lifecycle 仍是 compatibility boundary | SHT30 legacy wrapper 委托/退役 + B1/B2 |
