...
| Device | H1 | PC/L4/U5 build | Pandora board-owned software-I2C → HAL API → canonical I2C Device helper → AHT10 已取得 B1；AP3216C `85942a67` image write/verify/read-back byte-identical，UART 匹配 identity、0x1E ACK、mode 0x07 与 83 组 plausible ALS/PS/IR 样本，错误 marker 为 0 | n/a | registry/lifecycle 与 I2C helper 软件契约；Pandora 已证明两个 board software-I2C device/driver 静态正常路径，不等于 hardware-I2C peripheral、动态传感响应或 B2 | AP3216C 遮挡/靠近动态 delta；AHT10 NACK→recovery B2；hardware-I2C peripheral |
...
| Sensor legacy | H1 | PC/L4/U5 build；[active-source manifest](sensor-active-source-manifest.md) 记录 55 个 root sources | Pandora ICM20608 已取得在线/配置/静态采样诊断 B1；AP3216C 已经 HAL→Device helper→legacy owner 取得 0x1E ACK、mode 0x07、raw 与 converted ALS/PS/IR 静态 B1 | pending | AP3216C 10 秒 83 组 plausible 样本且无错误，但最后 50 组不变；ICM20608 两次动态 capture 未观察到预期响应，均不得升级动态 B1 | AP3216C 需独立 uncovered/far 与 covered/near capture 并机器验证 ALS、PS/IR meaningful delta；ICM20608 动态诊断保持隔离 |
