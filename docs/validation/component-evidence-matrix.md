...
| Display drivers | H1（SSD1306 init/refresh error propagation、GUI adapter；LCD/LED transaction contracts；ST7789 checked fill 使用 256-byte bounded buffer 并传播 SPI error） | PC/L4/U5 build；Pandora onboard ST7789 SPI3 board target 已链接 | Pandora ST7789 PB3/PB5 SPI3、PD7 CS、PB4 D/C、PB6 reset、PB7 active-high backlight 已按 V2.4 schematic 与 RT-Thread upstream Pandora BSP 绑定；当前仅待自动 write/read-back/UART 与人工视觉确认，SSD1306 仍 deferred | pending | Host transaction contract；ST7789 board image 使用 canonical LCD SPI/component API，未取得人工视觉前不得升级 B1/color/orientation；SSD1306 deferred 不阻塞 | clean committed ST7789 image write/read-back/UART，再由用户确认 red→green→blue→white→black 与最终 test pattern |
