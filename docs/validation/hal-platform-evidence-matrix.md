...
`COMPILE_ONLY`，not board runtime evidence。烧录、串口 banner、按键、LED 与 AHT10 数据仍须
保留 B1/B2 原始日志后才能升级。
