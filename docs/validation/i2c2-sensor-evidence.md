...
SC7A22H at `0x18` is compile-linked into the I2C2 probe with a Host-guarded initialization and
conversion contract, but remains hardware-pending: identity, register layout, and data format are
not confirmed.
...
SC7A22H integration has Host/compile evidence only; no hardware result is recorded.
