...
| `crypto_root_target_smoke` | links `xy_tiny_crypto` root target, therefore uses reconciled `components/crypto/xy_crc/xy_crc.c`, reconciled `components/crypto/xy_base/xy_base64.c`, reconciled `components/crypto/xy_hex/xy_hex.c`, reconciled `components/crypto/xy_blake/xy_blake2.c`, reconciled `components/crypto/xy_rng/xy_random.c`, reconciled `components/crypto/xy_rng/xy_csprng.c`, reconciled `components/crypto/xy_md/xy_md5.c`, reconciled `components/crypto/xy_hmac/xy_sha256.c`, reconciled `components/crypto/xy_hmac/xy_hmac.c`, reconciled `components/crypto/xy_aes/xy_aes.c`, limited-runtime LWC module sources `components/crypto/xy_ascon/xy_ascon.c`, `components/crypto/xy_tinyjambu/xy_tinyjambu.c`, and `components/crypto/xy_photon_beetle/xy_photon_beetle.c`, plus `components/crypto/src/xy_chacha20poly1305.c` and `components/crypto/src/xy_ecdsa.c` through the aggregate library; `components/crypto/src/xy_sha256.c` remains intentionally excluded and the byte-identical historical `components/crypto/src/xy_sha256_hmac.c` duplicate has been pruned | Minimal root/runtime public consumer proof for Base64/Hex/SHA-256/HMAC-SHA256, one BLAKE2s vector, ChaCha20-Poly1305 compact wrapper behavior, LWC aggregate-link availability, and ECDSA format-only guard paths; not broad duplicate-source reconciliation or security validation. |
