...
| SM2 | `components/crypto/xy_sm2/xy_sm2.c` | Security-rejected focused-test-only source; excluded from the root subdirectory/runtime target set and retained only for `crypto_sm2` rejection-contract coverage. |
...
| `components/crypto/src/xy_ecdsa.c` | `security-rejected` algorithm entry `ecdsa_root_format_only`; exercised only by focused `crypto_ecdsa_root_contract` | Excluded from root runtime. Simplified verifier returns success after format checks; do not treat it as production signature validation, secure boot evidence, or firmware authenticity validation. |
