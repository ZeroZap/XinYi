...
list(FILTER CRYPTO_SOURCES EXCLUDE REGEX ".*/src/xy_sha256_hmac\\.c$")
...
| SHA-256 | `components/crypto/xy_hmac/xy_sha256.c` | Reconciled SHA-256/HMAC follow-up: root `xy_tiny_crypto` now consumes the same SHA-256 module source as `crypto_hash`/`crypto_cipher_hmac`; `components/crypto/src/xy_sha256_hmac.c` is excluded from the aggregate target because it is a byte-identical historical root duplicate, and `components/crypto/src/xy_sha256.c` remains excluded because it exposes the older `xy_sha256.h` API shape and duplicate `xy_sha256_*` symbols. |
