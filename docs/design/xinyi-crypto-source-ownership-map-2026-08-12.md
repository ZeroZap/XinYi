...
| SHA-256 | `components/crypto/xy_hmac/xy_sha256.c` | Reconciled SHA-256/HMAC follow-up: root `xy_tiny_crypto` now consumes the same SHA-256 module source as `crypto_hash`/`crypto_cipher_hmac`; the byte-identical historical root duplicate `components/crypto/src/xy_sha256_hmac.c` has been pruned, and `components/crypto/src/xy_sha256.c` remains excluded because it exposes the older `xy_sha256.h` API shape and duplicate `xy_sha256_*` symbols. |
...
| `crypto_root_target_smoke` | links `xy_tiny_crypto` root target, therefore uses reconciled `components/crypto/xy_crc/xy_crc.c`, reconciled `components/crypto/xy_base/xy_base64.c`, reconciled `components/crypto/xy_hex/xy_hex.c`, reconciled `components/crypto/xy_blake/xy_blake2.c`, reconciled `components/crypto/xy_rng/xy_random.c`, reconciled `components/crypto/xy_rng/xy_csprng.c`, reconciled `components/crypto/xy_md/xy_md5.c`, reconciled `components/crypto/xy_hmac/xy_sha256.c`, reconciled `components/crypto/xy_hmac/xy_hmac.c`, reconciled `components/crypto/xy_aes/xy_aes.c`, subdirectory-linked SM sources `components/crypto/xy_sm2/xy_sm2.c`, `components/crypto/xy_sm3/xy_sm3.c`, and `components/crypto/xy_sm4/xy_sm4.c`, limited-runtime LWC module sources `components/crypto/xy_ascon/xy_ascon.c`, `components/crypto/xy_tinyjambu/xy_tinyjambu.c`, and `components/crypto/xy_photon_beetle/xy_photon_beetle.c`, plus `components/crypto/src/xy_chacha20poly1305.c` and `components/crypto/src/xy_ecdsa.c` through the aggregate library; `components/crypto/src/xy_sha256.c` remains intentionally excluded and the byte-identical historical `components/crypto/src/xy_sha256_hmac.c` duplicate has been pruned | Minimal root/runtime public consumer proof for Base64/Hex/SHA-256/HMAC-SHA256, SM2 link presence, one BLAKE2s vector, ChaCha20-Poly1305 compact wrapper behavior, LWC aggregate-link availability, and ECDSA format-only guard paths; not broad duplicate-source reconciliation or security validation. |
| `crypto_review_manifest` | `components/crypto/crypto_review_manifest.json` plus `components/crypto/CMakeLists.txt` root-source glob, reconciled module append shape, and SM2/SM3/SM4 subdirectory target source wiring | Policy guard only; not cryptographic validation. It now fails if the root target stops using the mapped source ownership shape without a matching map update, records Base64/Hex/CRC/BLAKE2/Random/CSPRNG/MD5/HMAC/AES as module-owned `single-active-source` runtime paths, verifies root-linked SM subtargets still build their mapped implementation sources, and links the root ECDSA placeholder to an explicit `security-rejected` review record. |
...
1. Keep machine checks that this map and `crypto_review_manifest.json` stay in sync, including the current root `file(GLOB CRYPTO_SOURCES "src/*.c")` collection shape plus explicit Base64/Hex/CRC/BLAKE2/SHA-256 module-source append.
2. Keep source-ownership guards for the current `single-active-source` algorithm set and the ChaCha root-wrapper boundary; no root/module byte-identical duplicate pairs remain guarded after MD5/HMAC/AES/SHA-256 moved to module ownership. Base64/Hex/CRC/BLAKE2/Random/CSPRNG/MD5/HMAC/AES/SHA-256 have left the old pending set because the root target now consumes the module copies, and stale root duplicates have been pruned or excluded from the aggregate.
...
5. Keep ad-hoc build helpers from naming removed historical `src/*.c` duplicates: `components/crypto/build.bat` and `components/crypto/Makefile` now delegate to the canonical CMake `xy_tiny_crypto` target instead of compiling stale root copies directly.
