...
- 已新增 `docs/design/xinyi-crypto-chacha-root-module-reconciliation-proposal-2026-08-13.md`，把 ChaCha20-Poly1305 剩余 split ownership 明确为“root compact API wrapper vs module RFC8439 implementation”的特殊收敛问题：下一步应先补 root-target wrapper CTest，再做 wrapper 委托到 module-source 的小步实现；不得直接删除 `src/xy_chacha20poly1305.c` 或把 module CTest 当成 root/API/security 审查结论。
