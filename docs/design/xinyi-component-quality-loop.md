...
- 已新增 `docs/design/xinyi-crypto-duplicate-source-reconciliation-proposal-2026-08-13.md`，把 duplicate-source 收敛策略限定为分组小步：优先让 `xy_tiny_crypto` 复用 module-directory 源并逐组更新 source map/manifest/checker，禁止一次性删除 `components/crypto/src/`、改名 root target、默认启用 Crypto 或把 host CTest 提升为安全/来源审查结论。下一步若继续 Crypto，应先做 Base64+Hex ownership slice，并跑 root target + focused crypto gates。
