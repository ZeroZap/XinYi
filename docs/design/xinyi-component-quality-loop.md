...
- SHA-256/HMAC root ownership 已完成 module-source follow-up：`xy_tiny_crypto` 现在显式链接 `components/crypto/xy_hmac/xy_sha256.c` 与 `xy_hmac.c`，排除 byte-identical historical `src/xy_sha256_hmac.c` 和旧 API `src/xy_sha256.c`；`crypto_root_target_smoke`/`crypto_hash`/`crypto_cipher_hmac`/`crypto_review_manifest` 共同守护 root 与 focused CTest 使用同一 SHA-256/HMAC module source。后续不应重复做 SHA-256 module-source 接入；只能在真实 consumer failure 或后续 provenance/security 证据出现时补最小 slice。
