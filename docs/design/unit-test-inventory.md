...
| `tests/unit/gui/test_gui_font_manifest.c` | `unity` | 0 | GUI bitmap font manifest/inventory contracts for legacy ASCII/Chinese assets; validates current ranges, required UI codepoints, and duplicate/placeholder inventory; paired with `gui_font_generator_*` Python CTests for generator-bootstrap manifest schema, manifest header write-path, glyph metadata/preview/write-path validation, without claiming generated font quality or hardware validation. |
