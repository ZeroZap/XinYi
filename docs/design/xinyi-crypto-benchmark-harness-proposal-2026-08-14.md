...
3. **STM32U5 compile probe**：已新增 `crypto_benchmark_stm32u5_compile_probe.py` 与默认 plan-only CTest；默认 `make test-unit` 只验证 probe command/evidence boundary，不运行 ARM 工具链。真实 target compile 需要显式 `--run-compile --i-understand-target-compile-only`，结果只能记录为 target compile-only，不声称 timing、hardware pass、安全/来源审查或生产启用。
